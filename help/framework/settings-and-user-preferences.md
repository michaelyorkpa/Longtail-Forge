...
Longtail Forge publishes the subscription name and the owner's current **Profile** timezone in the calendar feed. Current testing confirms Google Calendar uses both values. Outlook asks for the local calendar name before it reads the URL. New and rotated links also end with a path-safe version of the subscription name so Thunderbird can show a friendly name during setup. If the owner timezone is wrong, update that owner's Profile timezone and subscribe again.
