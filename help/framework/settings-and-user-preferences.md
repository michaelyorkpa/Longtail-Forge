...
Longtail Forge publishes the subscription name and the owner's current **Profile** timezone in the calendar feed. Google Calendar does not use the published name when adding from a URL, so rename the calendar after adding it. Google also reports a URL-subscribed calendar's timezone as read-only. Outlook asks for the local calendar name before it reads the URL. If the owner timezone is wrong, update that owner's Profile timezone and subscribe again.
