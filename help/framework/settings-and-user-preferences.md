...
Longtail Forge publishes the subscription name and the owner's current **Profile** timezone in the calendar feed. Calendar clients may keep their own display-name override. Google Calendar reports a URL-subscribed calendar's timezone as read-only, so confirm the imported name and timezone after subscribing; if the owner timezone is wrong, update that owner's Profile timezone and subscribe again.
