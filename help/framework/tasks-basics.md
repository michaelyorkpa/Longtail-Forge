...
The Tasks page supports single-task editing and selected-task bulk changes. Bulk edit can update status, priority, assignees, due date, due time, and tags. Due date and due time can be set or cleared separately; clearing a due date also clears the due time because a task cannot keep a due time without a date. When selected tasks have mixed due dates, due times, or tags, the page shows an in-app confirmation before applying the change.
