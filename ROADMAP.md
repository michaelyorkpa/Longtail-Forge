...
The unchecked-boundary figure was confirmed unchanged at **77 sites across 55 owners**. `0.33.33.32.28.3.2`, `0.33.33.32.28.3.1`, and `0.33.33.32.28.3.3` closed 46 of them across 35 owners, and `0.33.33.32.28.3` closed the remaining 30 across 20. **The unchecked-boundary count is 0.** Two of that 33 are already closed: `build-runtime-artifact` and `sanitized-demo-role-fixtures` annotate a `let` two lines above their parse, and the site classifier only inspects the immediately preceding line. Both were driven directly and answer `unknown` and `object` rather than `any`.

**Execution order.** `0.33.33.32.28.2`, `0.33.33.32.28.3.1`, `0.33.33.32.28.3.2`, `0.33.33.32.28.3.3`, `0.33.33.32.28.4.1`, `0.33.33.32.28.4`, `0.33.33.32.28.4.2`, `0.33.33.32.28.4.3`, `0.33.33.32.28.3`, and `0.33.33.32.28.4.4` have archived, closing the boundary work entirely. Next is `0.33.33.32.28.4.5`, the last helper child, and **`0.33.33.32.28.1` last**. The numbering is historical, not an execution order; `0.33.33.32.28.1` is not renumbered because it is referenced by every closeout rule in this rollup.
...
- [ ] **The recorded family sizes are wrong; measure before migrating.** `0.33.33.32.28.4.1` found family B splits into body-region and block-region owners, and `0.33.33.32.28.4` found that of the remaining local extractors only **12 owners across 48 extractions** are served exactly by the published pair. **50 owners return a third region** - the declaration through the *next* function - which is what the four children below are for. Classify each owner by driving its own helper against the published pair over real sources; do not trust the family label.
- [ ] The two extractor defects are closed. `scripts/test-support/source-scan.mjs` masks comments, strings, template literals, and regular expressions before locating a declaration or walking braces, and refuses a source whose masked braces do not balance. Thirty assertions in `framework.full-strict-governance` pin that behaviour, so changing the scanner fails loudly.
...
- [x] The family-C helper is published. `0.33.33.32.28.4.2` added `extractFunctionSpan` - a declaration through everything that follows it, up to the next top-level function or the end of the source - and sixteen assertions pin it. **A top-level `const` or `class` does not end a span; only the next `function` does.**
- [ ] **Take the equivalence proof at run time, not by reading.** `0.33.33.32.28.4.4` saw 23 extractions statically and **87** by instrumenting each owner - renaming its helper and interposing a wrapper that runs the local and all three published regions on every real call. Owners that call through a loop variable or bind subjects through `path.join(process.cwd(), ...)` are invisible to a static reader, and those are exactly the calls where a vacuity hides.
- [ ] **Two further span properties, both pinned.** A **generator** is refused by all three extractors, but a generator **does not end a span** - a subject declaring one will run its span to the end of the file, the same shape as the exported-function hazard.
- [ ] **Expect terminator variants, not one shared helper.** `0.33.33.32.28.4.3` found three among its eight owners: the common one, one anchoring on the declaration with its open paren so it cannot answer a longer name, and one whose terminator tolerates leading whitespace and therefore ends a span at an **indented** function - which matched the published helper on **none** of its 38 extractions. Where a variant bounds a region the published span does not, evaluate every assertion that consumes it against **both** regions, and require each positive assertion to still be satisfied by the narrower one; otherwise its match is being carried by text it never meant to cover.
- [ ] **Two span properties to check against your owners' subjects before migrating.** An **exported** function does not end a span, because the terminator anchors on a line-initial `function` or `async function`; a family-C owner reading an ES module surface will find its span running to the end of the file. And a source whose functions are indented inside a closure has no terminator at all - 58 of `0.33.33.32.28.4.2`'s 154 spans reach end of file for that reason. Both are inherited from the hand-written helper and must be preserved, not corrected, unless the owner's assertions are re-read with them.
