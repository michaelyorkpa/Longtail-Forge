...
- [ ] **Additive publication is not multi-writer, and the existing record cannot see the difference.** `MULTI_WRITER_RECORDS` carries `kind`, `writers`, `order`, `delivery`, `reason` and `disposition` - a good vocabulary - but it is keyed on surfaces with **more than one writer**, so a single writer that publishes additively is invisible to it. All three blocked surfaces have exactly one writer.
- [ ] **All three spreads are unexercised in the current tree, which is the finding that decides the shape of this child.** `dashboard.js` calls `publishDashboardApi()` from **one** site, is loaded by no page as a classic script, and is not a module-action dependency - it arrives through the ES-module bridge, so it evaluates once. `notes.js` and `lists.js` are each a classic script on their own page **and** a `module: true` module-action dependency, which is the one genuine double-delivery path in the estate - but the dependency descriptor carries a readiness probe naming the surface and member, so the loader skips a file that already published. **No spread in this estate currently preserves anything.**
- [ ] **Therefore the governance answer cannot be derived from runtime behaviour, and that is why it is a policy decision rather than a declaration child.** Each of the three is one of: an anticipated extension point, idempotency insurance for a file with two delivery mechanisms, or dead defensiveness. **`dashboard` is the one that reads as incoherent**: its comment explains that replacing the object would orphan captured references, yet `namespace.dashboard = { ...(namespace.dashboard || {}) }` replaces exactly that object - the comment is about the namespace root, which the same function does mutate in place.
- [ ] **Decide per surface, not globally.** The model may need more than one category, and the categories must stay distinct: one current writer preserving prior members; a deliberately open extension point; genuinely multiple measured writers; and stable shared object identity, which `dashboardBootstrap`'s mutable `Map` already depends on.
...
- [ ] **Land `notesDialog` and `listsDialog` first once the policy exists.** They are the same publication form with the same delivery story and five diagnostics between them, so they test the model twice at low cost; `dashboard` carries zero diagnostics and its incoherent comment means settling it is an archaeology question, not a typing one.
