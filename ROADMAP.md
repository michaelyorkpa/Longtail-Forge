...
- [ ] **`TS2451` reconciliation is necessary but not sufficient.** All 104 redeclaration diagnostics must be gone, but reaching zero does not prove the estate is scoped. `TS2451` counts block-scoped redeclarations only: a duplicate `function` declaration raises nothing at all, and a name declared by exactly one remaining script raises nothing either while still sitting in the shared lexical scope. `0.33.33.33.3` found `normalizeSettings` declared four times with no diagnostic, and `0.33.33.33.4` found `loadUsers` declared twice the same way.
- [ ] **The rollup closes against a direct post-wrap shared-global inventory, not against a diagnostic count.** The inventory lives in `scripts/regressions/framework/full-strict-governance.regression.mjs`, measures top-level-ness by brace depth through the published masker, and is enforced on every run as a shrink-only backlog so no scoped script can silently regress. `0.33.33.33.4.1` corrected the invariant it enforces, which had been stated too broadly:

  1. **Every classic browser script that participates in the shared lexical environment declares zero names in that shared depth-0 scope.** Native ES modules keep ordinary module-scoped top-level declarations; module scope is not leakage.
  2. **`TS2451` reaches zero**, as supporting evidence rather than as the criterion.
  3. **Every application-owned published surface has one named owner**, except explicitly recorded temporary migration exceptions.
  4. **An intentional multi-writer platform wrapper is allowed only through an enumerated, order-proven composition contract.**
  5. **The only expected temporary application-surface exception at close is the Workbench `filesDialog` bridge**, assigned to `0.33.33.34` for retirement.
  6. **`dashboard.entry.js` and `tasks-dashboard.js` remain native ES modules** and are not part of the classic global-scope estate. Each is exempt only while its native-module delivery is mechanically proved **and** the compiler itself classifies it as a module, so runtime scope and TypeScript scope cannot drift apart again.

- [ ] **The estate is 28 classic-script owners, and two files are native ES modules rather than members of it.** `0.33.33.33.4` counted `dashboard.entry.js` as a 30th owner because it declares names at depth 0. It is the page's native ES-module entry: its own header says so, `views/protected/dashboard.html` loads it with `type="module"`, it is the only `type="module"` tag in any view, `scripts/regression-contracts/views/dashboard-es-module-entry.contract.mjs` covers it, and it uses top-level `await`, which is a syntax error in a classic script. Its declarations are module scoped and never enter the classic shared lexical environment, so it must not be wrapped or converted. `0.33.33.33.5` found the second, `tasks-dashboard.js`, by trying to wrap it: its top-level `await` sequences dependency loading before panel registration, so a synchronous wrap is a parse error and an async wrap would break the ordering the importing module depends on. The inventory records both as proved exemptions rather than backlog items, and re-proves each on every run. **Top-level `await` is the structural test: a file that cannot be evaluated as a classic script is not part of the classic estate.**
- [ ] **Multi-writer `window.*` surfaces are classified, not pooled into an allowlist.** Each record carries the exact surface, its exact writers, the architectural reason, its proof, and its permanence. The recorded writers must match the tree in both directions, so a new writer fails and a record that outlives its writer also fails - a retired exception cannot quietly become permanent governance debt.
  - **`window.fetch` - platform-primitive composition, permanent.** `theme-init.js` installs the CSRF guard over the native fetch and `navigation.js` then wraps whatever fetch is current, so protected mutations still carry a CSRF header underneath the 401 handling. This is an ordered decorator chain over a host primitive, not two application modules claiming one Longtail Forge surface. The order is the contract and is pinned from the rendered load order; the composition is behaviourally proved by `scripts/regressions/framework/fetch-guard-composition.regression.mjs`.
  - **`window.LongtailForge.filesDialog` - temporary migration, retires in `0.33.33.34`.** `files.js` is the canonical owner and publishes the whole editor and preview surface. `workbench.js` conditionally merges a preview-only compatibility bridge and returns early when the canonical publisher is present. No third publisher may appear.
  - **The `window.LongtailForge` root is not a surface.** Every contributor opens it with an idempotent bootstrap, and the inventory checks that each root write extends the namespace rather than replacing it.

**`0.33.33.33.6` has archived**, scoping the six record module page controllers and taking **`TS2451` to 0** - the rollup's stated reconciliation target, reached before `0.33.33.33.7`. **1,215 names leave the shared global scope** and the browser program falls from 11,041 to 10,528. It also produced the rollup's sharpest governance finding. Scoping changes which declaration an identifier resolves to, so five files began resolving their own `state` literal instead of another page's, and **twenty per-code counts rose while every one of those files' totals fell**. The shrink-only ledger rule is written per code because that is a good proxy for "no new debt"; here the proxy and the thing it stands for disagreed. The answer is **governed reclassification**: a movement is permitted only when it names an exact file, an exact code, and the exact counts either side, the file's own total strictly falls, the program's total does not rise, and the file shares a top-level identifier with an owner the checkpoint scoped. No wildcard, no per-file blanket, no proportional allowance, and no place to defer `0.33.33.39`-`0.33.33.44` typing work. The records were struck once spent and the exact movement is recorded in the archive. **72 of 75 classic browser scripts are now out of the shared lexical environment**, leaving only the three `0.33.33.33.7` owners.

**`0.33.33.33.6.1` and its `0.33.33.33.6.1.1` hardening pass have archived**, and `0.33.33.33.6` has not started. The hardening closed the hole the depth correction opened: reading depth rather than column is what makes a same-depth function expression look like a declaration, and three of four expression forms were ending spans they had no business ending. A statement-position test fixes all of them and subsumes the `export function` special case. The first owner batch of `.33.6` hit a failure that was not about scoping at all: wrapping `lists.js` broke `lists-ui-workflow-regression`, because the published `extractFunctionSpan` helper located "the next top-level function" by looking for a `function` keyword at column 0. Every IIFE-wrapped file indents its declarations, so the span silently ran to the end of the file. **`0.33.33.32.28.4.2` had pinned exactly that behaviour on purpose** - 58 of its 154 spans already asserted against the widened region - so this was a direct conflict between a deliberate contract and the scoping programme, not an oversight. By `.33.5` the rule had converted **112 of the suite's 289 resolvable span extractions into whole-file reads across 13 regressions**, which turns a `doesNotMatch` assertion into a false failure and a `match` assertion into a vacuous one. Per the `.33.6` proviso the owner was stopped and the work resliced rather than allowed to expand the checkpoint: the helper now ends a span at the next declaration at the **same brace depth**, no extraction in the suite reaches end-of-file, and `.33.6` may now proceed unchanged.

**`0.33.33.33.5.1` has archived**, a corrective that made TypeScript's lexical scope match the browser's for both native ES modules. `0.33.33.33.4.1` and `.33.5` proved those two files are modules at runtime; the compiler was still modelling them as global scripts, because TypeScript decides module scope from syntax alone and neither carried a top-level `import` or `export`. **The compiler said so itself**: both reported `TS1375`, "this file has no imports or exports", and a probe confirmed a classic script could resolve `loadContributedAssets`, `dashboardPanelRoute`, `DEFAULT_TASK_SUMMARY_ROUTE`, and `hydrateTasksDashboardPanel` through the fake global scope. Each file now carries an empty `export {};` marker; the six `TS1375` diagnostics are the only ones that moved and **nothing was newly exposed**, so the hazard was real but unexploited. The inventory's exemption now requires both halves - mechanically proved native-module delivery **and** the compiler's own classification - so a file cannot be exempt from the classic inventory while still contributing to the type system's global scope.

**`0.33.33.33.5` has also archived**, scoping the reporting, calendar, and dashboard controllers and correcting the estate once more. It produced the cleanest evidence yet for why this rollup does not close on `TS2451`: **`reporting.js` and `calendar.js` carried 82 names in the shared lexical environment and not one diagnostic between them**, because every one of those names happened to be unique. Wrapping both moved the browser count by exactly zero. A rollup that closed on `TS2451` alone would have declared these two files finished while they were still sharing 82 names. **`tasks-dashboard.js` is a second native ES module**, found by trying to wrap it: it uses top-level `await` to load its dependencies before registering a panel renderer, so a synchronous IIFE is a parse error and an async one would let `import()` resolve before registration. It is exempt for the same structural reason `dashboard.entry.js` is, and the estate is **28 classic-script owners**. The two `dashboardDataPromises` redeclarations cleared exactly as `0.33.33.33.4.1` predicted they would. `TS2451` is now 64 and the browser program is 11,047.

**`0.33.33.33.4.1` has archived**, a planning and governance corrective that made the inventory `0.33.33.33.4` introduced measure the right thing. It changed no browser controller. The inventory was directionally right and too broad in two places. **`dashboard.entry.js` is not a leaking owner**: it is the repo's one native ES-module entry, proved by top-level `await` - a syntax error in a classic script - and by being the only `type="module"` tag in any view, so its top-level declarations are module scoped and the estate returns to **29 classic-script owners**. The invariant is now stated as classic-script participation rather than brace depth alone, and the exemption is re-proved on every run instead of being asserted once. **`window.fetch` is not a contested surface**: `theme-init.js` installs the CSRF guard over the native fetch and `navigation.js` then wraps the fetch it receives, which is an ordered decorator chain over a host primitive; the order is pinned from the rendered load order and the composition is behaviourally proved. **`window.LongtailForge.filesDialog` is a temporary migration exception** with `files.js` named as canonical owner and `0.33.33.34` named as the retirement checkpoint. Publication governance now distinguishes permanent single-owner application surfaces, order-proven platform-wrapper composition, and dated temporary co-ownership, and a record that outlives its writer fails just as an unrecorded writer does.

**`0.33.33.33.4` has also archived**, scoping the six administration and support controllers and building the inventory the rollup now closes against. It confirmed `0.33.33.33.3`'s finding on a second name: `loadUsers` was declared by both `lists.js` and `user-admin.js`, and `lists.js` had been calling the `user-admin.js` copy, which returns nothing - so its `users` value was typed `void`. Scoping `user-admin.js` made `lists.js` resolve its own loader and the value became what the API boundary really yields. **Two silent function collisions in two consecutive children is a pattern, not a coincidence.** Its inventory also reported `dashboard.entry.js` as a 30th owner, which `0.33.33.33.4.1` corrected: that file is a native ES module and the estate is 29. The child also found that scoping these owners moved `workbench.js`, which had been assigning to what the compiler thought was `support-view.js`'s `const statusText` and therefore carried a bogus "cannot assign to a constant" diagnostic. Four of the six pages had **no rendered coverage at all**, so a console-clean spec now loads each of them and asserts at runtime that the controller declares nothing in the page scope. `TS2451` is now 66 and the browser program is 11,049.
...
**Reconciliation: the estate is 28 classic-script owners, each in exactly one child, no duplicate and no orphan. `0.33.33.33.1` through `0.33.33.33.6` have taken 25; the table below is the remaining 3. `dashboard.entry.js` and `tasks-dashboard.js` are native ES modules and are not in the estate.**

| Child | Load surface | Owners | Lines | Diagnostics |
| --- | --- | --- | --- | --- |
| `.33.7` | Time tracking and orchestration controllers | 3 | 6,404 | 1,309 |
| **Remaining** | | **3** | **6,404** | **1,199** |
