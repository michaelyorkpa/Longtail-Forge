...
**Reconfirmed unchanged after `0.33.33.38.2.2.8` and `0.33.33.38.2.2.3` landed.** Still nine members, still 44 diagnostics across the same five files - now measurably **22 namespace and 22 `unknown`** - and `settings-renderer.js` still carries **49 unannotated parameters**. The two dead `showSaveAction: false` arguments are still at `user-settings.js:504` and `:512`, and the name still appears nowhere in the renderer. Neither `status` nor `modal` moved anything underneath this child, which is what the reslice predicted: **the blocker is the writer, not the neighbours.**
...
#### 0.33.33.38.2.2.4 - `LongtailForge.icons`

**Split from `tags` after the post-`modal` remeasurement. The two were grouped by consumer overlap, and their writers disagree about everything that decides a slice.**

`shared/icons.js` is **280 lines** publishing a plain `{ createIcon, createIconButton, decorateButton, names }` - **four members**, one writer, no asynchronous path, and no wire body anywhere in the file. Every member either returns the element it built or throws. **53 canonical diagnostics across 17 consumer files, every one of them `TS2339` on a `{}` receiver, and 0 genuine `unknown`.**

- [ ] **Every read in the estate is guarded, and that is the contract as much as the signatures are.** A guard-dominance audit - does any earlier line in the enclosing function test the surface? - finds **54 guarded reads and 0 unguarded ones** across 17 files. Fifty-three fall back to a plain button or plain text; **exactly one cannot**, and `task-dialog.js:2573` already throws its own dependency error. The instrument was proved by removing a single guard and watching that site, and only that site, flip to required.
- [ ] **The guards exist because the script is lazily injected.** `footer.js` loads `js/shared/icons.js` on demand for quick actions and probes for it twice. **Absence is a real state on a real page**, so this child declares the member and changes no consumer - the `0.33.33.38.2.2.1` model rather than the `0.33.33.38.2.2.3` one. **Do not introduce a checked accessor here**; there is nothing for one to make explicit.
- [ ] **Four surface aliases exist and a dotted search finds none of them** - `stop-watch.js:1235`, `task-dialog.js:528`, `workbench.js:510`, `shared/task-calendar.js:357`, two optional-chained and two direct. All four are followed by a guard, so none needs converting; they are recorded because the estate count is wrong without them.
- [ ] **The writer's nine unannotated parameters belong to `0.33.33.39` and stay there.** Drafting the contract and wiring the member leaves `shared/icons.js` at exactly its 28 diagnostics with nothing new inside it, and the estate-wide `TS7006` total unmoved. **This is not the `settingsRenderer` failure mode**: there the published `collectPayload(scope = document)` inferred a type too narrow for its own callers, so the writer had to be annotated before the publication could typecheck. Here the inferred parameter types are wide enough to satisfy a truthful contract, and the debt is genuinely independent.
- [ ] **`createIcon`'s `name` is `string`, not a union of `names`.** The registry is closed and the writer throws on a miss, but callers compute the name - `checklistActionIcon(action)`, and the view factory's descriptor `icon` - so a union would reject calls the runtime accepts today. **Runtime decides the vocabulary**, the same way it did for `status.type`.
- [ ] **Thirteen source contracts read `shared/icons.js` itself and none asserts a consumer expression**, so nothing needs retargeting. That is the opposite of `0.33.33.38.2.2.3`, where ten contracts pinned the receiver at the call site.

#### 0.33.33.38.2.2.10 - `LongtailForge.tags`

**Split out of `0.33.33.38.2.2.4`. It resembles `settingsRenderer` far more than it resembles `icons`, and it must not inherit `icons`' clearance.**

`shared/tags.js` is **838 lines** publishing **twelve members** - `NO_TAGS_FILTER_VALUE`, `allTagsOption`, `createTag`, `createFilterOption`, `loadTags`, `mountFilterPicker`, `mountPicker`, `noTagsOption`, `readTagIds`, `renderTagList`, and `suppressPropagatedTag`. **21 canonical namespace diagnostics across 4 files, 28 optional-chained reads, 21 direct, 2 probes, and no surface aliases.**

- [ ] **This one has a wire boundary and `icons` has none.** `loadTags` and `createTag` `fetch` and return parsed JSON - `Array.isArray(body.tags) ? body.tags : []` - so a truthful contract has a genuine `0.33.33.38.4` seam inside it. Do not type the parsed body by guessing the shape the server sends today.
- [ ] **The writer carries 64 unannotated parameters against `icons`' 9.** That is the profile that blocked `settingsRenderer` at 49, and it is why this child gets **its own writer-first preflight** rather than following `icons` on the assumption that a sibling's clearance transfers. **Measure whether the declaration forces writer annotation before promising anything.**
- [ ] **Do not let this child inherit `icons`' delivery finding either.** `footer.js` probes for `tags` as well, but the required-and-optional split is its own measurement.
