...
#### 0.33.33.38.3.11 - Framework element-with-parts reads

**Complete: one shared-contract addition, three `dom`.** See the archive entry. `LongtailForge.view.partsOf(element, kind)` answers the parts of an element the framework built as that kind, from the framework's own record, and `null` for anything else. The Clients/Projects bulk toolbar and the Lists picker adopt it. Browser 175 to 172 and `dom` 11 to 8; `lists.js` has no `dom` left.
