...
| Codex | `0.33.33.42.43` | `workbench.js` — related-context envelope integrated; checked-DOM adoption `0.33.33.42.44` next, then the Inspector candidate (ruled) | 1 owned |
| Claude | `0.33.33.38.3.10` | `lists.js` — checked-DOM adoption, in progress; `clients-projects.js` at 143 after `0.33.33.38.3.9`, whose 10 remaining `dom` are not lookups | 91 |
...
- **The `dom` family is a Lists boundary** — recorded by `0.33.33.43.24`, **largely resolved by `0.33.33.43.25`**, which narrowed 25 module handles and took the global family **149 to 92**. What remains in `lists.js` is nullability rather than subtype: 55 `possibly null` reads that are unguarded assignments, so closing them is a behaviour decision about what an absent control should do, not a typing one. The lookups are now duplicated across six modules (sixteen near-identical helpers); whether they belong in the shared view layer is an open framework-ownership decision. **Since decided (2026-09-25):** they do - the shared checked-DOM contract is `0.33.33.38.3`, Claude owns it and its adoption in Claude-owned files, and Codex adopts it in Workbench after it integrates, preserving required-versus-optional controls and the lazy and dialog-only initialization paths already ruled. **Published by `0.33.33.38.3.9`** as `LongtailForge.checkedDom`, injected into every rendered page and adopted first in `clients-projects.js`. Superseded detail below. `lists.js` holds **118 of the global 149**, and those are not incidental: the module handles are declared by `document.querySelector`, which answers `Element`, so `option`, `replaceOptions`, `populateProjectOptions`, `decorateFilterControl` and `decorateListEditorField` cannot be typed from above at all. Two checkpoints have now deferred annotations for this one reason. **`0.33.33.44` cannot close without it**, and it should be scheduled as its own boundary — narrowing the handles where they are declared — rather than met piecemeal from each consumer. Owner: Claude, with `lists.js`.
- **Workbench closeout rulings, 2026-09-24 - three authorized behaviour changes, no longer escalated.** Each is scoped to the named boundary and must be proved through the real path; none licenses a broader change.
  - **Related-context envelope - option (a).** A failed load stays a failed load: a null response or a nullish entry inside `groups` produces the error state, never the empty-success state. The native nullish-read wording is replaced with the existing "Related task context could not be loaded." Legitimate empty results, the default-argument behaviour and the fallbacks for an **absent** optional collection are preserved - a null entry inside `groups` is not an absent `groups`. The unknown response is not asserted to `TaskFocusRelatedEnvelope`, and no broader rejection is introduced. A late response for a previously focused task still cannot overwrite the current one.
  - **Inspector candidates - option (a).** Candidates stay opaque and keep their identity; only the members the Inspector consumes are read, through checked reads. No validator, filter, clone, normalisation or manufactured identifier. Ordering, deduplication, limits, label fallbacks, action availability, navigation and focus are preserved, optional reads stay optional and required stay required. The candidate from the collection must be the same object the action handler receives.
  - **Elapsed time - option (b), overriding the integrator's recommendation of (a).** `readElapsedSeconds` adopts the policy the server already applies in `activeTimerElapsedSecondsAt` (`src/modules/time-tracking/active-timers.service.js`): after the existing parse, an invalid or non-finite start time returns the accumulated base seconds, with no running increment. No reset, fabricated start, write-back or estimate; valid-input behaviour, paused timers, absent start times, whole-second rounding and future-start clamping are unchanged. Tests control `Date.now` inside the sandbox - no production fixed-Date bridge or date abstraction. `readElapsedSeconds` feeds timer actions and finalization as well as rendering, so both callers are traced and valid-timer requests proved unchanged. Recorded as an authorized malformed-input fix: the browser should not show a misleading zero because normal server writes happen to normalise timestamps.
  - **Start timestamps - option (a), 2026-09-25, and no permanent exception.** Extends the elapsed-time ruling above rather than replacing it. A start timestamp used to calculate a running increment must be a **primitive string**; an absent, non-string or unparseable start produces accumulated time only. Accepted-string parsing is preserved - no strict ISO-only validation, coercion, timestamp replacement or timer deletion - and the timestamp may be read once into a local and that same value validated and parsed. This settles the input contract that had left `readElapsedSeconds`'s parameter undeclared, so **no diagnostic exception is retained**. Its effect is recorded in three parts, not one: the pre-fix failure; the browser's calculation and request correction, keeping `finalizeTimer`'s `Math.max(1, …)` minimum and its end-minus-duration start; and the observed server result - pause persists the value the browser sends, while finalization of a stored active timer takes its duration from that stored timer through `finalizedTimerFacts`, so a valid outgoing request does not by itself establish the saved duration. It is not described as repairing every malformed timer or restoring time already lost.
  - **The two `dataset` reads stay with the framework-wide `dom` boundary** - no Workbench-only workaround, no change of diagnostic ownership. Raw, Workbench-owned and `dom` counts are reported separately, from the canonical tools against the same tree.
- **Clients/Projects builds two client shapes, and reads a member it never writes** — recorded by `0.33.33.43.32`, **both resolved by `0.33.33.43.33`** under an explicit operator ruling. "Workspace projects" is a **synthetic grouping, not a client**: it is not editable as a client, creates no child clients and joins no hierarchy, so it keeps no `parent_client_id`, `canCreateChild`, `canManage` or `tags` — absent because it has none of them, not defaulted to `false` — while its project permissions are untouched. The two records are now built by their own producers and united as `NormalizedClientEntry`, discriminated by `isWorkspaceScope`. The always-zero `workspaceProjectCount` was fixed **as a bug**, counting the grouping's own projects with no second collection kept for counting. The deferral pins are replaced by proofs of the approved behaviour, including a nonzero count and preserved project permissions. Carried no longer.
...
#### 0.33.33.38.3.10 - Lists checked-DOM adoption
...
- [ ] Keep Lists' initialization exactly: workspace readiness, then shell construction, then `cacheListsElements`, then binding, plus the separate synchronous dialog-only path. Every capture keeps its point, root and lifetime; nothing moves to module evaluation.
- [ ] `findListsFormControl` becomes a thin adapter over one shared lookup that keeps its four-type union and its single query. The single-type helpers move onto the shared `find`. No local helper is removed only to make the migration look complete.
- [ ] `require` is used only on handles whose subtype and `null`-only absence are established, at the point the page already reads each one unguarded, never at capture.
- [ ] Each `TS2339` site is traced to its markup. One that is not a lookup is reported, not forced through the contract.
- [ ] The real shipped module is loaded in every affected sandbox. Required, optional and dialog-only cases, desktop and mobile rendered coverage, a range-explicit `verify:slice` on a clean committed tree, and the full suite to `Completed N/N`.

#### 0.33.33.38.3.9 - The shared checked-DOM contract, and its first adoption

**Complete: a framework surface on every rendered page, and 17 lookups in one file.** See the archive entry. `LongtailForge.checkedDom` is two steps - `find` answers the subtype or `null`, `require` refuses that `null` by name - so a page keeps its capture lifetime and failure timing. `clients-projects.js` fell **160 to 143** and its `dom` **27 to 10**; params, state and assorted did not move, and no message rose.

**Each site was decided, not defaulted.** Four controls the page builds itself are required; the add-project submit button keeps its fallback; each sort row is required rather than dropped. The ten `dom` left are not lookups, and none was forced through the contract.
...
**The subtypes were traced to the markup before a narrowing was chosen**, which is the lesson `0.33.33.38.3.4` paid for. The break set is 20 of 20 refused.
...
#### 0.33.33.39.13 - Correct the modal footer declaration and type the element factory

**Complete: the declaration now matches the writer, `createElement` is typed, and `view-builder.js` falls 382 to 372.** See the archive entry.

**`BrowserViewModalParts.footer` is `HTMLElement | null`**, because `createModal` builds a footer only for `actions.length || options.footer` and had always attached `null` otherwise. **`BrowserViewModalFormParts.footer` stays `HTMLElement`**, because `createModalForm` builds its footer on every path - a form modal keeps the stronger guarantee rather than inheriting the base's nullability.

**No consumer needed changing, and that is a measured result rather than an assumption.** Every unguarded `viewParts.footer` read in the estate is on a **form** modal - `notes.js` x3 and `files.js` x1 through `renderDescriptorModalForm`, `task-dialog.js` through `renderDescriptorModalForm` and `createModalForm`. The only plain-`createModal` reader, `shared/file-preview.js:228`, already guarded with `if (dialog.viewParts?.footer)`. Dropping the form-modal guarantee in a probe produces exactly those six consumer diagnostics, which is the evidence that keeping it is what spares them.
...
#### 0.33.33.39.25 - Narrow the published action-list input to BrowserViewAction

**Complete.** See the archive entry. `renderDescriptorActionMenu`, `renderDescriptorActionStrip` and `renderDescriptorInlineActions` publish `actions?: readonly BrowserViewAction[]` instead of `readonly unknown[]` - the shape their builder already consumed. **A published compile-time narrowing with no intended runtime change**; zero new diagnostics shows compatibility with the repository's checked callers, not with every possible external one.
...
#### 0.33.33.39.38 - Take the task calendar to zero
...
#### 0.33.33.43.44 - The Lists page refused every real list

**Complete: a correction to this lane's own `9427cf2d`.** See the archive entry. From 2026-09-02, any workspace with a list showed "The list collection could not be read.", because the page checked responses against the database column types while the repository's row mappers booleanize `is_reusable` and parse `metadata_json`. The page now reads the shape actually sent. The response suite runs the real row mappers, and a rendered case loads a real list, so neither half can recur unseen.
...
**All four stages together, in their real order.** `withInitialProjectClientFilter` runs on the Projects page path alone, then the client-field, billing-field and unavailable-action filters. Two of them had been split out of `.43.40` and `.43.41` so that no stage would be typed on its own.

**The descriptor is opaque below its root, so every nested read is checked where it happens.** `viewSurfaces` arrives as `unknown[]`, and the surface's own predicate vouches only for the root record. The whole surface is not declared a descriptor it was never proved to be. **A malformed section still fails, as it did before; it is not quietly treated as absent**, because this slice carried no authority to alter behaviour and degrading a throw into a render would be exactly that. The throw is unobservable to a user: it reaches the shared `browser-recovery` boundary, which shows fixed copy.

**Proved against the implementation it replaced, over the real descriptors.** The real composed function runs over 48 scenarios - both pages, three workspace types, the context and settings sources of the workspace type, both permission outcomes, with and without `?client=` - against the registry's own descriptors round-tripped through JSON. Output and **every member read, in order**, match the baseline. **Swapping the client and billing stages leaves the delivered output identical** - they remove disjoint members - **and only the read-order comparison catches it**, which is why the order is proved by reads and not by output.
...
**Complete: three behaviour defects fixed under an explicit ruling, one deferred diagnostic discharged with them.** See the archive entry. `clients-projects.js` 277 to 276, browser **378 to 377**. The count is not the point of this checkpoint.

**All three were the same mistake in different places.** The draft was pushed into `targetClient.projects` before the request went out - invisible on success, because the refresh replaces the whole collection, but on failure the refresh never ran and an unsaved draft stayed in the saved-project collection carrying no `id`. `onSaved` was called unconditionally, so a failed create closed the dialog and discarded what had been typed - **while the project editor's save, the archive action and client creation all already gated on their result**, making this a consistency fix rather than a new rule. And a committed write followed by a failed refresh reported "were not saved", which is precisely the message that produces a duplicate record.

**The persistence wrapper now has two phases.** Past the write, nothing may report the change as unsaved: a failure in reading the tree back, restoring the open rows or refreshing the read surface leaves a stale view, not a lost record, and the message leads with `Saved,` and says to reload. The refused-write path is unchanged.

**The deferred `TS2345` closed itself.** `0.33.33.43.33` could not declare `getWorkspaceProjectClient`'s return because doing so reported this very push. With the push gone the diagnostic is gone - but the declaration still does not land, for a **different and newly-found reason**: the stand-in grouping is missing `taskReminderPolicy`, which the builder produces. Nothing reads that member off a grouping today, so the gap is latent; closing it changes what the object carries and is its own decision.

**Two things observed and deliberately not taken.** On a post-write failure the host module-action still does not complete - pre-existing for every failure path, unchanged here. And the stand-in duplicates thirteen lines of the builder; consolidating them is the same decision as the missing member.
...
**Complete: 48 diagnostics closed, zero introduced, and one live defect repaired.** See the archive entry. `clients-projects.js` 350 to 302, browser **456 to 408**. `dom` unchanged at 90. The largest single-checkpoint fall in this file.

**The decision `0.33.33.43.32` reported was taken, and it was not to merge the two shapes.** "Workspace projects" is a synthetic grouping of the projects that belong directly to the workspace - not a client, not editable as one, creating no children and joining no hierarchy. It keeps no `parent_client_id`, `canCreateChild`, `canManage` or `tags`, **absent because it has none of them rather than defaulted to `false`**, and the consumers were checked before that was chosen: every hierarchy and editor reader already reaches clients through `getRealClients()`. Each record is now built by its own producer, so each literal names its own type and neither can be restated wrongly; `NormalizedClientEntry` unites them, discriminated by `isWorkspaceScope`.
...
**Complete: 37 diagnostics closed, zero introduced, in a file no lane had touched.** See the archive entry. `clients-projects.js` 391 to 354, browser **499 to 462**, `0.33.33.43` 394 to 357. `dom` unchanged at 90.

**The file was measured before it was sliced** - 364 owned across 142 functions, largest 16, a long tail from the first line. The readers were taken first because that is where the contract work is.

**A membership test keeps the vocabularies text rather than widening them.** Declaring the four closed vocabularies `readonly unknown[]` was probed and rejected: it compiled but pushed `unknown` into three consumers needing text. `vocabularyHas` guards on `typeof` and delegates, which answers exactly what `includes` answered - measured across fifteen values.

**One cascade was measured and not banked**, and it is not the defect it first looked like: the member that surfaced, `canManage`, is genuinely built by the normaliser, and a case asserts that so the next checkpoint does not chase it.

#### 0.33.33.43.30 - The module-action parameter root, half discharged

**Complete: 5 diagnostics closed, zero introduced, and `TS2322` and `TS7031` both reach zero in this file.** See the archive entry. `lists.js` 96 to 91, browser **507 to 502**, `0.33.33.43` 399 to 394. `dom` unchanged at 90.

**The root was split by measurement, not by argument.** Both blocked readers end in a `||` chain over `unknown` members, and templating it is the conversion that already happens one step later - so the question was only where the result goes.

**The defaults half is discharged**: every default lands in `control.value = x || ""`, and templating writes the same text for all ten kinds probed, including the three that are truthy yet stringify to empty.

**The identifier half cannot be, and the reason is control flow.** `openListEditor` tests the result for truthiness *before* anything converts it, so those same three kinds would turn a fetch that happens today into one that does not. The deferral now records the measurement and names its discharge as a product decision.

**It unblocked what `0.33.33.43.23` measured and refused** - the dialog's option bag, and the `Partial` local that checkpoint had to back out.
...
**Complete: 13 diagnostics closed, zero introduced, nothing executable changed.** See the archive entry. `lists.js` 160 to 147, browser **575 to 562**, `0.33.33.43` 463 to 450. `dom` unchanged at 90.

**The wall this boundary was warned about did not apply.** `bindListsEvents`' four are module-action registration parameters, not listener parameters; the submit-listener wall `0.33.33.43.23` recorded stands untouched and unrelated.

**Four readers share one root, now recorded once.** `BrowserNormalizedListRecord` declares its columns optional because a draft has none, so every reader turning one into something the platform requires as text is blocked by that alone - `runAction`, `moveItem`, `listIndexItem`, `detailActionButtons`. `0.33.33.43.23` had recorded the first against `moveItem`; the four now share one note and **one** discharge condition, pinned so they cannot drift back into four.
...
**Complete: 60 diagnostics closed, zero introduced, and the `dom` family moves for the first time in this campaign.** See the archive entry. `lists.js` 239 to 179, browser **660 to 600**, **`dom` 149 to 92** - 57 of the 60.

**The shape is `files.js`'s, and the union names the member rather than the tag.** `findListsFormControl` accepts the four elements carrying `value` and `disabled`, which proved load-bearing: the filters are a genuine mixture of input and select, so a lookup per tag would have silently nulled one.

**The safety claim is about the real DOM, and is settled against it.** `tests/e2e/lists-element-handles.spec.mjs` checks all 25 handles on the rendered page, desktop and mobile, and was proved by making it fail.

**It discharged `0.33.33.43.24`'s deferral.** `replaceOptions` takes a select because it reads `.options`. Two deferrals survive and both moved to a different reason - nullability and the unvalidated users payload - neither of which is a `dom` question any more.

**Reported, not decided:** these lookups now exist in six modules, sixteen near-identical helpers. Whether they belong in the shared view layer is a framework-ownership decision this checkpoint did not make.
...
**Complete: 15 diagnostics closed, zero introduced, and `0.33.33.43.19`'s first deferral discharged.** See the archive entry. `lists.js` 327 to 312, browser **761 to 746**, `0.33.33.43` 573 to 558. All 15 are `params`; only `TS2339` moves, 111 to 96.

**The checker went where the deferral said, not where the diagnostic was** - declared with `isListSummary` and `readListDetail`, consumed by the normaliser. The published `progress` member stays `unknown`, which is what `0.33.33.43.19` proved it had to.

**One input changes behaviour, and it is a latent crash.** Nine input kinds read identically through the checker; `null` alone differs, because `progress.assignedUserIds` was a bare member read and `isListSummary` never looked inside the bag. A server sending `progress: null` crashed the list.

**A malformed bag no longer costs the whole list** - rejecting the summary would drop a real list over one member, so an unusable bag falls to the counts derived from the items.

**Three suites lift this normaliser, not one** - giving it a dependency broke nine cases across two other files, and every consumer was then found by grep rather than by the next failing run.

**Two pins fired because their reason stopped being true, which is the design working**, and were replaced by cases guarding the discharge. A third appeared to fail only because reflowing the note split its pinned phrase; the note was rewrapped rather than that pin edited.
...
**Complete: 17 diagnostics closed, and the record is declared where the estate declines to publish it.** See the archive entry. `fileRow` and the small readers it feeds, plus the row focus lookup.

**The estate refuses to name this shape for consumers**, typing it `attachment?: unknown` on every published surface that carries one, so it is declared locally inside the only mapper that reads it - both spellings, every member optional, and nothing proved.

**One executable line changed.** `fileRow` now defaults `fileId` like every sibling id it builds; that was the only id left un-defaulted, and the inconsistency is why the row could not describe its own id.

**The `fileId` deferral from `0.33.33.43.8` was attacked and survives, for a reason now recorded in the source.** Defaulting at the producer is real, but `FileEditorRow` describes rows generally and `normalizeFileEditorRow` may hand back a caller's own object without passing it through `fileRow` at all. Declaring the member required makes that branch and the published carrier unassignable - two new diagnostics in place of four. The row cannot promise what only one of its two producers guarantees.

#### 0.33.33.43.8 - The file row's actions

**Complete: 37 of a measured 40 closed, and three deferred with their cost stated.** See the archive entry. Fourteen readers covering what a row can do, who may do it, and what happens when they do.

**A published contract was found where I had started to declare one.** `BrowserFileActionRecord` already names the *carrier* a module action is invoked with, so the local shape that nearly collided with it was withdrawn rather than renamed around; TypeScript's weak-type rule is what surfaced the collision, and it was right - the two describe different things.

**Three diagnostics are deferred deliberately, not overlooked.** Typing the row mutations' `fileId` makes `encodeURIComponent(fileId)` a type error, and both ways out are worse than leaving it: writing the conversion out means widening **25 route pins** whose job is to catch route drift, and leaving it `unknown` unconverted moves three eliminations from `params` into `assorted`. The reason is recorded in the source next to the code, and the real fix belongs at `fileRow`, which builds the id without a string fallback.
