...
| --- | --- | --- |
| `0.33.33.43` closeout | Claude | `lists.js` 91 raw (30 owned, 61 `dom`) at its deferral floor; `clients-projects.js` 206 raw, continuing by feature. Neither waits on a product decision. Both files' remaining `dom` reads wait on the framework-wide checked-lookup decision. |
| `0.33.33.42` closeout | Codex | **Typing:** 4 Workbench-owned remain, **every one now ruled (2026-09-24)** - the focus-mode handoff (`.42.41`, in flight), the related-context envelope, Inspector candidates and elapsed time; the rulings are recorded below. **Workbench-owned zero is not `.42` complete**: the two `dataset` reads stay with the framework-wide `dom` boundary, and Task Focus extraction is an open acceptance criterion. `0.33.33.42.22`, called reserved "pending the integrator's scope decision" in six archive entries, **never had a recorded scope** in this file, the archive or any commit, and every Workbench boundary it could have named is now assigned or ruled - **retired unused** by the integrator. |
