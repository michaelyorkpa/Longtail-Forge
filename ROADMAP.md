...
The reslice follows the seams the estate already has — subsystem ownership and shared fixtures — not counts. Every one of the 202 files was assigned to exactly one child, and the children summed to the measured 3,150. `0.33.33.32.1` through `0.33.33.32.27`, plus the corrective child `0.33.33.32.22.1`, have since archived, closing **all 3,143 diagnostics across all 202 files** — the measured cohort exactly — and leaving 0 across 0. The remaining two children own the audits and the permanent proof, not more typing: `0.33.33.32.28.1` is still the only child that may close the scripts program's governance state, and reaching zero once is not the same as proving it stays there; `0.33.33.32.7` also removed four further diagnostics by correcting the TimeEntry write contract. Every remaining child below was remeasured against the live ledger before `0.33.33.32.11` began; all seventeen reconcile exactly to 1,753 across 134, and only `.32.25` moved, because `0.33.33.32.10` closed one module that belongs to it. `0.33.33.32.10.1` has since archived, correcting the resume-state resolver scope seam without moving either program's totals:

| Child | Subject | Diagnostics | Files | Lines |
| --- | --- | --- | --- | --- |
| `.32.28` | Rollup-wide audits and the corrections they find | see below | 78 candidates | — |
...
#### 0.33.33.32.28 - Audit the rollup and correct what the audits find

**Model: High Effort - The audit that catches inherited zeros, plus every correction it produces.**

Runs after `0.33.33.32.27`. This child has no diagnostics of its own, which is not the same as having no work: a measured probe puts **78 of the 202 owners** in scope for the boundary audit alone — 39 `JSON.parse` sites across 28 owners, 61 owners reading `*_json` columns, and 5 spawning child processes. It owns the audits and the concrete fixes they produce; `0.33.33.32.28.1` owns the final proof.

- [ ] **Perform the rollup-wide dynamic-boundary audit.** Search every owner closed anywhere in `0.33.33.32` for `JSON.parse` and equivalent inferred-`any` boundaries feeding an assertion, including database `*_json` columns, child-process output, filesystem reads, environment records, and event payloads. The equivalent audit at `0.33.33.31.11` found 21 unchecked boundaries across 10 files in a rollup less than half this size, so treat a finding of zero as a result that needs evidence, not as the expected outcome. Correct only concrete unchecked boundaries and type erasure; do not reopen closed owners for stylistic normalization.
...
- [ ] **Close the historical-pin acceptance across the whole estate, not just this rollup's owners.** Remeasured at `0.33.33.32.27`, with every typing child's strips landed: **33 files under `scripts/` match the historical pattern and 58 match the planning pattern**, and the pin baseline stands at **19 historical pinners and 37 planning-document readers** by regression id — already inside the acceptance's under-40 ceiling on the count alone, which is exactly why the criterion's second half is what remains. The prediction of 34 survivors held. Disposition each survivor as either a live-contract read that is allowed to remain or a historical pin to strip, and land the result under 40 surviving programs with the criterion's second half — that each survivor owns a current live contract — actually satisfied rather than merely counted. `scripts/lib/roadmap-cursor.mjs`, `scripts/bump-version.mjs`, `scripts/release/checkpoint-commits.mjs`, and `scripts/typecheck-governance.mjs` read the live roadmap by design and are expected survivors; the closeout owners under `scripts/regressions/release/` and the database, Files, and view-builder owners are not automatically.
  - **The scan's coverage is not the acceptance, and `0.33.33.32.27` found two more reasons why.** First, `legacy-cleanup` names all four planning documents in an *escaped* regular expression (`ROADMAP-ARCHIVE\.md`), so the baseline's unescaped pattern never matches it and the owner appears in neither list — even though a reader looking for planning-document dependencies would count it. Second, `version-literal-guardrail-regression.mjs` reads all three documents and is not a discovered regression at all: it runs as the `version:guard` closeout gate, so the manifest-walking scan cannot see it. Both owners' reads are entirely legitimate — the cleanup audit is naming allowed paths, and the guardrail owns the rule that historical version literals are permitted in exactly those documents — which is the point: these are the fourth and fifth blind-spot classes found in this scan, and none of the five was a real violation. Decide whether the scan should be widened or whether the acceptance should be restated against something other than a filename grep.
- [ ] **Keep the collapsed-assertion class closed.** `assert.equal(actual, expected, message)` silently accepts a value literal in the message slot, so folding a second assertion into an existing call stops it running with no signal. Four instances existed: `notes-integration-closeout` lost a public-API-router count and `notes-foundation` lost `enabledByDefault` (both restored at `0.33.33.32.11`), `notes-collections` lost an entire `note_library_collections` column check (restored at `0.33.33.32.14`), and `lists-foundation` was restored at `0.33.33.32.15`. **All four are closed.** A precise detector — parse each `assert.equal`/`deepEqual`/`strictEqual` argument list at top level and flag a third argument beginning `[`, `{`, `true`, `false`, `null`, or a digit — was run estate-wide at `0.33.33.32.14` and found exactly those four and nothing else. Re-run it at closeout and consider adopting it as a standing guard.
...
- [ ] **Disposition the retired scripts that are still on disk.** `0.33.33.32.21` measured this while inventorying its own pins: the manifest lists **19 retired script entries, 14 of which still exist under `scripts/`**. Twelve of those fourteen still contain `ROADMAP.md` or `CHANGELOG.md` reads, and the pin baseline excludes every one of them by construction, because its scan walks discovered manifest entries and a retired script is not discovered. This is the third blind-spot class in that scan, after the cursor-floor callers `0.33.33.32.14` found. **The cursor-floor count has been remeasured and the recorded eleven is now seven.** `0.33.33.32.25` removed fourteen redundant `assertRoadmapCursorAtLeast("0.33.8")` calls from the Workbench and Time Tracking contract modules — every one a duplicate of the single floor `release.roadmap-cursor-floor` already asserts against the same live roadmap, and one of them, `workbench-guided-ui`, was in the invisible class. Estate-wide callers fell from 29 to 15, of which 7 name no planning document and are therefore invisible to the baseline; one of those 7 is `release.roadmap-cursor-floor` itself, which legitimately owns the contract, leaving **6 genuine invisible owners for this child to disposition**.
...
- [ ] **Review the permanent form of the estate-wide bare-`ip` governance guard `0.33.33.32.22.1` added.** The guard is correct for the estate as it stands and it caught a real defect class: seven fixtures across two module families set `ip` on a session record while omitting the `ip_address` every published session contract declares, and six of the seven hid that behind a double cast through `unknown`, so no diagnostic count could ever have surfaced it. **Do not weaken the protection against those six unknown-laundered session fixtures.** The question is only whether a blanket prohibition is the right permanent shape.
  - As written, the guard fails any `.mjs` in the scripts program whose source matches `/^\s*ip: /m`. That is a source-text rule about a property name, not about a session, so a future legitimate `ip` property on an unrelated shape — a request-context record, an HTTP client option, a rate-limit or audit fixture describing a remote address — would produce a false governance failure in a file that has nothing to do with sessions.
...
- [ ] **Decide the disposition of the seventy-five divergent source-slicing helpers this rollup measured, and do not consolidate them blindly.** `0.33.33.32.25` first recorded this as fifteen copies under two helper names; `0.33.33.32.26` widened the name set to five — `functionBody`, `functionBlock`, `extractFunctionBody`, `readFunctionBody`, and `constBlock` — and walked the whole scripts estate. The true figure is **75 local definitions across 73 owners, with 12 distinct return expressions**, and `scripts/test-support/source-scan.mjs` already publishes `extractFunctionBlock(source, functionName)` for the same job. **No local copy matches the published one.** The two largest families are 32 that slice from the declaration to the next top-level declaration and 10 that brace-balance and include the braces; 5 brace-balance and exclude them; the remaining 28 are spread across nine further shapes, three of which pass `undefined` rather than a length as the slice end.
  - The hazard is specific and not stylistic. Every one of these helpers feeds `assert.doesNotMatch` as well as `assert.match`, so broadening the returned text broadens what a negative assertion searches and can turn a real proof vacuous, while narrowing it can drop the region a positive assertion depends on. A single shared helper is the right end state, but it must either publish each contract under a distinct name or migrate each caller with its assertions re-verified — not swap one name for another.
  - Both children typed their copies in place rather than consolidating, because most of the 73 owners sit in owners earlier children already closed and pinned strict-clean, and a silent change to what their assertions search is exactly the class this rollup keeps having to correct. Consolidation across 73 owners is its own slice, not a line item; if this child cannot do it honestly within its scope, record that and size it rather than doing it partially.
  - The hazard is specific and not stylistic. Every one of these helpers feeds `assert.doesNotMatch` as well as `assert.match`, so broadening the returned text broadens what a negative assertion searches and can turn a real proof vacuous, while narrowing it can drop the region a positive assertion depends on. A single shared helper is the right end state, but it must either publish both contracts under distinct names or migrate each caller with its assertions re-verified — not swap one name for another.
  - `0.33.33.32.25` deliberately typed all fifteen in place rather than consolidating, because nine of the fifteen sit in owners earlier children already closed and pinned strict-clean, and a silent change to what their assertions search is exactly the class this rollup keeps having to correct.
- [ ] **Review the express `Application` call signature `0.33.33.32.27` added to `src/types/server-runtime-modules.d.ts`.** The first-party declaration described `Application` as a router with `listen`, `all`, `disable`, and `set`, but omitted that an express application *is* a Node request listener — which is how `http.createServer(app)` works and how `runtime-diagnostics-route` mounts it. The choice was to declare the truth or to cast at the call site; the declaration was corrected, because a cast there would have been an unchecked claim about a first-party contract this repository owns. Confirm the signature is right (it is `(request: IncomingMessage, response: ServerResponse): void`), that no consumer regressed, and that this is the only omission of its kind in that file — `compression`, `cookie-parser`, and `busboy` are declared there too and were not audited by that child. `framework.full-strict-governance` pins the signature so dropping it fails rather than silently pushing callers back onto casts.
