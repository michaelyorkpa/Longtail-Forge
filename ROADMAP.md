...
- [ ] **The rollup closes against a direct post-wrap shared-global inventory, not against a diagnostic count.** The inventory lives in `scripts/regressions/framework/full-strict-governance.regression.mjs`, measures top-level-ness by brace depth through the published masker, and is enforced on every run as a shrink-only backlog so no scoped script can silently regress. `0.33.33.33.4.1` corrected the invariant it enforces, which had been stated too broadly:
  ...
  6. **`dashboard.entry.js` remains a native ES module** and is not part of the classic global-scope estate.

- [ ] **The estate is 29 classic-script owners, and `dashboard.entry.js` is not one of them.** `0.33.33.33.4` counted it as a 30th because it declares names at depth 0. It is the repo's single native ES-module entry: its own header says so, `views/protected/dashboard.html` loads it with `type="module"`, it is the only `type="module"` tag in any view, `scripts/regression-contracts/views/dashboard-es-module-entry.contract.mjs` covers it, and it uses top-level `await`, which is a syntax error in a classic script. Its declarations are module scoped and never enter the classic shared lexical environment, so it must not be wrapped or converted. The inventory now records it as a proved exemption rather than a backlog item, and re-proves that exemption on every run.
...
**Reconciliation: the estate is 29 classic-script owners, each in exactly one child, no duplicate and no orphan. `0.33.33.33.1` through `0.33.33.33.4` have taken 16; the table below is the remaining 13. `dashboard.entry.js` is a native ES module and is not in the estate.**

| Child | Load surface | Owners | Lines | Diagnostics |
| --- | --- | --- | --- | --- |
| `.33.5` | Reporting, calendar, and dynamically loaded surfaces | 4 | 2,089 | 385 |
| `.33.6` | Record module page controllers | 6 | 16,478 | 3,626 |
| `.33.7` | Time tracking and orchestration controllers | 3 | 6,404 | 1,309 |
| **Remaining** | | **13** | **24,971** | **5,320** |
