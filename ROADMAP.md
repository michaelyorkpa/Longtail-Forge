...
- [ ] **The rollup closes against a direct post-wrap shared-global inventory, not against a diagnostic count.** The inventory lives in `scripts/regressions/framework/full-strict-governance.regression.mjs`, decides top-level-ness by brace depth through the published masker, and closes only when **(a)** every browser script declares zero names at depth 0, and **(b)** every deliberate `window.*` publication has a single, explicitly named owner. It is enforced on every run today as a shrink-only backlog, so no scoped script can silently regress.
- [ ] **The estate is 30 owners, not 29.** The original count came from a first-code-line classifier. The depth-based inventory found `dashboard.entry.js` leaking 18 names while belonging to no child's owner list; it is assigned to `0.33.33.33.5` below. The same measurement cleared six scripts the column-based first version wrongly accused, so the correction is +1, not +6.
- [ ] **Three `window.*` surfaces have more than one publisher and must be resolved before close.** `window.LongtailForge` is the namespace bootstrap idiom and is fine. `window.LongtailForge.filesDialog` is genuinely co-owned by `files.js` and `workbench.js`, which state no owner between them. `window.fetch` is patched independently by `navigation.js` and `theme-init.js`. The closeout must give the last two a single named owner each.

**`0.33.33.33.4` has also archived**, scoping the six administration and support controllers and building the inventory the rollup now closes against. It confirmed `0.33.33.33.3`'s finding on a second name: `loadUsers` was declared by both `lists.js` and `user-admin.js`, and `lists.js` had been calling the `user-admin.js` copy, which returns nothing - so its `users` value was typed `void`. Scoping `user-admin.js` made `lists.js` resolve its own loader and the value became what the API boundary really yields. **Two silent function collisions in two consecutive children is a pattern, not a coincidence.** The child also found that scoping these owners moved `workbench.js`, which had been assigning to what the compiler thought was `support-view.js`'s `const statusText` and therefore carried a bogus "cannot assign to a constant" diagnostic. Four of the six pages had **no rendered coverage at all**, so a console-clean spec now loads each of them and asserts at runtime that the controller declares nothing in the page scope. `TS2451` is now 66 and the browser program is 11,049.
...
**Reconciliation: the estate is 30 owners - the original 29 plus `dashboard.entry.js`, which the depth-based inventory found leaking while listed under no child. `0.33.33.33.1` through `0.33.33.33.4` have taken 16; the table below is the remaining 14.**

| Child | Load surface | Owners | Lines | Diagnostics |
| --- | --- | --- | --- | --- |
| `.33.5` | Reporting, calendar, and dynamically loaded surfaces | 5 | 2,320 | 385 |
| `.33.6` | Record module page controllers | 6 | 16,483 | 3,629 |
| `.33.7` | Time tracking and orchestration controllers | 3 | 6,406 | 1,315 |
| **Remaining** | | **14** | **25,209** | **5,329** |
...
`reporting.js`, `calendar.js`, `dashboard.js`, `tasks-dashboard.js`, and `dashboard.entry.js` (18 names, added by the `0.33.33.33.4` inventory correction). The last two have **no `<script src>` anywhere**: `dashboard.js` is loaded by `dashboard.entry.js` and `tasks-dashboard.js` by a module's dynamic loader, so their globals are consumed through a different delivery path and must be proved through that path rather than through a page load.
