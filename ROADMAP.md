...
- [ ] Close the 55 diagnostics across the fifteen diagnostic-bearing modules under `scripts/regression-contracts/tasks/`, three under `notes/`, two under `lists/`, and one under `tags/`. The aggregators own 25, 8, 4, and 4 modules respectively; the remainder are already strict-clean and out of scope.
- [ ] Apply the same rule as `0.33.33.32.25`: type the local helper parameters truthfully, reuse the shared source-reader type where the repeated helper contract is real, and add no execution abstraction these side-effect modules do not have.
- [ ] Preserve each module's aggregator contribution, `contractCount`/`assertionCount` results, and coverage floor exactly.
...
- [ ] **Decide the disposition of the fifteen divergent source-slicing helpers `0.33.33.32.25` measured, and do not consolidate them blindly.** Fifteen owners define a local `functionBody(source, name)` or `extractFunctionBody(source, name)` that slices one named function out of a source file, and `scripts/test-support/source-scan.mjs` already publishes `extractFunctionBlock(source, functionName)` for the same job. None of the fifteen uses it, and they are not interchangeable: **nine return the block including its braces, five return the bare body excluding them**, and the published helper returns a third thing — the text from the `function` keyword through the closing brace. Six further variants differ in whether they find `async function` or a `name: () => (` property form, and in whether a miss fails through `assert.fail` or a bare `throw`. Three `actionDefinitionBlock` copies and one `readArrayLiteral` bring the estate total to nineteen local source-slicing helpers.
