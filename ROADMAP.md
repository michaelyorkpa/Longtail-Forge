...
- [ ] **Preserve the measured `project_id` persistence asymmetry exactly.** `client_id` and `task_id` pass through `nullableTextParam` and can persist SQL `NULL`; `project_id` passes through `textParam`, so a null input persists as the empty string. `TimeEntryWriteInput` may admit `project_id: string | null` where null is a truthful accepted caller input, but this checkpoint must not move the write path from empty string to SQL `NULL` merely to make the three scope identifiers symmetrical. Record the asymmetry as existing runtime behaviour and leave it unchanged unless separate evidence shows it is itself a defect warranting a dedicated behaviour change. This checkpoint owns contract truthfulness, not storage normalization.
...
- [ ] Close the 34 diagnostics across the thirteen diagnostic-bearing modules under `scripts/regression-contracts/workbench/` and the two under `scripts/regression-contracts/time-tracking/`. The Workbench aggregator owns seventeen contract modules in total; the other four are already strict-clean and are not this child's scope.
