...
- [ ] **Remeasured with both corrected instruments - the logical-assignment inventory fix of `0.33.33.38.2.7` and lexically scoped alias attribution.** 50 undeclared members: **32 multi-consumer carrying 402 attributed diagnostics**, 13 single-consumer carrying 14, and 5 zero-consumer carrying 2. `modal` 64 across 14 files, `settingsRenderer` 44/5, `icons` 36/10, `timezones` 27/6, `moduleActions` 23/6, `tags` 21/4, `status` 20/7, `clientProjectOptions` 20/5, `tasksDialog` 15/2, `workspaceContext` 15/6, `taskCalendar` 14/2, `settingsHost` 12/5, `settingsPageController` 10/5, and the rest. **Every one has exactly one writer**, so none introduces a multi-writer question.
- [ ] **Cluster by adoption semantics, and note that the attribution view is not additive.** Roughly half of several members' attributed diagnostics are canonically `unknown` rather than namespace - `modal` is 33 namespace and 31 `unknown`, `settingsRenderer` 22 and 22 - because declaring the member does not narrow the value its methods return. **Declaring a surface closes its namespace half; the `unknown` half is `0.33.33.38.4`.** A child sized on the attribution total will be sized wrong.
