...
**Re-verified against the post-`0.33.33.36` tree; the boundary holds and two facts are added.** All four named symbols are where the checkpoint says, with the three responsibilities it describes: `taskWorkflowActionVisible` at `tasks.js:1402` returns a boolean, `taskFocusLifecycleDisabledReason` at `workbench.js:1743` returns a message string, and `updateCompleteTaskActionState` / `updateBlockTaskActionState` at `task-dialog.js:1418` and `:1428` update DOM state. `0.33.33.35` reordered `tasks.js`'s bootstrap and `0.33.33.36` typed the Notes and Lists producers; neither touched these functions, and `task-dialog.js` has not been modified since before the rollup. Current state: **tasks.js 3,047 / 557, workbench.js 4,134 / 785, task-dialog.js 3,410 / 984**, browser total 10,377.

**The shared kernel is measurable, and it is a duplicated vocabulary rather than duplicated logic.** The active-status set `["open", "in_progress", "blocked"]` is written out **six times** - `tasks.js:1374`, `:1385`, `:1396`, `:1545`, and `task-dialog.js:27` as a `Set` - and its complement `["complete", "archived"]` **five more**, at `tasks.js:2830`, `task-dialog.js:871`, `:1437`, and `workbench.js:1756`, `:1762`. The same rule is spelled both ways across three files. All three also read `timer_status` - eight, thirteen, and eight references - so the timer half is shared too, not a Tasks-only concern.

**`visibleStatuses` is client-owned, so this does not collide with `0.33.33.35.1.2`.** It is a literal in the `tasks.js` action array, not a field the server descriptor delivers; `src/modules/tasks/module.js` declares no `visibleStatuses` or `timerVisibility`. Extracting a client-side legality core therefore recreates no descriptor authority.

**The published status type cannot serve as the contract in its current form.** `TaskLifecycleStatus` in `src/types/task-block-recovery-contracts.d.ts` reads `"open" | "in_progress" | "blocked" | "complete" | "archived" | string` - **the trailing `| string` collapses the union to `string`**, so it discriminates nothing. The authoritative runtime vocabulary is `src/modules/tasks/tasks.service.js:85`. This is the `0.33.33.36` lesson again: do not adopt a published type because its name fits. Either narrow it - which reaches `task-block-recovery-engine.js`, its only consumer - or publish the browser-facing contract separately and leave the server type alone.

- [ ] Extract only the **status-and-timer legality core** into one typed shared contract, and have all three surfaces consume it. Classic-script delivery means one narrow, single-writer sibling surface injected at `<head>`, on the mechanism `0.33.33.35.2` settled; three consumers satisfy the two-surface rule comfortably.
