...
| Claude | `0.33.33.38.3.10` | `lists.js` — checked-DOM adoption complete, 31 raw with 1 `dom` (not a lookup); `clients-projects.js` at 143, whose 10 remaining `dom` are not lookups | 31 |
...
**Complete: 60 `dom` in one page with two initialization paths.** See the archive entry. `lists.js` fell **91 to 31** and its `dom` **61 to 1**; params, state and assorted did not move, and no message rose. Every required handle is checked where the page already read it unguarded, never at capture, so the dialog-only path still caches its absent workspace handles without failing.

**It found two pre-existing Lists defects on nightly**, each fixed first as its own correction: `0.33.33.43.44` (every real list refused since `9427cf2d`) and `0.33.33.43.45` (no other page could open the Lists dialog since `1da0fdd0`).
