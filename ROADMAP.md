...
- [ ] **An extraction reference audit searches for the symbol, not for call syntax.** `0.33.33.35.2` searched `symbolName(` and missed ten `.filter(actionPermissionsAllowed)` sites that only the linter caught, because a bare reference has no parenthesis after it. Before moving any symbol, check every form that can carry it: direct invocation, bare reference or callback, property assignment, destructuring, alias, membership in an array, map, or registry, publication on a namespace, and any test or source contract naming it. A symbol that survives the move under one form and not another fails at runtime rather than at the boundary.
- [ ] **The frozen rule is about ownership of `window.LongtailForge.view`, not a ban on sibling surfaces.** `0.33.33.35.2` settled this: a new narrow, typed, frozen, single-writer `window.LongtailForge.*` sibling is acceptable where classic-script delivery genuinely requires one and the responsibility behind it is cohesive - which is the shape `view-surface-descriptor.js` and `view-response-records.js` already have. What stays forbidden is moving, republishing, reordering, expanding, or adding a second writer to `view` itself. **Do not merge unrelated responsibilities into one surface merely to keep the published count down**; a cohesive seam is worth a surface.
