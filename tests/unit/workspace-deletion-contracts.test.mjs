...
describe("the consumers", () => {
  ...
  it("leaves the page's other producers to their own children", () => {
    ...
    // Anchored on the call site, not the spelling: the reader's own definition also contains
    // its name, so matching the bare name would have passed even with the read left raw.
    assert.match(page, /const jobs = readJobStatusResponse\(/,
      "the jobs readout now belongs to the Jobs Status producer, not to this child");
    ...
    // The settings save read was on this list until `0.33.33.38.4.5.5` claimed it for the
    // shared `/api/settings` producer. That is another child doing its job, not this one
