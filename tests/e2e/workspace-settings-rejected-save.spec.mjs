...
test("a rejected workspace name leaves the form dirty and keeps the saved baseline", async ({ page }) => {
  ...
  try {
    ...
    if (await nameInput.inputValue() !== savedName) {
      ...
      if (await save.isEnabled()) {
        await save.click();
        await expect(page.getByRole("combobox", { name: "Active workspace" })).toHaveText(savedName);
      }
    }
  }
});
