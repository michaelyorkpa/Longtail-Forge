...
assert.match(auditDocs, /Current totals as of 0\.33\.5\.27\.33:[\s\S]*Remaining runtime literal-helper invocations: 0[\s\S]*Remaining direct interpolated SQL operation sites: 0[\s\S]*Existing direct bound-params operation sites: 385[\s\S]*Total runtime database operation calls seen by the audit scanner: 429/, "audit docs should publish the final parameter-binding ratchet");
assert.match(auditDocs, /## Dialect Adoption Guardrail[\s\S]*Current totals as of 0\.33\.5\.27\.33:[\s\S]*Remaining raw seam-backed dialect sites at application call sites: 0/, "audit docs should publish the final dialect ratchet");
...
assert.match(moduleContractDocs, /As of 0\.33\.5\.27\.33[\s\S]*starting point for every new first-party or future third-party module[\s\S]*module-local SQL literal escaping[\s\S]*Browser descriptors and view adapters remain data consumers only/, "module contract should publish the new-module database boundary");
assert.match(moduleDevelopmentDocs, /## Database Access[\s\S]*As of 0\.33\.5\.27\.33[\s\S]*Import database access from `src\/core\/database\.js`[\s\S]*Do not call `sqlText\(\)`[\s\S]*Browser scripts, view descriptors, and module adapters must not become database access layers/, "module development docs should give authors the enforcement rules");
assert.match(viewContractDocs, /As of 0\.33\.5\.27\.33[\s\S]*view helpers and descriptors also must not learn database provider behavior[\s\S]*canonical filtering, sorting, paging, permission pruning, database reads\/writes/, "view-building contract should keep provider behavior out of helpers");
assert.match(declarativeViewDocs, /As of 0\.33\.5\.27\.33[\s\S]*completed agnostic database contract[\s\S]*must not express SQL, raw provider dialect[\s\S]*server-side route\/service\/repository concerns/, "declarative view docs should keep database ownership server-side");
