...
assert.match(declarativeGuide, /list shell and bulk toolbar use shared framework helpers[\s\S]*reported-only until later Tasks slices/, "Declarative guide should document the 8.4 Tasks boundary");
...
assert.match(tasksView, /css\/longtail-forge\.css\?v=60[\s\S]*js\/shared\/view-builder\.js\?v=13[\s\S]*js\/shared\/view-renderer\.js\?v=12[\s\S]*js\/tasks\.js\?v=16/, "Tasks host should load the list-shell cache keys");
