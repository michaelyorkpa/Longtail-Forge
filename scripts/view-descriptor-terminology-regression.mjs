...
assert.match(moduleContract, /Lists now uses a descriptor for its protected workspace read shell, item form fields, item table\/action placement, and create\/edit modal shell while later slices still own linked-record placement and workflow-action migration/, "Module contract should document the current Lists descriptor boundary");
