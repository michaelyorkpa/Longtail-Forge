...
assert.match(tasksView, /js\/shared\/overlay-host\.js/, "Tasks view should load the shared overlay host before task-dialog");
assert.match(tasksView, /js\/shared\/overlay-host\.js[\s\S]*js\/task-dialog\.js/, "Tasks view should keep loading the shared overlay host before task dialog code for any remaining non-modal overlay users");
