...
assert.match(inventoryDoc, /Current as of 0\.33\.5\.18\.14\.1/, "Clients/Projects inventory should report this slice");
...
assert.match(declarativeGuide, /0\.33\.5\.18\.14\.1[\s\S]*client-projects\.clients[\s\S]*client-projects\.projects[\s\S]*framework-rendered read anatomy[\s\S]*reporting-only[\s\S]*shared module-action registry/, "Declarative guide should document the Clients/Projects framework-rendered read and action boundary");
...
assert.match(changelog, /Version 0\.33\.5\.18\.14\.1[\s\S]*action registration[\s\S]*clients-projects-action-registration-regression\.mjs/, "Changelog should record the completed 0.33.5.18.14.1 slice");
