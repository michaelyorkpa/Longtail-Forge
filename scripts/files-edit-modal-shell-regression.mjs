...
assert.match(filesPage, /css\/longtail-forge\.css\?v=8/, "Files page should cache-bust modal styling");
assert.match(filesPage, /js\/files\.js\?v=7/, "Files page should cache-bust the Files adapter");
...
assert.match(buildBlock, /view\.renderDescriptorModalForm\(fileEditorModalDescriptor\(\),[\s\S]*actions:\s*\[closeButton\]/, "File editor should use the shared descriptor modal form shell and footer actions");
assert.match(buildBlock, /viewParts\.form\.addEventListener\("submit"[\s\S]*event\.preventDefault\(\)/, "File editor form should be non-submitting in this slice");
...
assert.doesNotMatch(editorSource, /postJson|putJson|patchJson|\/api\/files\/attachments\/[^"`']+\/context/, "File editor shell should not save context changes yet");
assert.doesNotMatch(editorSource, /rename|replacement|storageProvider|storageKey|quarantine|hardDelete|permanent|purge|preview|fileContextSave/i, "File editor shell should not add forbidden controls");
assert.doesNotMatch(functionBlock(filesScript, "createFilesTable"), /openFileEditor/, "Files table should not wire row-open behavior in this slice");
