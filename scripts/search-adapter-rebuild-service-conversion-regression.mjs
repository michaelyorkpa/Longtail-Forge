...
function assertStaticContract() {
  ...
  assert.match(auditDocs, /Current totals as of 0\.33\.5\.27\.26:[\s\S]*Remaining runtime literal-helper invocations: 304[\s\S]*Remaining direct interpolated SQL operation sites: 57[\s\S]*Existing direct bound-params operation sites: 302[\s\S]*Total runtime database operation calls seen by the audit scanner: 419/, "audit docs should record the Search adapter/rebuild service conversion ratchet");
  ...
  assert.match(roadmap, /### Version 0\.33\.5\.27\.25 - Conversion wave: Search adapter and rebuild service[\s\S]*- \[x\] Convert `core\/search\/adapters\/sqlite-search-adapter`[\s\S]*- \[x\] Keep FTS5 maintenance[\s\S]*- \[x\] Update the burndown ratchet/, "roadmap should mark the Search adapter/rebuild service conversion slice complete");
