...
assert.match(
  roadmap,
  /## Version 0\.33\.5\.25 - Storage branch cleanup/,
  "roadmap should hand off to the storage cleanup branch after Node 24 closeout",
);
...
assert.match(runtimeDocs, /live storage\/scanner keys/i, "runtime docs should explicitly collect live storage/scanner keys");
...
assert.match(runtimeDocs, /No direct\/presigned S3 upload or download route is implemented in 0\.33\.5\.22\.15/, "runtime docs should keep signed URL routes out of this branch");
...
assert.match(sqliteDocs, /As of 0\.33\.5\.22\.15[\s\S]*Local-vs-S3 deployment guidance/i, "SQLite docs should keep local-vs-S3 deployment guidance current");
