...
const footerToggle = functionBlock(taskDialogScript, "toggleTaskFooterPanel");
const footerFallback = functionBlock(taskDialogScript, "toggleTaskFooterPanelFallback");
...
assert.match(ensureDialog, /fields\.tagToggle\?\.addEventListener\("click", \(\) => toggleTaskFooterPanel\("tags"\)\)[\s\S]*fields\.fileToggle\?\.addEventListener\("click", \(\) => toggleTaskFooterPanel\("files"\)\)/, "Tags and Files footer buttons should still dispatch to Tasks-owned handlers");
...
assert.match(saveTimer, /currentTaskTimer\(task\.task_id\)[\s\S]*readTaskTimerElapsedSeconds\(timer\)[\s\S]*api\.putJson\(`\/api\/tasks\/\$\{encodeURIComponent\(task\.task_id\)\}\/timer`[\s\S]*timer_status: timerStatus[\s\S]*upsertTaskTimer\(result\.timer\)[\s\S]*writeTaskTimerFields\(task\)/, "Start/pause should preserve Tasks-owned timer save behavior");
assert.match(finalizeTimer, /api\.postJson\(`\/api\/tasks\/\$\{encodeURIComponent\(task\.task_id\)\}\/timer\/finalize`[\s\S]*duration_seconds: durationSeconds[\s\S]*removeTaskTimer\(task\.task_id\)[\s\S]*setStatus\("Task time saved\."\)/, "Finalize should preserve Tasks-owned time-save behavior");
assert.match(resetTimer, /modal\.confirm\(\{[\s\S]*title: "Reset task timer"[\s\S]*danger: true[\s\S]*api\.deleteJson\(`\/api\/tasks\/\$\{encodeURIComponent\(task\.task_id\)\}\/timer`\)[\s\S]*removeTaskTimer\(task\.task_id\)/, "Reset should preserve confirmation and Tasks-owned delete behavior");
...
assert.match(footerToggle, /taskOverlayHost[\s\S]*taskOverlayHost\.toggle\(panelName\)[\s\S]*toggleTaskFooterPanelFallback\(panelName\)/, "Task utilities should prefer the shared overlay host and keep a fallback");
assert.match(footerFallback, /const nextPanel = panelName === "files" \? fields\.filePanel : fields\.tagPanel[\s\S]*otherPanel\.hidden = true[\s\S]*nextPanel\.hidden = !shouldOpen[\s\S]*nextToggle\?\.setAttribute\("aria-expanded", String\(shouldOpen\)\)/, "Tags and Files fallback panels should remain mutually exclusive");
