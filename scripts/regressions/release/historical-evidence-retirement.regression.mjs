...
const activeById = new Map(manifest.regressions.map((entry) => [entry.id, entry]));
const creditedById = new Map(policy.retiredScripts.filter((entry) => entry.floorCredit === true).map((entry) => [entry.id, entry]));

assert.equal(evidence.version, "0.33.33.2");
assert.deepEqual(
  evidence.retirements.map(({ id }) => id).sort(),
...
const activeCloseoutOwners = manifest.regressions
  .filter((entry) => entry.tags.includes("closeout") || /closeout/i.test(entry.id) || /closeout/i.test(entry.path))
  .map((entry) => entry.id)
  .sort();
...
for (const filePath of archivedModules) {
  assert.ok(policy.retiredScripts.some((entry) => entry.script === filePath), `${filePath} must remain registered as retired evidence`);
}
