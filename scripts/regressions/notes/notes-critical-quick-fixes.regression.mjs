...
assert.match(notesModule, /id: "note-bulk-editor"[\s\S]*field: "library"[\s\S]*field: "collection"[\s\S]*field: "noteType"[\s\S]*field: "visibility"/, "the Notes descriptor should declare the four-field bulk editor");
...
assert.match(functionBlock(notes, "applyBulkEdit"), /postJson\("\/api\/notes\/bulk"[\s\S]*state\.selectedNoteIds = new Set\(errors[\s\S]*Updated \$\{updatedNotes\.length\} notes/, "the browser should retain failed selections and report partial bulk results");
