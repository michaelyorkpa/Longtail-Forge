...
function assertStaticContract() {
  ...
  assert.match(auditDocs, /Current totals as of 0\.33\.5\.27\.18:[\s\S]*Remaining runtime literal-helper invocations: 709[\s\S]*Remaining direct interpolated SQL operation sites: 145[\s\S]*Existing direct bound-params operation sites: 206[\s\S]*Total runtime database operation calls seen by the audit scanner: 417/, "audit docs should record the Files browse/read conversion ratchet");
  assert.match(auditDocs, /\| services\/files\.service \| Remaining \| 123 \| 22 \| 7 \| 31 \|/, "audit inventory should record the remaining Files service partial conversion state");
