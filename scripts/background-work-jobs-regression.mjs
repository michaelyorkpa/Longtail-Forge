...
try {
  ...
  assert.match(fileScannerJobsSource, /FILE_SCAN_JOB_TYPE = "file\.scan"/, "file scanning should have a durable job type");
  assert.match(fileScannerJobsSource, /registerJobHandler\(FILE_SCAN_JOB_TYPE/, "file scanning should register a worker handler");
  assert.match(filesSource, /filesScannerJobService\.registerFileScanJobHandlers/, "Files facade should retain the existing scanner-handler registration entry point");
