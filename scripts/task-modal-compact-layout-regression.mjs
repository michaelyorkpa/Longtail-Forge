...
assert.match(tasksView, /data-task-notification-toggle hidden aria-expanded="false" aria-controls="task-notification-panel"/, "Task notification settings should be behind an accessible bell toggle");
...
assert.match(tasksView, /<script src="js\/task-dialog\.js\?v=4"><\/script>/, "Task dialog cache bust should advance");
assert.match(tasksView, /<link rel="stylesheet" href="css\/longtail-forge\.css\?v=12">/, "Shared stylesheet cache bust should advance");
...
assert.match(taskDialogScript, /toggleTaskNotificationPanel/, "Task dialog should toggle notification settings from the bell");
...
assert.match(taskDialogScript, /task\?\.status === "complete" \|\| task\?\.status === "archived"/, "TTC should be limited to completed or archived tasks");
...
assert.match(tasksModule, /version: "0\.33\.5\.12\.3"/, "Tasks module version should match the compact modal release");
