...
const dialectGuardrailSliceVersion = appVersion;
...
function assertDocumentationContract() {
  assert.match(roadmap, /### Version 0\.33\.5\.27\.32 - Dialect enforcement guardrail[\s\S]*- \[x\] Extend the enforcement guardrail[\s\S]*- \[x\] Critically, run the dialect guardrail[\s\S]*whole-tree sweep[\s\S]*- \[x\] Add a distinct dialect-adoption axis[\s\S]*- \[x\] Reconcile the guardrail[\s\S]*durable-job[\s\S]*- \[x\] Add regressions proving the guardrail rejects raw dialect use[\s\S]*- \[x\] Document the dialect guardrail/, "roadmap should mark the dialect enforcement guardrail slice complete");
