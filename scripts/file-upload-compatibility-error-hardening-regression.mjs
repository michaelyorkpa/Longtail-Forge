...
function assertStaticContracts() {
  ...
  assert.match(moduleContract, /As of 0\.33\.5\.22\.6[\s\S]*POST \/api\/files[\s\S]*POST \/api\/files\/batch/, "module contract should record legacy JSON compatibility");
  assert.match(moduleDevelopment, /As of 0\.33\.5\.22\.6[\s\S]*base64 compatibility routes/, "module docs should guide new module uploads to streamed routes");
  assert.match(runtimeDocs, /As of 0\.33\.5\.22\.6[\s\S]*retired no earlier than 0\.33\.5\.23\.0/, "runtime docs should define the base64 retirement floor");
  ...
  assert.match(roadmap, /### Version 0\.33\.5\.22\.6 - Upload compatibility and error hardening[\s\S]*- \[x\] Upload cancellation\/error cleanup/, "roadmap should mark upload hardening coverage complete");
