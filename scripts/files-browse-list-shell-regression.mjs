...
assert.match(fileRow, /fileSizeLabel:\s*formatBytes\(file\.fileSizeBytes \|\| file\.file_size_bytes\)/, "Files row shaping should keep file size display data owned by Files");
...
assert.match(styles, /\.files-table\s*\{[\s\S]*table-layout:\s*fixed[\s\S]*min-width:\s*960px/, "Files table should use a fixed-width scan-friendly row layout");
