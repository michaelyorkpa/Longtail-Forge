...
for (const source of [
  ...
  for (const heading of [
    ...
    assert.match(
      text,
      new RegExp(`taskEditorSectionHeading\\(view, "summary", "${escapeRegExp(heading)}"\\)`),
  ...
  assert.match(
    text,
    /className: "surface-modal-section-help"[\s\S]*"data-task-checklist-status"/,
    `${label} should use shared help styling for Checklist status`,
  );
  assert.match(
    text,
    /className: "surface-modal-section-help"[\s\S]*"data-task-recurrence-summary"/,
...
assert.match(
  taskDialogScript,
  /className: \["task-recurrence-field", "surface-modal-group", "surface-divider-top"\][\s\S]*taskEditorSectionHeading\(view, "summary", "Recurrence"\)/,
  "Recurrence divider should live on the section being toggled, above its heading",
);
assert.match(
  taskDialogScript,
  /className: \["task-reminder-field", "surface-modal-group", "surface-divider-top"\][\s\S]*taskEditorSectionHeading\(view, "summary", "Reminders"\)/,
  "Reminder divider should live on the section being toggled, above its heading",
);
assert.match(
  taskDialogScript,
  /className: \["task-notes-field", "surface-modal-group", "surface-divider-top"\][\s\S]*taskEditorSectionHeading\(view, "summary", "Notes"\)/,
