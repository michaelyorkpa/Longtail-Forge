...
assert.equal(packageJson.version, "0.33.6.12c-2", "package.json should report the current app version");
assert.equal(packageLock.version, "0.33.6.12c-2", "package-lock root should report the current app version");
assert.equal(packageLock.packages[""].version, "0.33.6.12c-2", "package-lock package entry should report the current app version");
...
assert.match(html, /js\/shared\/icons\.js\?v=4[\s\S]*js\/shared\/view-builder\.js\?v=11[\s\S]*js\/shared\/view-renderer\.js\?v=12[\s\S]*js\/notes\.js\?v=71/, "Notes host should load the icon helper, view builder, and renderer before the module adapter");
