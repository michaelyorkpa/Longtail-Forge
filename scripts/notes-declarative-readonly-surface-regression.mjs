...
assert.equal(packageJson.version, "0.33.5.18.6.1", "package.json should report the current app version");
assert.equal(packageLock.version, "0.33.5.18.6.1", "package-lock root should report the current app version");
assert.equal(packageLock.packages[""].version, "0.33.5.18.6.1", "package-lock package entry should report the current app version");
...
assert.match(html, /js\/shared\/icons\.js\?v=2[\s\S]*js\/shared\/view-builder\.js\?v=8[\s\S]*js\/shared\/view-renderer\.js\?v=7[\s\S]*js\/notes\.js\?v=31/, "Notes host should load the icon helper, view builder, and renderer before the module adapter");
