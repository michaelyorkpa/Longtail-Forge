...
assert.equal(packageJson.version, "0.33.5.18.5.4", "package.json should report the current app version");
assert.equal(packageLock.version, "0.33.5.18.5.4", "package-lock root should report the current app version");
assert.equal(packageLock.packages[""].version, "0.33.5.18.5.4", "package-lock package entry should report the current app version");
...
assert.match(html, /css\/longtail-forge\.css\?v=30/, "Notes host should load the refreshed stylesheet");
assert.match(html, /js\/shared\/icons\.js\?v=2[\s\S]*js\/shared\/view-builder\.js\?v=6[\s\S]*js\/shared\/view-renderer\.js\?v=5[\s\S]*js\/notes\.js\?v=24/, "Notes host should load the icon helper, view builder, and renderer before the module adapter");
