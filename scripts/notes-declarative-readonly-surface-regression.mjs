...
assert.equal(packageJson.version, "0.33.5.21.9.4", "package.json should report the current app version");
assert.equal(packageLock.version, "0.33.5.21.9.4", "package-lock root should report the current app version");
assert.equal(packageLock.packages[""].version, "0.33.5.21.9.4", "package-lock package entry should report the current app version");
...
assert.match(html, /js\/shared\/icons\.js\?v=4[\s\S]*js\/shared\/view-builder\.js\?v=11[\s\S]*js\/shared\/view-renderer\.js\?v=12[\s\S]*js\/notes\.js\?v=70/, "Notes host should load the icon helper, view builder, and renderer before the module adapter");
