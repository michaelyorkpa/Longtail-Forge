...
function assertStaticContract() {
  ...
  assert.match(listsModuleSource, /version:\s*"0\.33\.5\.27\.17"/, "Lists module should report the Lists catalog/link conversion version");
  ...
  assert.match(listsDocs, /current Lists implementation as of 0\.33\.5\.27\.17/, "Lists docs should report the current implementation version");
...
async function createFixtures(session) {
  ...
  const list = await listsRepository.create(session.workspace_id, {
    ...
    metadata_json: JSON.stringify({ slice: "0.33.5.27.17" }),
