...
assert.match(tasksView, /<button type="button" data-task-notification-toggle hidden aria-pressed="false">Follow task notifications<\/button>/, "Task notification bell should be the direct follow toggle");
...
assert.doesNotMatch(tasksView, /task-notification-popover|data-task-notification-field|data-task-notification-follow|data-task-notification-status|aria-controls="task-notification-panel"/, "Task notification controls should not render a popover or secondary follow button");
...
assert.match(tasksModule, /version: "0\.33\.5\.12\.4\.1"/, "Tasks module version should match the modal follow-up release");
