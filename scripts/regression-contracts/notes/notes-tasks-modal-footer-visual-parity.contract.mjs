...
assert.match(
  [extractFunctionSpan(notesScript, "cacheNotesElements"), extractFunctionSpan(notesScript, "bindNotesEvents")].join("\n"),
  /copyLinkButton = findNotesControl\("\[data-copy-note-link\]", HTMLButtonElement\)[\s\S]*copyLinkButton\?\.addEventListener\("click", copyCurrentNoteLink\)/,
  "Notes Copy Link should bind its typed optional button to the module-owned click handler",
);
...
assert.match(taskDecorateControls, /icons\.decorateButton\(requireTaskIconButton\(fields\.tagToggle\), \{ icon: "tag", label: "Task tags", text: "Tags", title: "Task tags", iconOnly: false \}\)[\s\S]*icons\.decorateButton\(requireTaskIconButton\(fields\.fileToggle\), \{ icon: "file", label: "Task files", text: "Files", title: "Task files", iconOnly: false \}\)[\s\S]*icons\.decorateButton\(requireTaskIconButton\(fields\.copyLink\), \{ icon: "copy", label: "Copy task link", text: "Copy Link", title: "Copy task link", iconOnly: false \}\)/, "Tasks footer utility controls should keep icon plus text decoration");
assert.match(taskDecorateControls, /icons\.decorateButton\(requireTaskIconButton\(fields\.cancel\), \{ icon: "close", label: "Cancel", text: "", title: "Cancel", iconOnly: true \}\)[\s\S]*icons\.decorateButton\(requireTaskIconButton\(fields\.save\), \{ icon: "save", label: "Save task", text: "", title: "Save task", iconOnly: true \}\)/, "Tasks commit controls should remain compact icon buttons");
