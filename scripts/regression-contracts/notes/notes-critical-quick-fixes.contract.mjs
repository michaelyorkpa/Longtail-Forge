...
assert.match(functionBlock(notes, "mountBulkTagPicker"), /tagSurface\.mountPicker\(bulkTagsEditor,[\s\S]*allowCreate: false[\s\S]*placeholder: "Type to search tags"[\s\S]*tags: state\.availableTags/, "Notes bulk tagging should use the native typable shared picker instead of a multi-select scroll box");
...
assert.match(functionBlock(notes, "transitionCreatedNoteToEdit"), /state\.editingNoteId = note\.note_id[\s\S]*requireNotesValue\(dialogTitle\)\.textContent = "Edit Note"[\s\S]*mountTagEditor\(note\)[\s\S]*mountNoteEditorFiles\(note\)/, "the persisted note should enable the canonical saved-note utilities without closing the dialog");
