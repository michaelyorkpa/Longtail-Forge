...
const rowActions = extractFunctionSpan(filesScript, "createFileActions");
...
const rowQuarantineAction = extractFunctionSpan(filesScript, "createQuarantineAction");
...
const rowDeleteAction = extractFunctionSpan(filesScript, "createDeleteAction");
...
assert.match(extractFunctionSpan(filesScript, "createRestoreAction"), /action: "files\.restore"[\s\S]*restoreFile\(row\.fileId\)/, "Restore should keep a distinct Files route handler");
...
assert.match(extractFunctionSpan(filesScript, "canQuarantineFileRow"), /canManageFileReview\(attachment, file, fileId\)[\s\S]*status !== "quarantined"/, "Quarantine row visibility should use the shared explicit review permission evidence");
assert.match(extractFunctionSpan(filesScript, "canManageFileReview"), /workspaceHasPermission\("files\.manage_quarantine"\)/, "Review visibility should use explicit permission evidence");
assert.match(extractFunctionSpan(filesScript, "workspaceHasPermission"), /permissionHints\?\.filesManageQuarantine === true/, "Files page should honor the app-shell quarantine permission hint");

const helperActions = extractFunctionSpan(attachmentHelper, "createAttachmentActions");
...
const helperReport = extractFunctionSpan(attachmentHelper, "reportFile");
...
const helperQuarantine = extractFunctionSpan(attachmentHelper, "quarantineFile");
...
assert.match(extractFunctionSpan(attachmentHelper, "workspaceHasPermission"), /permissionHints\?\.filesManageQuarantine === true/, "Attachment helper should honor the app-shell quarantine permission hint");
...
assert.match(extractFunctionSpan(appShellService, "readPermissionHints"), /files\.manage_quarantine[\s\S]*filesManageQuarantine: canManageFileQuarantine/, "App shell should expose a narrow quarantine permission hint");
...
assert.match(extractFunctionSpan(filesService, "reportFile"), /canReadAnyAttachment[\s\S]*normalizeReportReason[\s\S]*db\.transaction[\s\S]*filesRepo\.createFileReport[\s\S]*filesRepo\.markFileReported/, "Report service should keep read checks, allowed reasons, and explicit transaction orchestration");
assert.match(extractFunctionSpan(filesRepository, "markFileReported"), /SET status = 'quarantined'[\s\S]*quarantine_reason = :quarantineReason/, "Files repository should own the bound report quarantine write");
assert.match(extractFunctionSpan(filesService, "quarantineFile"), /assertCan\(session, "files\.manage_quarantine"[\s\S]*filesRepo\.quarantineFile/, "Quarantine service should keep server-side permission authority");
assert.match(extractFunctionSpan(filesRepository, "quarantineFile"), /SET status = 'quarantined'[\s\S]*quarantine_reason = :quarantineReason/, "Files repository should own the bound quarantine write");
