...
const statusSelect = extractFunctionSpan(filesScript, "createStatusSelect");
...
assert.match(extractFunctionSpan(filesScript, "createFileStatusChip"), /surface-chip files-status-chip[\s\S]*File status:[\s\S]*dataset:\s*\{\s*fileStatusChip/,
  "Files availability chip should use shared chip styling and accessible status labels");
assert.match(extractFunctionSpan(filesScript, "createFileScanStatusChip"), /text === statusLabelText[\s\S]*surface-chip files-status-chip[\s\S]*Review state:[\s\S]*dataset:\s*\{\s*fileScanStatusChip/,
  "Files review chip should use shared chip styling, accessible labels, and suppress duplicate review text");

const filesStatusLabel = extractFunctionSpan(filesScript, "statusLabel");
...
assert.match(extractFunctionSpan(filesScript, "scanStatusLabel"), /not_required[\s\S]*No review needed[\s\S]*passed[\s\S]*Reviewed[\s\S]*pending[\s\S]*Review pending[\s\S]*error[\s\S]*Review needed/,
  "Files scan-status chips should be normalized as review-state labels");
assert.match(extractFunctionSpan(filesScript, "reviewStateLabel"), /status === "quarantined"[\s\S]*"In review"[\s\S]*scanStatusLabel\(scanStatus\)/,
  "In-review files should not show contradictory reviewed scan text in review-state display");
assert.match(extractFunctionSpan(filesScript, "createFileEditorMetadataList"), /\["Review state", reviewStateLabel\(row\.status, row\.scanStatus \|\| ""\), "review-state"\]/,
...
assert.match(extractFunctionSpan(filesScript, "createReportAction"), /icon:\s*"alert"[\s\S]*iconOnly:\s*true[\s\S]*text:\s*""[\s\S]*action:\s*"files\.report"[\s\S]*stopFileRowActionEvent/,
  "Report should be icon-only and route-backed on dense Files rows");
assert.match(extractFunctionSpan(filesScript, "createQuarantineAction"), /icon:\s*"shield-alert"[\s\S]*iconOnly:\s*true[\s\S]*label:\s*`Review \$\{row\.fileName\}`[\s\S]*text:\s*""[\s\S]*action:\s*"files\.quarantine"[\s\S]*quarantineFile\(row\.fileId, row\.file\)/,
  "The quarantine route action should appear as an icon-only Review control in normal UI");
assert.match(extractFunctionSpan(filesScript, "quarantineFile"), /title:\s*"Move file to review\?"[\s\S]*confirmLabel:\s*"Move to Review"[\s\S]*\/quarantine/,
...
assert.match(extractFunctionSpan(attachmentHelper, "createAttachmentMetaChip"), /surface-chip[\s\S]*file-attachment-meta-chip[\s\S]*aria-label/,
...
const attachmentStatusMessage = extractFunctionSpan(attachmentHelper, "statusMessage");
...
assert.match(extractFunctionSpan(attachmentHelper, "attachmentList"), /"No attachments yet\."/,
  "Attachment panels should share the standardized empty state copy");
const attachmentStatusLabel = extractFunctionSpan(attachmentHelper, "statusLabel");
...
assert.match(extractFunctionSpan(attachmentHelper, "attachmentRecoveryMessage"), /recovery window[\s\S]*in review[\s\S]*review completes[\s\S]*review is complete/,
