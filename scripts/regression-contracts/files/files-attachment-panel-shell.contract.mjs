...
const render = extractFunctionSpan(helper, "render");
...
const panelShell = extractFunctionSpan(helper, "createAttachmentPanelShell");
...
const listShell = extractFunctionSpan(helper, "createAttachmentListShell");
...
const emptyShell = extractFunctionSpan(helper, "createAttachmentEmptyState");
...
const uploadResults = extractFunctionSpan(helper, "uploadResultList");
...
const actions = extractFunctionSpan(helper, "createAttachmentActions");
...
const actionButton = extractFunctionSpan(helper, "createAttachmentActionButton");
...
const recovery = extractFunctionSpan(helper, "attachmentRecoveryMessage");
...
const refresh = extractFunctionSpan(helper, "refresh");
...
assert.match(uploadFiles, /postMultipartJson\("\/api\/files\/upload\/batch", buildUploadForm\(options, files\)\)/, "Upload should use the Files streamed batch route");
assert.doesNotMatch(uploadFiles, /readFileBase64|contentBase64|\/api\/files\/batch/, "Attachment helper should not use base64 JSON for normal uploads");
assert.match(extractFunctionSpan(helper, "buildUploadForm"), /form\.append\("files", file, file\.name\)/, "Upload form should keep file selection handling Files-owned");
assert.match(extractFunctionSpan(helper, "removeAttachment"), /\/api\/files\/attachments\/\$\{encodeURIComponent\(attachmentId\)\}\/remove/, "Remove should keep using the Files attachment route");
assert.match(extractFunctionSpan(helper, "deleteFile"), /\/api\/files\/\$\{encodeURIComponent\(fileId\)\}\/delete/, "Delete should keep using the Files lifecycle route");
...
assert.match(extractFunctionSpan(notesJs, "openFilesDialog"), /view\.showModal\(filesDialog, \{ parent: dialog, trigger: filesToggle \}\)[\s\S]*data-file-attachment-input[\s\S]*focusTarget\?\.focus\(\)/, "Notes Files utility should still open as a stacked child dialog and focus the helper input when available");
assert.match(extractFunctionSpan(taskDialog, "openTaskFilesDialog"), /showTaskModal\(filesDialog, \{ parent: dialog, trigger: fields\.fileToggle \}\)[\s\S]*\[data-file-attachment-input\]/, "Tasks Files utility should still open as a stacked child dialog and focus the helper input when saved");
assert.match(extractFunctionSpan(taskDialog, "mountTaskFileAttachments"), /saveFirstMessage: "Save the task before adding files\."/ , "Tasks should preserve save-first messaging for unsaved records");
