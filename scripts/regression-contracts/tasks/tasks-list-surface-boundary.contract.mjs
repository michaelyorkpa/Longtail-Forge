...
assert.match(renderTasks, /requireTaskElement\(taskList\)\.replaceChildren\(\)[\s\S]*nestedTaskDisplayRows\(tasks\)[\s\S]*taskNestingDepths\.set\(task, depth\)[\s\S]*createTaskRow\(task\)/, "Tasks should still render parent-before-child rows through the module-owned row builder");
