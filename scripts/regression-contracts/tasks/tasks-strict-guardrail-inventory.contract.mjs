...
const declarativeGuardrails = readText("scripts/regression-contracts/views/view-descriptor-declarative-guardrails.contract.mjs");
...
assert.match(renderTasks, /view\.createElement\("tr"[\s\S]*emptyTaskMessage\(\)[\s\S]*nestedTaskDisplayRows\(tasks\)[\s\S]*taskNestingDepths\.set\(task, depth\)[\s\S]*createTaskRow\(task\)/, "Empty list state should be helper-built while nested rows stay Tasks-owned");

assert.match(fieldNodes, /taskEditorTitleField\(view\)[\s\S]*taskEditorMetadataRibbon\(view\)[\s\S]*taskEditorContinuitySection\(view\)[\s\S]*taskEditorDetailsSection\(view\)[\s\S]*taskEditorChecklistSection\(view\)[\s\S]*taskEditorRecurrenceSection\(view\)[\s\S]*taskEditorTimerSection\(view\)[\s\S]*taskEditorReminderSection\(view\)[\s\S]*taskEditorNotesSection\(view\)/, "Task editor field nodes should compose helper-built sections");
assert.match(taskDialogScript, /function taskEditorContinuitySection[\s\S]*className: \["task-continuity-row", "surface-modal-group"\][\s\S]*data-task-continuity-row[\s\S]*taskEditorLabel\(view, "Resume note"[\s\S]*taskEditorLabel\(view, "Next action"[\s\S]*taskEditorLabel\(view, "Blocked reason"/, "Task continuity row should be helper-built and always mounted");
assert.match(taskDialogScript, /const complete = view\.createActionButton\([\s\S]*action: "complete-task"[\s\S]*iconOnly: true[\s\S]*label: "Complete task"/, "Task Complete should be a header action built through the shared action primitive");
...
assert.match(taskContext, /blockingSummaryText\(taskRowField\(task, "relationshipSummary"\)\)[\s\S]*view\.createDetailBadgeRow/, "Relationship and recovery context should stay Tasks-owned while using shared badge anatomy");
