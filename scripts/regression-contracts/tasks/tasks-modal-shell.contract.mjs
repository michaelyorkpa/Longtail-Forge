...
assert.match(taskDialogScript, /renderDescriptorModalForm\(descriptor, \{[\s\S]*className: "task-detail-dialog"[\s\S]*formClassName: "task-form"[\s\S]*size: descriptor\.size[\s\S]*fields: taskEditorFieldNodes\(\)[\s\S]*utilityActions: taskEditorUtilityActions\(descriptor\)[\s\S]*actions: taskEditorCommitActions\(descriptor\)/, "Task editor should render through the framework descriptor modal form helper");
...
assert.ok(
  tasksView.indexOf("js/shared/view-builder.js") < tasksView.indexOf("js/shared/view-renderer.js")
    && tasksView.indexOf("js/shared/view-renderer.js") < tasksView.indexOf("js/task-dialog.js")
    && tasksView.indexOf("js/task-dialog.js") < tasksView.indexOf("js/tasks.js"),
