...
assert.match(parentOptions, /taskProjectionFields\(task\)\.task_id !== taskId[\s\S]*!selectedClientId \|\| !taskProjectionFields\(task\)\.client_id \|\| taskProjectionFields\(task\)\.client_id === selectedClientId[\s\S]*!selectedProjectId \|\| !taskProjectionFields\(task\)\.project_id \|\| taskProjectionFields\(task\)\.project_id === selectedProjectId[\s\S]*localeCompare/, "Parent task options should preserve task-owned scope filtering and readable title sorting");
...
assert.match(linkedNoteItem, /displayLabel:\s*note\.label \|\| "Untitled note"[\s\S]*hintLabel:\s*note\.excerpt \|\| \(note\.security_mode === "secure" \? "Secure note body is hidden\." : ""\)[\s\S]*sourceUrl:\s*note\.sourceUrl \|\| `notes\.html\?note=\$\{encodeURIComponent\(String\(note\.id \|\| ""\)\)\}`/, "Linked note rows should use readable note labels, safe hints, and normal note URLs");
...
assert.match(mountNotes, /namespace\.notesLinkedPanel\.mount\(fields\.notesContainer,[\s\S]*moduleId:\s*"tasks"[\s\S]*readonly:\s*optionalTaskProjectionFields\(task\)\?\.status === "archived"[\s\S]*saveFirstMessage:\s*"Save the task before adding notes\."[\s\S]*targetType:\s*"task"[\s\S]*title:\s*"Task Notes"/, "Task dialog should keep the Notes-owned linked panel contract");
