...
assert.match(decorateControls, /icons\.decorateButton\(requireTaskIconButton\(fields\.timerStart\), \{ icon: "start", label: "Start task timer"[\s\S]*icons\.decorateButton\(requireTaskIconButton\(fields\.timerPause\), \{ icon: "pause", label: "Pause task timer"[\s\S]*icons\.decorateButton\(requireTaskIconButton\(fields\.timerFinalize\), \{ icon: "save", label: "Save task timer as time"[\s\S]*icons\.decorateButton\(requireTaskIconButton\(fields\.timerReset\), \{ icon: "restore", label: "Reset task timer"[\s\S]*variant: "danger"/, "Task timer controls should keep recognizable icon treatment");
...
assert.match(saveTimer, /currentTaskTimer\(taskProjectionFields\(task\)\.task_id\)[\s\S]*readTaskTimerElapsedSeconds\(timer\)[\s\S]*api\.putJson\(`\/api\/tasks\/\$\{encodeURIComponent\(`\$\{taskProjectionFields\(task\)\.task_id\}`\)\}\/timer`[\s\S]*timer_status: timerStatus[\s\S]*applyTaskTimerMutationResult\(result, task\)/, "Start/pause should preserve Tasks-owned timer save behavior and apply the returned task state");
assert.match(finalizeTimer, /api\.postJson\(`\/api\/tasks\/\$\{encodeURIComponent\(`\$\{taskProjectionFields\(task\)\.task_id\}`\)\}\/timer\/finalize`[\s\S]*duration_seconds: durationSeconds[\s\S]*removeTaskTimer\(taskProjectionFields\(task\)\.task_id\)[\s\S]*applyTaskTimerMutationResult\(result, task\)[\s\S]*setStatus\("Task time saved\."\)/, "Finalize should preserve Tasks-owned time-save behavior and apply the returned task state");
assert.match(resetTimer, /modal\.confirm\(\{[\s\S]*title: "Reset task timer"[\s\S]*danger: true[\s\S]*api\.deleteJson\(`\/api\/tasks\/\$\{encodeURIComponent\(`\$\{taskProjectionFields\(task\)\.task_id\}`\)\}\/timer`\)[\s\S]*removeTaskTimer\(taskProjectionFields\(task\)\.task_id\)[\s\S]*applyTaskTimerMutationResult\(result, task\)/, "Reset should preserve confirmation, Tasks-owned delete behavior, and returned task state");
assert.match(applyTimerMutation, /optionalTaskProjectionFields\(result\)\?\.timer[\s\S]*upsertTaskTimer\(taskProjectionFields\(result\)\.timer\)[\s\S]*optionalTaskProjectionFields\(result\)\?\.task[\s\S]*currentTask = \{[\s\S]*\.\.\.taskProjectionFields\(currentTask \|\| \{\}\)[\s\S]*\.\.\.optionalTaskProjectionFields\(taskProjectionFields\(result\)\.task\)[\s\S]*syncTaskStatusField\(currentTask\)[\s\S]*writeTaskMetadataRibbon\(currentTask\)[\s\S]*notifyTaskEditorSaved\(result\)[\s\S]*writeTaskTimerFields\(currentTask\)/, "Task timer mutations should immediately apply the authoritative task payload returned by the service");
assert.match(syncTaskStatusField, /fields\.status[\s\S]*optionalTaskProjectionFields\(task\)\?\.status[\s\S]*taskContextOptionItems\(taskProjectionFields\(fields\.status\)\.options\)[\s\S]*writeTaskControl\(fields\.status, "value", taskProjectionFields\(task\)\.status\)/, "Timer-start status transitions should update the visible Task modal status select");
assert.match(writeTimerFields, /options\.taskTimersEnabled !== false[\s\S]*options\.timeTrackingEnabled !== false[\s\S]*writeTaskControl\(fields\.timerStart, "disabled", !eligible \|\| optionalTaskProjectionFields\(timer\)\?\.timer_status === "running"[\s\S]*writeTaskControl\(fields\.timerPause, "disabled", !eligible \|\| optionalTaskProjectionFields\(timer\)\?\.timer_status !== "running"[\s\S]*writeTaskControl\(fields\.timerFinalize, "disabled", !eligible \|\| !timer[\s\S]*writeTaskControl\(fields\.timerReset, "disabled", !timer/, "Timer state should preserve eligibility and disabled rules");
...
assert.match(currentTimer, /taskTimers\.find\(\(timer\) => taskProjectionFields\(timer\)\.task_id === taskId\)/, "Current task timer lookup should remain task-owned");
assert.match(upsertTimer, /timer_status === "running" && taskProjectionFields\(item\)\.task_id !== taskProjectionFields\(timer\)\.task_id[\s\S]*timer_status: "paused"[\s\S]*requireTaskControl\(context\)\.taskTimers = taskTimers/, "Starting one task timer should pause other running task timers in modal state");
assert.match(removeTimer, /taskTimers = taskTimers\.filter\(\(timer\) => taskProjectionFields\(timer\)\.task_id !== taskId\)[\s\S]*requireTaskControl\(context\)\.taskTimers = taskTimers/, "Removing a task timer should refresh modal timer state");
...
assert.match(fileMount, /namespace\.fileAttachments\?\.mount[\s\S]*canRemove: Boolean\(optionalTaskProjectionFields\(task\)\?\.task_id\)[\s\S]*canUpload: Boolean\(optionalTaskProjectionFields\(task\)\?\.task_id\)[\s\S]*moduleId: "tasks"/, "Files utility should remain mounted through the Files-owned attachment helper");
assert.ok(
...
assert.match(notesMount, /namespace\.notesLinkedPanel\?\.mount[\s\S]*writeTaskControl\(fields\.notesPanel, "open", options\.focus === true\)[\s\S]*moduleId: "tasks"[\s\S]*readonly: optionalTaskProjectionFields\(task\)\?\.status === "archived"[\s\S]*saveFirstMessage: "Save the task before adding notes\."[\s\S]*targetType: "task"[\s\S]*title: "Task Notes"/, "Notes utility should remain mounted through the Notes-owned linked panel helper");
assert.match(followWriter, /const statusOwner = namespace\.notificationSubscriptions;\s+const readStatus = statusOwner\.readStatus;\s+const targetOwner = namespace\.notificationSubscriptions;\s+\/\*\* @type \{unknown\} \*\/\s+const target = Reflect\.apply\(targetOwner\.taskTarget, targetOwner, \[taskId\]\);\s+\/\*\* @type \{Awaited<ReturnType<typeof readStatus>>\} \*\/\s+const result = await Reflect\.apply\(readStatus, statusOwner, \[target\]\)[\s\S]*writeNotificationFollowState\(result\.isFollowing === true\)/, "Notification follow state should read through the shared subscription helper");
assert.match(followToggle, /Reflect\.apply\(subscriptions\.taskTarget, subscriptions, \[currentTaskId\]\)[\s\S]*namespace\.notificationSubscriptions\.unfollow\(target\)[\s\S]*namespace\.notificationSubscriptions\.follow\(target\)[\s\S]*setStatus\(result\.isFollowing \? "Task notifications followed\." : "Task notifications unfollowed\."\)/, "Notification follow toggle should preserve follow/unfollow behavior");
...
assert.match(copyTaskLink, /new global\.URL\("tasks\.html", global\.location\.href\)[\s\S]*url\.searchParams\.set\("task", `\$\{taskProjectionFields\(task\)\.task_id\}`\)[\s\S]*navigator\.clipboard\.writeText\(url\.toString\(\)\)[\s\S]*setStatus\("Task link copied\."\)/, "Copy task link should preserve task URL and clipboard behavior");
...
assert.match(assertTimersEnabled, /canWriteModule\(session\.workspace_id, TASKS_MODULE_ID\)[\s\S]*canWriteModule\(session\.workspace_id, TIME_TRACKING_MODULE_ID\)[\s\S]*tasksSettingsService\.readTaskTimersEnabled\(session\)[\s\S]*!taskTimersEnabled/, "Task timer writes should preserve Tasks, Time Tracking, and the Tasks-owned setting gate");
...
assert.match(transitionTask, /task\.status !== "open" && task\.status !== "blocked"[\s\S]*blocked_reason: ""[\s\S]*status: "in_progress"[\s\S]*action: "task_timer_status_started"[\s\S]*movedTaskToInProgress: true[\s\S]*previousBlockedReason/, "Starting a timer should move Open or Blocked Tasks to In Progress while retaining recoverable transition metadata");
assert.match(revertTransition, /transition\.movedTaskToInProgress !== true \|\| task\.status !== "in_progress"[\s\S]*taskWorkEvidenceService\.readStartedWorkEvidence[\s\S]*evidence\.hasStartedWork[\s\S]*restoredStatus[\s\S]*previousBlockedReason[\s\S]*action: "task_timer_status_reverted"/, "Reset should restore only timer-started lifecycle transitions when no independent work evidence remains");
