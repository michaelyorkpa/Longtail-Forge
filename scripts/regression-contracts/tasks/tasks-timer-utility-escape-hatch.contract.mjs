...
assert.match(applyTimerMutation, /result\?\.timer[\s\S]*upsertTaskTimer\(result\.timer\)[\s\S]*result\?\.task[\s\S]*currentTask = \{[\s\S]*\.\.\.taskProjectionFields\(currentTask \|\| \{\}\)[\s\S]*\.\.\.result\.task[\s\S]*syncTaskStatusField\(currentTask\)[\s\S]*writeTaskMetadataRibbon\(currentTask\)[\s\S]*notifyTaskEditorSaved\(result\)[\s\S]*writeTaskTimerFields\(currentTask\)/, "Task timer mutations should immediately apply the authoritative task payload returned by the service");
assert.match(syncTaskStatusField, /fields\.status[\s\S]*task\?\.status[\s\S]*taskContextOptionItems\(taskProjectionFields\(fields\.status\)\.options\)[\s\S]*writeTaskControl\(fields\.status, "value", task\.status\)/, "Timer-start status transitions should update the visible Task modal status select");
