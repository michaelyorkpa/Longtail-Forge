...
assert.match(taskDialogScript, /client_id: usesClientScope\(\) \? requireTaskControl\(fields\.client\)\.value : ""/, "Personal and Family task saves should keep client context empty");
...
assert.match(taskDialogScript, /function syncClientFromSelectedProject\(\)[\s\S]*const project = findProjectOption\(fields\.project\.value\)[\s\S]*const derivedClientId = taskProjectionFields\(project\)\.client_id \|\| ""[\s\S]*writeTaskControl\(fields\.client, "value", derivedClientId[\s\S]*populateProjectInput\(fields\.project\.value\)/, "Project selection should derive the Business client and preserve the selected project");
