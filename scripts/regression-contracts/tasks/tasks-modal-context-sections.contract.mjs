...
assert.match(taskDialogScript, /const isBlocked = optionalTaskProjectionFields\(fields\.status\)\?\.value === "blocked"[\s\S]*fields\.blockedReasonField\.hidden = !isBlocked;[\s\S]*writeTaskControl\(fields\.blockedReason, "disabled", !isBlocked\);/, "Blocked Reason should only be selectable when status is blocked");

assert.match(taskDialogScript, /requireTaskControl\(dialog\)\.querySelectorAll\("\[data-client-workspace-control\]"\)\.forEach\(\(element\) => \{[\s\S]*Reflect\.set\(element, "hidden", !hasClientScope\);/, "Personal and Family workspaces should hide Client controls");
...
assert.match(taskDialogScript, /client_id: usesClientScope\(\) \? taskProjectionFields\(requireTaskControl\(fields\.client\)\)\.value : ""/, "Personal and Family task saves should keep client context empty");
...
assert.match(taskDialogScript, /function syncClientFromSelectedProject\(\)[\s\S]*const project = findProjectOption\(taskProjectionFields\(fields\.project\)\.value\)[\s\S]*const derivedClientId = taskProjectionFields\(project\)\.client_id \|\| ""[\s\S]*writeTaskControl\(fields\.client, "value", derivedClientId[\s\S]*populateProjectInput\(taskProjectionFields\(fields\.project\)\.value\)/, "Project selection should derive the Business client and preserve the selected project");
