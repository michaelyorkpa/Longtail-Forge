...
assert.match(extractFunctionSpan(tasks, "captureBulkBlockedReason"), /value !== "blocked" \|\| bulkBlockedReasonInput\?\.value\.trim\(\)[\s\S]*Why is the task now blocked\?[\s\S]*if \(!result\.confirmed\)[\s\S]*bulkBlockedReasonInput\.value = result\.value/, "bulk Block should reuse an existing reason, cancel cleanly, or capture the shared prompt value");
