...
assert.match(
  extractFunctionSpan(taskDialog, "handleChecklistInputKeydown"),
...
assert.match(
  extractFunctionSpan(taskDialog, "handleChecklistListKeydown"),
...
assert.match(
  extractFunctionSpan(tasks, "createTaskFilterChrome"),
...
assert.match(extractFunctionSpan(tasks, "populateTagFilter"), /tags\?\.mountFilterPicker\?\.\(tagFilter,[\s\S]*tags,[\s\S]*value: nextValue/, "the Tasks filter should use the shared searchable tag picker");
...
assert.match(inheritance, /writeTaskControl\(fields\.dueDate, "value", taskProjectionFields\(parentTask\)\.due_date[\s\S]*writeTaskControl\(fields\.dueTime, "value", taskProjectionFields\(parentTask\)\.due_time[\s\S]*writeTaskControl\(fields\.priority, "value",/, "selecting a parent should inherit schedule and priority");
assert.match(inheritance, /!taskProjectionFields\(requireTaskControl\(fields\.client\)\)\.value && taskProjectionFields\(parentTask\)\.client_id[\s\S]*writeTaskControl\(fields\.client, "value", taskProjectionFields\(parentTask\)\.client_id/, "selecting a parent should fill an empty client");
assert.match(inheritance, /!taskProjectionFields\(requireTaskControl\(fields\.project\)\)\.value && taskProjectionFields\(parentTask\)\.project_id[\s\S]*populateProjectInput\(taskProjectionFields\(parentTask\)\.project_id/, "selecting a parent should fill an empty project");
...
assert.match(extractFunctionSpan(tasks, "createTaskRow"), /is-task-child[\s\S]*--task-nesting-depth/, "child rows should expose a bounded visual nesting depth");
...
assert.match(extractFunctionSpan(tasks, "openTaskDialogForBlock"), /focusTarget: "blocked_reason"[\s\S]*promptBlockedReason: true[\s\S]*status: "blocked"/, "Tasks Block should open the canonical editor and request the shared Block capture");
assert.match(extractFunctionSpan(workbench, "blockFocusedTask"), /openTaskCandidate\([\s\S]*defaults: \{ status: "blocked" \}[\s\S]*focusTarget: "blocked_reason"[\s\S]*promptBlockedReason: true/, "Workbench Block should open the canonical editor and request the shared Block capture");
assert.match(extractFunctionSpan(taskDialog, "focusTaskEditorTarget"), /blocked_reason: fields\.blockedReason[\s\S]*blocked_reason: fields\.taskDetailsPanel/, "Blocked Reason focus should open Task Details and focus the field");
assert.match(extractFunctionSpan(taskDialog, "updateBlockedReasonState"), /writeTaskControl\(fields\.blockedReason, "required", isBlocked\)/, "the canonical editor should mark Blocked Reason required while status is Blocked");
...
assert.match(extractFunctionSpan(taskDialog, "performBlockCapture"), /callTaskContextCollection\(previousReason, "trim", \[\]\)[\s\S]*if \(!blockedReason\)[\s\S]*prompt: "Why is the task now blocked\?"[\s\S]*if \(!result\.confirmed\)[\s\S]*writeTaskControl\(fields\.status, "value", priorStatus[\s\S]*saveTaskForm\([\s\S]*Blocking task/, "Continue should persist Blocked and its reason while Cancel restores the prior editor status, with an existing reason suppressing the prompt");
...
// 0.33.33.38.2.2.5.2 removed the hasTaskEditPermission half. It read a grant list the
// canonical stored context has never published, so it was constant true and "permission-
// gated" described nothing. Every other clause of this claim is unchanged and still asserted.
assert.match(extractFunctionSpan(taskDialog, "updateBlockTaskActionState"), /isBlocked[\s\S]*!requireTaskLifecycleLegality\(\)\.isTerminalStatus\(status\)[\s\S]*icon: isBlocked \? "start" : "pause"[\s\S]*label/, "the header action should be edit-only, Play/Resume while Blocked, Pause/Block while active, and hidden only for terminal states");
assert.doesNotMatch(taskDialog, /hasTaskEditPermission/, "and must not reintroduce a browser gate with no grant source");
assert.match(extractFunctionSpan(taskDialog, "resumeBlockedTask"), /writeTaskControl\(fields\.status, "value", "in_progress"[\s\S]*writeTaskControl\(fields\.blockedReason, "value", ""[\s\S]*saveTaskForm\([\s\S]*Resuming task/, "the header Play action should persist Blocked work as In Progress and clear its Blocked Reason");
...
assert.match(extractFunctionSpan(taskDialog, "saveTaskForm"), /optionalTaskProjectionFields\(currentTask\)\?\.status === "blocked"[\s\S]*await refreshTaskTimers\(\)/, "saving Blocked in the canonical editor should reload its automatically paused timer state");
assert.match(extractFunctionSpan(taskService, "pauseRunningTimersForBlockedTask"), /shouldPauseRunningTimersForBlockedTask\(task\)[\s\S]*taskTimersService\.pauseRunningForBlockedTask\(task, session\)/, "every Tasks-owned Blocked persistence path should use the typed task-timer pause decision boundary");
assert.match(extractFunctionSpan(taskTimerService, "pauseRunningForBlockedTask"), /activeTimersService\.pauseRunningSourced\(taskTimerSource\(task\), session\)/, "Tasks should request source-scoped pause through the Time Tracking service");
assert.match(extractFunctionSpan(activeTimerRepository, "pauseRunningBySource"), /db\.run\(pauseRunningBySourceSql\(\), \{[\s\S]*sourceId:[\s\S]*sourceModuleId:[\s\S]*sourceType:/, "source-scoped pause should use named parameters through the reviewed dialect-aware SQL builder");
assert.match(extractFunctionSpan(activeTimerRepository, "pauseRunningBySourceSql"), /source_module_id = :sourceModuleId[\s\S]*source_type = :sourceType[\s\S]*source_id = :sourceId[\s\S]*timer_status = 'running'/, "source-scoped pause should cover every running timer for the Task");
assert.doesNotMatch(extractFunctionSpan(activeTimerRepository, "pauseRunningBySourceSql"), /user_id/, "source-scoped pause should not leave another user's running timer active on the Blocked Task");
assert.match(await readText("src/modules/tasks/module.permissions.js"), /tasks\.edit_all[\s\S]*tasks\.edit_own/, "the all-task and own-task edit distinction stays declared where the server enforces it");
assert.match(extractFunctionSpan(taskService, "normalizeTaskPayload"), /status === "blocked" && !blockedReason[\s\S]*Blocked Reason is required when a task is Blocked\.[\s\S]*400/, "the service boundary should reject blocked tasks without a reason");
assert.match(extractFunctionSpan(tasks, "applyBulkAction"), /captureBulkBlockedReason[\s\S]*selectedBulkActions/, "bulk Block should capture a missing reason before constructing status actions");
assert.match(extractFunctionSpan(tasks, "captureBulkBlockedReason"), /value !== "blocked" \|\| optionalBulkInput\(bulkBlockedReasonInput\)\?\.value\.trim\(\)[\s\S]*Why is the task now blocked\?[\s\S]*if \(!result\.confirmed\)[\s\S]*requireBulkInput\(bulkBlockedReasonInput\)\.value = result\.value/, "bulk Block should reuse an existing reason, cancel cleanly, or capture the shared prompt value");
assert.match(extractFunctionSpan(tasks, "selectedBulkActions"), /blocked_reason: status === "blocked" \? blockedReason : ""/, "bulk Block should send the required reason through the canonical status action");
assert.doesNotMatch(extractFunctionSpan(taskService, "blockParentsForIncompleteChild"), /capturePrompt|Why is the task now blocked/, "automatic parent rollup should remain a non-interactive service transition");

assert.match(
  extractFunctionSpan(tasks, "taskWorkflowActionVisible"),
...
assert.match(
  extractFunctionSpan(workbench, "visibleTimerPanelTimers"),
