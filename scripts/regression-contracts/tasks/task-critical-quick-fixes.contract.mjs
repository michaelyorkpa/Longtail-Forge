...
assert.match(functionBlock(taskService, "pauseRunningTimersForBlockedTask"), /shouldPauseRunningTimersForBlockedTask\(task\)[\s\S]*taskTimersService\.pauseRunningForBlockedTask\(task, session\)/, "every Tasks-owned Blocked persistence path should use the typed task-timer pause decision boundary");
