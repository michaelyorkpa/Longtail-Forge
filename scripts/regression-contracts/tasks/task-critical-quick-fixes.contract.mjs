...
assert.match(inheritance, /!requireTaskControl\(fields\.client\)\.value && taskProjectionFields\(parentTask\)\.client_id[\s\S]*writeTaskControl\(fields\.client, "value", taskProjectionFields\(parentTask\)\.client_id/, "selecting a parent should fill an empty client");
assert.match(inheritance, /!requireTaskControl\(fields\.project\)\.value && taskProjectionFields\(parentTask\)\.project_id[\s\S]*populateProjectInput\(taskProjectionFields\(parentTask\)\.project_id/, "selecting a parent should fill an empty project");
...
assert.match(extractFunctionSpan(taskDialog, "updateBlockedReasonState"), /fields\.blockedReason\.required = isBlocked/, "the canonical editor should mark Blocked Reason required while status is Blocked");
...
assert.match(extractFunctionSpan(taskDialog, "performBlockCapture"), /previousReason\.trim\(\)[\s\S]*if \(!blockedReason\)[\s\S]*prompt: "Why is the task now blocked\?"[\s\S]*if \(!result\.confirmed\)[\s\S]*writeTaskControl\(fields\.status, "value", priorStatus[\s\S]*saveTaskForm\([\s\S]*Blocking task/, "Continue should persist Blocked and its reason while Cancel restores the prior editor status, with an existing reason suppressing the prompt");
