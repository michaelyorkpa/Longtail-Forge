...
assert.match(functionBlock(helper, "buildUploadForm"), /form\.append\("files", file, file\.name\)/, "Upload form should keep file selection handling Files-owned");
...
assert.match(workbenchHtml, /css\/longtail-forge\.css\?v=25/, "Workbench should cache-bust the shared stylesheet for attachment panel shell changes");
