...
assert.match(filesPage, /css\/longtail-forge\.css\?v=11/, "Files page should cache-bust preview modal styling");
assert.match(filesPage, /js\/files\.js\?v=10/, "Files page should cache-bust the preview modal browser wiring");
...
assert.match(previewAction, /event\.stopPropagation\(\)[\s\S]*openFilePreview\(row,\s*\{\s*trigger:\s*event\.currentTarget\s*\}\)/, "Preview button should open Preview without triggering row edit");
...
assert.match(downloadAction, /href = `\/api\/files\/\$\{encodeURIComponent\(row\.fileId\)\}\/download`[\s\S]*setAttribute\("download", ""\)[\s\S]*aria-label[\s\S]*dataset\.surfaceAction = "download-preview-file"/, "Preview modal Download action should keep using the existing Files download route");
