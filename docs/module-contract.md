...
As of 0.33.4.8, Notes and Lists are registered as first-party workflow modules with module-owned migration sources and workspace module-status settings. Notes contributes storage, Library-linking contracts, hierarchical Library collections, Markdown/revision/secure-note behavior, searchable/taggable/attachable declarations, current-state product Help pages, conservative owner notifications, and the protected Notes Library UI; its developer handoff lives in `docs/notes-module.md`, and import planning lives in `docs/notes-import-planning.md`. Lists contributes storage for lists, list items, explicit catalog suggestions, and linked records; workspace-aware Shopping/Procurement labels; service/API/UI workflows for item execution, reusable lists, bill-of-materials finalization, linked records, progress summaries, and resume-safe context; searchable/taggable/attachable declarations; current-state product Help pages; and the protected Lists UI. Its developer handoff lives in `docs/lists-module.md`. Both modules stay on framework services for permissions, tags, search sync, file attachments, audit, events, Help, and module lifecycle, and neither module writes directly to framework search or file storage tables.
