...
`viewSurfaces` is the declarative manifest field for framework-rendered protected views. As of 0.33.5.16.10, the backend validates descriptor shape and references, resolves descriptor labels through the existing workspace terminology resolver, exposes active descriptors through the existing app-shell bootstrap payload, and the browser renderer at `public/js/shared/view-renderer.js` can fetch descriptor data sources through `LongtailForge.api.getJson`, render descriptor select filters, expose stable generic field hooks, preserve basic field attributes, and dispatch declarative actions through route metadata or registered behavior handlers. Lists now uses a descriptor for its protected workspace read shell, item form fields, item table/action placement, and create/edit modal shell while later slices still own linked-record placement and workflow-action migration. A descriptor requires `id`, `moduleId`, `viewId`, `layout`, and `dataSource.fieldBindings`. Supported layouts are `single-column`, `split-list-detail`, and `table-page`. Descriptors may declare `pageHeader`, `filters`, `indexPanel`, `table`, `detail`, `modals`, and `actions` using framework-owned anatomy names. Descriptor `moduleId` values must reference a known module, `viewId` values must bind to a protected view for that module, surface IDs must be unique across loaded modules, action `requiredPermissions` must reference known core or module permissions, action roles are limited to `primary`, `secondary`, `destructive`, and `utility`, and descriptor routes must be local app paths with supported HTTP methods.
