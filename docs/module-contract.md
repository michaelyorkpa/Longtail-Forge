...
Modules contribute declarative view data, normalized data endpoints, field-binding maps, and named behavior IDs for custom actions. The framework owns common page/layout/modal/table/action anatomy, surface classes, overflow wrappers, responsive behavior, focus/accessibility defaults, terminology resolution, and static descriptor-to-DOM rendering through the existing `LongtailForge.view` primitives. Modules keep ownership of data loading semantics, record labels and fields, validation, save payloads, API routes, permission checks, service logic, behavior handlers, and workflow decisions.
