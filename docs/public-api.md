...
Task payloads may include `title`, `description`, `status`, `priority`, nullable `estimate_minutes`, `client_id`, `project_id`, `assignee_ids`, `due_date`, `due_time`, reminder policy fields, and recurrence fields used by the browser workflow. `estimate_minutes` is returned on list/detail/write responses as an integer or `null`; non-null writes must be non-negative multiples of 15. Project-linked tasks inherit the project's client context server-side. Personal and Family workspaces may create workspace-only and project-linked tasks, but direct client task scopes are Business-only.

As of 0.33.9.6, completing a recurring task through `POST /api/v1/tasks/:taskId/complete` follows the asynchronous recurrence contract. The response includes the completed task, `createdTask` remains `null`, and `recurrenceContinuity` reports `pending`, `available`, `ended`, or `handoff_failed` with the computed `nextScheduledDate`. When available, `nextTask` contains only the readable task ID, title, due date, and task URL. `recurrenceJob` contains queue/failure booleans only. A handoff failure means the task completed and the durable recurrence sweep still owns recovery; public API responses never expose job IDs, dedupe keys, payload JSON, or worker internals.
