...
The 0.33.5.18.10.8 modal action standardization branch should keep the Tasks-style compact Cancel and Save commit controls, but align converted modal utility actions with Notes by rendering readable icon-plus-text Tags, Files, and Copy Link buttons where space allows. That visual alignment must not move Tags picker behavior, Files attachment behavior, Copy Link URL/clipboard behavior, notification follow/unfollow, save payloads, validation, or permissions out of Tasks.
