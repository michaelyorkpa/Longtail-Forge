...
As of 0.33.32.6, the active-timer service and repository are checked against one repository-owned `ActiveTimer` record plus their Zod-inferred mutation payloads and the canonical Time Entry input. Finalization always treats integer seconds as authoritative: a stored timer uses its accumulated active seconds plus any running segment, while the compatibility fallback retains its existing one-second minimum when no timer row exists. Both paths derive the stored four-decimal `duration_hours` projection from those final seconds, so caller-supplied hours cannot contradict billing and reporting totals. Timer removal, lifecycle events, project scope, billable handling, timestamps, and Tasks delegation remain unchanged.

As of 0.33.32.7, the authenticated session timezone is the single calendar authority for Project Time & Billing periods, custom date ranges, billing chart months, and Dashboard effort windows. Stored Time Entry timestamps remain UTC. Each service derives local date keys for the signed-in user and converts inclusive starts plus exclusive next-period ends to UTC before querying, so the application server's own timezone cannot shift totals and a DST transition day naturally spans 23 or 25 UTC hours. The checked billing and Dashboard services share the canonical timezone helpers instead of maintaining separate calendar arithmetic.
...
As of 0.33.21.19.1, the effort-summary time-entry path is bounded to the displayed seven local calendar days. A purpose-built repository read uses the existing `time_entries (workspace_id, end_time)` index, computes entry count plus today/window duration totals with SQL aggregates, and loads at most the three recent rows rendered by the card. Existing workspace, owner, Client, and Project visibility is applied in the SQL predicates and rechecked only for those displayed rows; the Dashboard does not invoke the shared full-list or tag-decoration pipeline. Request-scoped workspace-settings memoization is shared with active-timer shaping, so repeated reads on this endpoint produce one physical settings query.

As of 0.33.21.19.5, the branch-level `dashboard.hot-endpoint-budgets` HTTP guard caps effort summary at 500 ms / 4 KiB / 20 statements and proves 500 out-of-window historical entries add no payload and at most two statements. This conservative CI ceiling complements the tighter `time-tracking.dashboard-effort-summary-budgets` service contract (14 statements, 2.8 KiB, one settings query, and exact three-row authorization); it does not authorize wider history, extra rows, or a return to the full Time Entries list/tag path.
...
As of 0.33.21.19.4, the generic Dashboard entry starts the declared effort-summary `dataRoute` before contributed assets finish and stores it in the shared route-keyed promise map. Both Time Tracking renderers reuse that one live `no-store` promise, preserving the existing compact payload and module ownership without issuing a second browser request.
...
As of 0.33.21.1, the framework Reporting host renders `tag` filter metadata as the shared typable tag filter instead of a plain select. The Time Tracking renderer still owns option hydration and supplies canonical tag IDs plus readable labels; `LongtailForge.tags.mountFilterPicker(...)` owns search-and-select interaction, All tags/No Tags choices, and canonical selection state used by deep links and report execution.
...
As of 0.33.6.12d-1, Workbench Focus Selection consumes active and paused timers as a read/control list from the Time Tracking contribution, but it does not render the manual creation row. Manual timer creation is deferred to the QAC/Time Tracking create-timer modal slice. Task-sourced timers remain backed by the same active timer storage, but readable task timers in Workbench dispatch Start/Pause/Save Time/Reset through the Tasks timer routes so task eligibility, permissions, status side effects, audit/event/search behavior, and final time-entry creation remain owned by the existing task timer service path. As of 0.33.21.3.1, "Pick up where I left off" merges these live timers with resume state: task-sourced timers pass a batched Tasks lifecycle/readability check and become Task Focus candidates, running before paused, while manual timers keep Time Tracking identity. As of 0.33.21.4.2, manual-timer recommendations say `Continue in Time Tracking` and use the deliberate `time-tracker.html` fallback. Time Tracking has no registered opener that can safely carry an existing active timer's identity and lifecycle; `time-tracking.timer.create` remains creation-only, and Workbench must not substitute its own timer editor. As of 0.33.21.3.3, Time Tracking's public service contract also exposes a workspace-scoped existence check for persisted task-linked entries; Tasks consumes that fact with checklist and active-timer evidence while remaining the sole owner of task status. As of 0.33.21.4.3, the existing `source_metadata_json` on a task-sourced timer carries the Tasks-authored prior lifecycle status and exact Blocked Reason when timer start recovers a Blocked Task. Time Tracking persists that opaque source metadata with the timer; Tasks alone decides whether Reset may restore it after checking current checklist and saved-time evidence. As of 0.33.21.21.1, Time Tracking also owns the internal workspace/source-scoped pause operation used whenever Tasks persists Blocked. It pauses every running timer for that Task across users, accrues elapsed time through the database time seam, and clears the running timestamp without changing source metadata or deleting the timer.

As of 0.33.21.17.2, successful task-timer pause and finalize responses are the lifecycle signals for the non-blocking `Add resume note?` convenience capture. Time Tracking still owns persisted running/paused state and final time-entry creation; the Tasks browser adapter checks and writes the linked Task's canonical `resume_note` through the Tasks service path. Manual timers remain outside this Task-owned capture until they are deliberately linked to a Task, preserving the manual-timer handoff contract.
...
As of 0.33.21.8, Workbench consumes that timer-state notification through the framework-owned `LongtailForge.quickActionRefresh` helper. Its declarative `active_timer` subscription reloads the active-timer source and re-renders the Timers card in place; it does not reload the Workbench or disturb Task Focus/disclosure state.
...
As of 0.33.21.18.1, deterministic development and sanitized-demo Task Timers are persisted exactly like runtime sourced timers: only `running`/`paused` status tokens, the canonical `source:tasks:task:<taskId>` slot and source identity, matching readable Client/Project/task labels, and opaque Tasks-authored lifecycle-transition metadata. The runtime source-unique index remains authoritative; malformed seed rows are corrected in the fixture rather than accommodated by Time Tracking.
