...
As of 0.33.6.12l, Task Focus checklist toggles refresh the selected task summary status chip from the Tasks checklist mutation response without requiring a full page reload. Checking checklist work on an Open task should show In Progress immediately, while clearing all checked checklist work on an eligible In Progress task should show Open immediately. Complete, archived, and blocked task chips must not be softened by checklist toggles.
