...
As of 0.33.5.16.10, Lists item entry fields, item table/action placement, and the create/edit modal shell are descriptor-declared. The Lists browser module binds module-owned field meaning, catalog suggestions, validation, payloads, permissions, and API workflows to that anatomy; the descriptor does not make Lists a generic inventory or purchasing system and does not move linked-record placement or list workflow actions yet.
