...
As of 0.33.6.11, the Workbench Inspector is a subordinate right-side panel on wide Workbench layouts and is not a fixed drawer, overlay, QAC rail, or embedded viewer. It uses the Workbench layout grid, hides on narrow screens, and keeps QAC's bottom-right drawer space separate. Inspector rows show permission-safe candidate titles and context only; opening a title dispatches the existing module modal action where one exists, otherwise the Workbench keeps its explicit page fallback.
