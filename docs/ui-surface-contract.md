...
As of 0.33.21.9, Dashboard and Calendar show only active task statuses by default. Calendar adds a shared-pattern task-status multi-select for widening or narrowing the server-backed read; Dashboard remains a compact active-only summary and does not grow a status control.

As of 0.33.21.8, the QAC timer notification is consumed by Workbench through the shared declarative refresh subscription. A timer created from the footer capture appears in the Timers card without a page reload; the targeted card refresh leaves the active Task Focus surface and the user's timer disclosure choice intact.

As of 0.33.21.10.3, the Workbench Inspector remains a subordinate right-side grid panel on wide layouts and becomes the existing framework full-screen drawer only at the canonical phone breakpoint. The icon-only `Open Inspector` deerstalker action appears in the Workbench header immediately before `Change Focus`; the shared slide-out controller owns body-scroll lock, Tab containment, Escape/close behavior, and focus return. Intermediate narrow layouts continue to omit the Inspector, the phone presentation stays separate from QAC's bottom-right drawer, and neither layout makes the Inspector an embedded viewer. In Focus Selection, it remains the bounded "More in this focus" overflow panel: rows start after the top-five recommended cycle window, show permission-safe candidate titles/context, and title clicks use the same primary candidate action path as the main recommendation. Task Focus continues to use the same Inspector shell for selected-task context.
...
As of 0.33.6.12e-2, the Task Focus Inspector renders that selected-task related-context read model in the existing Workbench right panel. It is default-open and collapsible with a visible caret on wide layouts, hides only the scrollable related-context body when collapsed, keeps the existing narrow-layout hide behavior, and opens related rows through existing module actions or explicit safe fallback URLs. As of 0.33.6.12m, linked Note rows open a Notes-owned read modal through `notes.view`. As of 0.33.6.12n, recurring linked-note propagation uses the same Inspector rows and does not give Workbench a note-body read path; the Inspector still does not embed Notes, Files, Lists, or Tasks previews inline.

As of 0.33.6.12f, the Pick up where I left off Focus Selection recommendation list may insert a Tasks-owned second-most-recent updated active task after running/paused timer candidates. As of 0.33.21.3.1, actual live timers are merged even when resume state exists; readable task-sourced timers render as Task candidates and enter Task Focus, with running before paused, while manual timers remain Time Tracking candidates. This is candidate ordering only: it does not add a new Workbench panel, embedded task list, task filter bar, or browser-side task ordering source.

As of 0.33.6.12g, Workbench view-state panels are mutually exclusive in both layout and focus order. Focus Selection must not show the Task Focus action box, summary shell, checklist/timer/details wrappers, or disabled task buttons; Task Focus must not show the focus-question cards, split focus filters, recommended-action panel, or unfiltered Focus Selection timer list until `Change Focus` returns there.
...
As of 0.33.6.13a, Focus Selection due/project/client/blocked modes render the service-owned overdue-aware candidate order. Due-focused modes include overdue active task work before due-today and upcoming work. As of 0.33.21.3.1, due-focused modes also include due In Progress Tasks, and Blocked candidates appear only in "Review blocked work" rather than leaking into Start my day, due, week, In progress, Project, Client, or resume focus. The blocked mode stays blocked-only and puts blocked-overdue work first. The recommended-action top-five plus right-side Inspector overflow must stay slices of the same ordered list.
...
As of 0.33.21.10.4, the canonical phone layout stacks Task Focus summary chips below the title/context copy and keeps each `Other Active Timers` source/status chip group immediately below its own timer title. Long task, context, tag, and timer labels wrap within their card, and timer actions align within the same card without horizontal page overflow. The desktop summary/timer placement and all timer lifecycle behavior remain unchanged.

As of 0.33.6.12l, Task Focus checklist toggles refresh the selected task summary status chip from the Tasks checklist mutation response without requiring a full page reload. Checking checklist work on an Open task should show In Progress immediately. As of 0.33.21.3.3, clearing the last checked item should show Open only when the Tasks response confirms there is no running or paused task timer and no persisted task-linked time; otherwise the chip remains In Progress. As of 0.33.21.4.3, starting timer/checklist work on a Blocked Task immediately shows In Progress with its active Blocked Reason cleared. An eligible timer Reset may return the exact prior Blocked state/reason, while checked checklist work or persisted time keeps the chip In Progress. Complete and archived task chips remain unchanged. Workbench applies the returned Task; it does not infer these transitions.

As of 0.33.6.13a, Dashboard is the framework-owned workspace pulse/orientation surface. It summarizes state, pressure, and direction through a Workspace Pulse, optional safe setup warnings, and contribution-driven regions; it must not become a full task list, report page, inline editor, full module index, or browser-owned permission/ranking layer. `public/js/dashboard.js` owns the page/header/status/pulse/warning/region anatomy through `LongtailForge.view` helpers, while `src/services/dashboard.service.js` owns the stable `/api/dashboard` read model for workspace summary, region metadata, safe warning summaries, and permission-filtered contribution metadata. Modules own their panel declarations, renderers, data routes, permissions, and record meaning. Dashboard panel placement must come from the allowlisted contribution `placement` field, defaulting to `main`; browser placement logic must not special-case first-party panel IDs such as `project-summary`.

As of 0.33.6.13b, Dashboard's first task pressure surface is split into three compact Tasks-owned panels. `Needs Attention` lives in the `attention` region and renders up to five service-ordered, deduped rows from overdue active tasks, blocked active tasks, due-soon active tasks, and task-linked running/paused timer signals. `Today / Upcoming` lives in the `today` region and shows due-today or due-this-week active task work while excluding overdue and blocked rows. The main `Tasks` card shows only pressure metrics for overdue, due soon, blocked, and assigned-to-me plus a short deduped row list. Task row and metric drilldowns go to Workbench or the Tasks page, not directly into the Task edit modal. The browser must not rebuild task due, status, assignment, timer, workspace, or permission logic, and it must not render task descriptions, raw IDs, private bodies, or module-owned edit forms inside Dashboard.

As of 0.33.6.13c, Dashboard's Time Tracking surface is limited to compact active/recent effort signals. `Active Timers` may show running/paused counts, up to three safe timer rows, and a Workbench handoff. `Recent Time` may show recent saved-time totals, up to three safe time-entry rows, and Time Entries/Reporting handoffs. Dashboard must not render detailed billing tables, billing charts, invoice-ready copy, billable amount totals, Current Month Billables, Hours & Billables by Month, or Client billing language as default Dashboard content. Reporting owns detailed time and billing analysis; Personal and Family workspaces must stay free of billable/billing/invoice Dashboard language.

As of 0.33.6.13d, Dashboard includes a compact Module Overview grid in the `main` region. Module overview cards summarize enabled/permitted modules with safe metrics, at most one suggested/latest row, and one primary handoff; they must not become full module indexes, inline editors, fake future-module placeholders, or walls of disabled/empty cards. Notes, Lists, and Files should be omitted from the grid until their owning modules expose existing body-free summary rows that are safe for Dashboard. The `activity` region exists as Recent Activity, but it currently renders a quiet deferred/empty state unless an existing source can provide permission-safe rows without audit payload JSON, raw IDs, secure/private content, storage paths, scanner internals, protected filesystem data, or job payload JSON.

As of 0.33.6.14.1, Dashboard's visual and interaction rule is summary, pressure, direction. Workspace Pulse is first and visually distinct without becoming a hero billboard; Needs Attention comes before calmer horizon/module/secondary surfaces; Recent Activity and Reporting shortcuts stay visually secondary. Dashboard cards must wrap/truncate safely on narrow and desktop widths, use calm practical copy, and keep empty states useful rather than broken-sounding. Dashboard must not add capture forms, inline editors, full report tables/charts, full task indexes, browser-owned permission/ranking logic, fake future-module cards, unsafe warning/activity payloads, or Personal/Family Client/billing language.
...
As of 0.33.21.4.1, the Task Reminders override grid keeps primary offsets as ordinary required numeric controls and places a compact Enabled checkbox beside each secondary offset heading. Disabling a secondary also disables its number input and persists omission rather than a hidden fallback value; reopening reflects the saved one- or two-offset policy.
