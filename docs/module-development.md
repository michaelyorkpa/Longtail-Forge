...
As of 0.33.32.10, `scripts/typecheck-seam-inventory.json` is the one reviewable inventory of every first-party runtime/test JavaScript file opted in with a first-line `// @ts-check` pragma. Its 52-file floor includes the browser/API authentication middleware, request-session resolver, central Support View gate, raw JSON reader and checked route consumer, canonical normalizers and timezone math, the Time Tracking time-entry, active-timer, billing, and Dashboard service/repository seams, the public API service, the Tasks timer bridge, the SQLite adapter/provider/driver/literal/dialect/binding cluster, six framework repositories, and one checked utility contract test. Active-timer persistence owns one importable `ActiveTimer` record; its service consumes that record with Zod-inferred save/status/finalize payloads and canonical Time Entry finalization inputs. Billing and Dashboard calendar windows consume the shared checked local-date-key/day arithmetic and local-date-to-UTC helpers; do not derive those boundaries from the application server's local `Date` getters or constructors. The database contract separates the full `DatabaseAdapter` from callback-scoped `TransactionClient`: repository functions that open transactions require the adapter, while injected helpers that can participate in one accept the client and cannot typecheck a nested transaction. Dialect calls use the declared builder option bags, and binding consumers narrow normalized `kind` and named-entry `isArray` before accessing variant fields. Settings, Users, and Workspaces own their public input/projection types locally; checked consumers must narrow every nullable single-row read. Later slices add their checked seams to the sorted inventory and raise the floor, while removing a pragma, dropping an inventoried file, or weakening the typecheck scope fails `framework.typecheck-seams`. The checking dials live in `tsconfig.json`: `strict` stays on so real type conflicts fire, `noImplicitAny` is off so unannotated legacy helpers do not drown the signal, and `checkJs` stays per-file opt-in. The nominal `tests/**/*.mjs` include makes tests eligible for explicit opt-in; it does not claim type coverage for unchecked test files. Do not remove an existing `// @ts-check` pragma, add `@ts-nocheck`/`@ts-ignore` to runtime files, import `.ts` files from runtime JavaScript, or add `scripts/` to the typecheck program — the release gate rejects those escapes and scope changes.

As of 0.33.32.11, the inventory adds all eight bundled `src/modules/*/module.js` declarations and raises the monotonic floor from 52 to 60 files. Each manifest is explicitly annotated as `ModuleManifest`, whose identity metadata is required to match runtime validation and whose contribution types match the already-validated notification declarations, Dashboard counts/links, Workbench public-demo capabilities, and readonly protected-content consumers. New structural drift must fail `npm run typecheck` before runtime registry validation; do not use the declaration alignment to add manifest fields or change module data, activation, or loading behavior.

As of 0.33.32.12, the inventory adds `src/core/modules/modules.service.js` and raises the monotonic floor from 60 to 61 files. The service consumes `ModuleManifest`, `TransactionClient`, and explicit API-scope/event-hook catalog projections; generic terminology decorators preserve the validated caller shape. Keep this boundary type-only: do not weaken registry validation or change contribution filtering, module enablement, ordering, workspace caching, envelopes, or lifecycle behavior to satisfy the checker.

As of 0.33.32.13, the inventory adds all six first-party module/Help `search-indexers.js` owners and raises the monotonic floor from 61 to 67 files. Registered indexers consume the shared camelCase `SearchReference`: `workspaceId` is always present, the single-record identifiers are optional for rebuild mode, and `declaration` supplies the active searchable type. Queue and service callers may still use their documented casing aliases before normalization; do not destructure snake_case fields inside an indexer or make rebuild fabricate a record ID merely for the checker.

As of 0.33.32.31, permission-safe Search request composition carries the shared request-session shape through the service boundary and refines `workspace_id` only after the existing active-workspace check. `normalizeSearchRecordReference` produces required camelCase `searchIndexId`, `workspaceId`, `moduleId`, `recordType`, and `recordId` fields; the parallel snake_case fields remain compatibility aliases for persistence adapters and result envelopes, not an indexer-facing vocabulary. Search route query guards depend on `createApp()` retaining Express `extended` parsing, so modules must not mount a second Search parser or route.

As of 0.33.32.36, the inventory adds `src/services/search-index-rebuild.service.js` and raises the floor from 108 to 109 files. Active declaration discovery returns the complete normalized declaration contract, rebuild indexers receive the same camelCase `SearchReference` vocabulary established in 0.33.32.13, and app/workspace/module rebuilds return a named counts/targets/errors summary consumed by durable jobs and maintenance tooling. Module indexers must continue returning object documents or the established document envelope; malformed results remain failures. Keep module eligibility, canonical normalization, stale/inactive cleanup, backend repair, audit, failure recovery, and workspace isolation in the framework rebuild path.

As of 0.33.32.37, the inventory adds the framework-owned private-feed service and route and raises the floor from 109 to 111 files. `src/types/private-feed-contracts.d.ts` is the shared vocabulary for token rows, lifecycle payloads/results, normalized scopes, secret-free subscription descriptors, provider render context, and the dedicated sessionless permission identity. A private-feed provider receives only the frozen descriptor plus that bounded authorization session; it must return calendar content or `null`, must independently reauthorize its module scope, and must never receive the raw bearer, selector, digest, or repository row. Tasks remains the sole provider and owner of task selection and RFC 5545 behavior. Future providers require a separately justified content owner and must not widen browser sessions or bypass the framework credential, throttle, eligibility, generic-rejection, cache, and audit boundary.

As of 0.33.32.14, the inventory adds `work-resume-state-initial-producers.js` and `work-resume-state-read-checks.js` and raises the monotonic floor from 67 to 69 files. Register per-record and batch read resolvers only through their shared callback contracts, and return the established readable/lifecycle flags; first-party producer builders return the shared `ResumeStateProducerResult`, which intentionally extends the dual-cased payload with the existing remove action and title alias. Keep module services authoritative for access and lifecycle reads, and keep filtering, ranking, and normalization in the Resume State service rather than recreating them in producers.

As of 0.33.32.15, the inventory adds `job-queue.js`, `job-handlers.js`, `job-runner.js`, and `event-summaries.js` and raises the monotonic floor from 69 to 73 files. Enqueue keeps its established camelCase/snake_case aliases, including caller-supplied job IDs; registered handlers consume the canonical `{ job, payload }` execution envelope rather than the persisted database row. Event summary resolvers receive the normalized internal event and must keep safe record labels, same-origin relative URLs, recipient hints, and fallback text free of raw record-ID substitution. Keep business effects and idempotency in owner handlers and keep worker claims, retries, and lifecycle transitions in the framework runner.

As of 0.33.32.16, the inventory adds `auth.service.js` and `api-keys.service.js` and raises the monotonic floor from 73 to 75 files. Authentication code narrows a repository user only after the admitted real/dummy credential operation succeeds, distinguishes recovery and Support View sessions from normal workspace-bound sessions, and retains existing audit/security event shapes. API-key management requires a workspace-bound request session, while public API authentication returns the shared nullable `ActiveApiKey`; never widen these type declarations to expose raw key material or bypass live permission, scope, status, creator, or public-demo checks.

As of 0.33.32.17, the inventory adds `permissions.service.js` and `core/permission-resource.js` and raises the monotonic floor from 75 to 77 files. `PermissionResource.workspace_id` is required: checked permission callers must identify the active workspace even for workspace-wide or any-scope operation overrides. Use the shared constructor for generic framework route resources; modules should continue constructing their owner-specific Client/Project record resources explicitly and must not replace service-side permission decisions with browser hints.

As of 0.33.32.18, an already-clean server rollout must first add one exact, sorted path list to `scripts/typecheck-clean-file-passes.json`, name one coherent ownership tier, and stay at or below 40 files. A later worthwhile tier uses a numeric sub-pass instead of expanding an existing review. The guardrail requires pass paths to be non-overlapping members of the complete checked inventory and keeps separately scoped roadmap files out. The first pass adds 12 framework core leaf utilities and raises the floor from 77 to 89 files; defer any candidate whose checker output requires behavior reasoning or contract repair.

As of 0.33.32.19, checked browser utilities use the separate `tsconfig.public.json` program with DOM libraries and no Node ambient types. The first tier contains only `api-client.js`, `error-contract.js`, `records.js`, `formatters.js`, `cached-fetch.js`, and `page-controller.js`; keep them as classic scripts, declare shared globals in `src/types/browser-contracts.d.ts`, and do not add runtime imports, bundling, or reorder page assets for the checker. Reuse framework-owned contracts such as `ApiErrorEnvelope` instead of restating server envelopes. `error-contract.js` owns envelope parsing and request clients delegate to it. As of 0.33.32.30, `api-client.js` must capture that parser during initialization and fail visibly with fixed safe copy if it is absent; do not add a standalone field parser or degraded fallback. The framework preamble owns parser-before-page-assets order. Because classic scripts share one global program scope, use file-specific local JSDoc typedef aliases rather than repeating the same alias name across files. The complete inventory floor is 95 files.

As of 0.33.32.20, app-shell consumers receive the framework-owned `AppShellBootstrap` through `LongtailForge.appShellBootstrap.normalize(unknown)`. Keep the server producer checked against that shared contract and keep field defaults/fallbacks in the small checked adapter; do not make `navigation.js` a whole-file checking target or reimplement bootstrap parsing in a module page. The adapter remains a classic framework preamble script loaded before protected page assets, and the complete inventory floor is 97 files.

As of 0.33.32.21, declarative list descriptors should declare `dataSource.recordsKey` with the exact array field returned by their route, then use `dataSource.fieldBindings` for record fields. `LongtailForge.viewResponseRecords.read(unknown, recordsKey)` is the checked browser boundary: the declared key wins, while direct arrays, the historical known-key set, and the first-array/object behavior remain isolated compatibility for older or external descriptors. Do not add envelope-key guesses to `view-renderer.js` or a module adapter. All bundled descriptors declare their real key, the adapter is a classic framework preamble dependency, and the complete inventory floor is 98 files.

As of 0.33.32.22, declarative view authors consume the precise `ViewSurfaceDescriptor` family in `src/types/framework-contracts.d.ts`, while the browser receives delivered descriptors through `LongtailForge.viewSurfaceDescriptor.normalize(unknown)`. The adapter is injected before page assets, rejects renamed or malformed supported fields with a descriptor path, and is exposed to the renderer through `LongtailForge.view.normalizeSurfaceDescriptor(...)`. Keep descriptor reads behind that one boundary; do not opt the large builder or renderer into whole-file checking, loosen the manifest validator to an index signature, or add module-local descriptor parsers. The inventory floor is 99 files.

As of 0.33.32.23, the inventory adds `src/routes/search.routes.js` and raises the floor from 99 to 100 files. Treat `request.query` as an untrusted Express value: normalize only absent strings or repeated string values, reject nested shapes through the established Search filter error, and keep page/cursor offsets within the declared bound. Search routes may shape already-pruned results, but must keep target/filter composition in the Search service and must test workspace and declared read permission before visible-offset accounting, browser shaping, tag reads, or Client/Project enrichment.

As of 0.33.32.25, the inventory adds `src/services/sessions.service.js` and raises the floor from 100 to 101 files. Credential-lifecycle consumers must use the revocation input/result types inferred from that owning service rather than redeclaring or casting its callable signature. Exception-scoped revocation requires an explicit `preservedSessionId` and fails closed when it is absent; full-user revocation remains a separate service operation. Stored session-list rows and permission assignment/override decisions use named projections at their owners so checked credential and authorization consumers do not regain wildcard inputs.

As of 0.33.32.26, background worker catches must pass unknown failures through the runner-owned `summarizeJobError` boundary before logging or persistence. Do not read `.message` directly, stringify a rejected object, or widen `JobWorkerLogger.warn` beyond one string. The safe summary accepts string or guarded string-message input, normalizes whitespace, caps length, and uses generic fallback text for every other shape; module handlers retain their existing `{ job, payload }` envelope and retry/dead-letter ownership.

As of 0.33.32.27, server database consumers import adapter, transaction, row, dialect, and binding types from `src/types/database-contracts.d.ts`, never the browser-consumed framework declarations. Treat a generic `DatabaseRow` as unknown-valued: define the selected row projection at the repository/consumer boundary and keep `db.get(...)` nullable instead of reading fields through `any`. Repository entry points that open an atomic unit require `DatabaseAdapter`; helpers inside a callback accept `TransactionClient` and cannot open a nested transaction. Browser scripts and browser declaration aliases must not import or re-export the Node-only database contract. The complete checked inventory floor is 103 files.

As of 0.33.32.28, `parseTimeTrackingEdgePayload(schema, unknown)` returns the inferred output of that exact Zod schema. A source module may construct one sourced-timer payload through Time Tracking's public service boundary, where `ActiveTimerSourcedSaveSchema` parses it once; do not pre-cast the payload, bypass that boundary, or re-parse the trusted Time Entry assembled during finalization. Sourced metadata remains an opaque object owned by the source module. Timer billable handling must use `normalizeTimeEntryBillable()` before workspace capability enforcement so boolean `false` and `"no"` remain non-billable and Personal/Family writes remain forced to `"no"`. The complete checked inventory floor is 104 files.

As of 0.33.32.33, the inventory adds `src/services/support-view.service.js` and raises the floor from 104 to 105 files. `RequestSession` is a normal-or-Support-View union: code must narrow the required `support_view` discriminant before reading actor/effective identity and must not assertion-cast a normal session into that shape. Support View service and repository seams use named nullable rows, closed event/outcome vocabularies, and `TransactionClient` inside atomic callbacks. Preserve generic target denial, effective permission evaluation, current-password admission, session rotation, audit safety, and public-demo refusal when extending this boundary.

As of 0.33.32.34, the inventory adds `src/services/users.service.js` and raises the floor from 105 to 106 files. User lifecycle code consumes `UsersRequestSession` plus the named contracts in `src/types/users-service-contracts.d.ts`; nullable identity rows must be narrowed before credential/session work, and membership/workspace reads retain explicit selected-column projections even while their repository awaits its later checked pass. Keep type-only changes subordinate to the established Users rules for protected identities, public demo, role assignments, owner transfer/blocking, last-membership recovery or retirement, Personal fallback, workspace creation, password revocation, and audit/security events.

As of 0.33.32.35, bounded clean-file pass `18.1` names the two-file `framework-read-delivery-services` tier and raises the floor from 106 to 108 files. Help and static delivery consume the explicit types in `src/types/help-static-contracts.d.ts`: Help authors retain validated section/article declarations, hydrated Markdown bodies, filtered navigation, nullable article-section detail, and Search documents; static delivery retains resolved-or-denied path, protected-view, response, and initial-theme variants. Keep Help discovery behind `modulesService.listActiveHelpContributions(...)`, keep Markdown paths inside the Help root, and keep protected page eligibility, `Cache-Control: no-store`, versioned asset decoration, and first-paint theme injection in the framework static service. These types describe trusted post-validation data and do not replace manifest validation or authorize content, route, permission, capability, caching, or asset-resolution changes.
...
`normalizeTimeEntry()` is the canonical application-record boundary for Time Tracking consumers. Import its `TimeEntry` JSDoc type rather than restating the record: duration seconds and hours intentionally remain strings, billable is `"yes" | "no" | ""`, and invoice status is the exact persisted union. `normalizeTimeEntryBillable()` is the shared trusted-value bridge when an internal caller must convert boolean or string billable intent before constructing that record; an empty result still leaves the owning workflow responsible for its default. Convert duration strings to numbers only at a persistence or calculation boundary that owns the numeric semantics. The checked timezone utility accepts its documented date/fallback unions, returns ISO text, and restricts local-date bounds to `"start" | "end"`. `normalizeUtcIso()` treats explicit `Z`/offset input as an absolute instant, interprets timezone-free ISO input in the supplied source timezone, uses the supplied fallback or call time for missing/invalid date input, chooses the earlier repeated-hour occurrence, maps a nonexistent local time backward using the post-transition offset, and still throws for a valid timezone-free value paired with an invalid named timezone. Runtime session values must pass through `normalizeTimezone()` before reaching calendar helpers; do not weaken the low-level invalid-zone signal.
