...
As of 0.33.32.9, the binding implementation is also a checked trusted-code seam. Its normalized parameter states and named binding entries are discriminated unions: checked code must narrow `kind` before accessing array/map values and narrow `isArray` before accessing scalar `placeholder`/`value` or array `placeholders`. This compile-time protection complements the scanner and existing runtime validation; it does not change the audit baseline or permit new dynamic SQL composition.
