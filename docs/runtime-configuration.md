...
Express 5.2.1 is the supported HTTP runtime baseline. `createApp()` explicitly retains the prior extended query-parser shape for nested and repeated query values, uses root-inclusive named wildcard syntax for the protected browser fallback, preserves the established middleware/router/error-handler order, and continues to use the shared bounded JSON and multipart body readers. As of 0.33.32.31, the Search route pins this setting directly because its query guards accept repeated scalar arrays and reject bracketed nested objects; changing parser mode is therefore a reviewed Search contract change, not a local Express cleanup. This migration adds no runtime environment variable and does not broaden proxy trust, static-file reach, request-body limits, or public API behavior.
...
As of 0.33.28.5, the checksummed allowlisted tarball described in [Runtime Artifact](runtime-artifact.md) is the controlled Docker image payload and a development/testing artifact, not a supported production installer. It carries this configuration contract and `.env.example`, but never a real `.env`, secret, database, uploaded file, log, cache, or other installation state. Direct extraction and `npm ci --omit=dev` remain available for development and advanced experimentation; supported production configuration is supplied to the Compose container through the protected external environment contract.

As of 0.33.28.5, [Compose Production Support](preview-deployment.md) is the sole supported production/self-hosted shape; staged Node release material remains only for development, testing, and advanced experimentation. Compose variables such as image digest, loopback host port, volume name, network range, and backup directory configure orchestration; they do not become application settings. Runtime secrets and application variables still come from a protected external environment file, while the container fixes database and Files paths to its one local persistent data volume.

As of 0.33.17.5, maintained immutable deployments may also provide `LONGTAIL_RELEASE_COMMIT` as an exact 40-character hexadecimal Git commit and `LONGTAIL_RELEASE_ARTIFACT_SHA256` as the exact 64-character artifact checksum. They are release identity, not user settings or secrets. `/api/app-info` returns them so operators can compare the running process to selected release metadata; ordinary local runs report `null`. The Compose release environment is generated from verified immutable release metadata and supplies these values to the container.
...
As of 0.33.31.4, [Development and Demo Data](development-and-demo-data.md) owns two explicit role-fixture modes. The local mode requires `LONGTAIL_ENV=development`, no release branch, an empty or loopback public URL, a marked contained data directory, and a version 1 file with seven private role values. The additional `public-demo` mode is accepted only for sanitized-demo with the exact `rt-ltf-demo` target/origin binding and a version 2 file containing only the private Super Administrator value; six deterministic public visitor values come from reviewed source and cannot be supplied or overridden by environment input. Neither mode runs from normal startup.

As of 0.33.31.7, [Public Demo Compose Reset](demo-data-operations.md) owns the operator-only data contract for the named `rt-ltf-demo` installation. A root-run one-off container under the exact production Compose demo profile mounts the separate root-owned `0600` version 2 role document read-only, gives the repository seed only a minimal development/SQLite environment plus that protected path, and builds under the canonical `/var/lib/longtail-forge` volume without opening or replacing the active database/Files. The six public visitor credentials remain source-owned fixture data valid only under the exact binding, while the protected document contains only the private operator password. The dedicated root-owned reset helper shares one exclusive host lock with Compose deploy and rollback, asserts maintenance, proves app and optional worker stopped, creates and inspects a whole-unit backup, then promotes the validated database, Files, marker, and stopped WAL/SHM state as one same-filesystem operation. It restarts the recorded release and requires direct/public health, readiness, exact app identity, active baseline identity, old-session rejection, and representative role read/write proof; any failure automatically reconstructs and verifies the retained prior unit or stays curtained with evidence. Neither command runs from app/worker startup, ordinary deployment, Nightly, or an in-process timer, and the earlier systemd application helper is historical. As of 0.33.31.8, the exact demo host has a separate root-owned systemd timer that schedules only the Compose reset operation at the top of every UTC hour. Its `LTF_PUBLIC_DEMO_RESET_ENABLED` value is a host-only scheduler control, not an Admin setting, app environment value, database setting, or capability available to other installations. Missed boundaries are skipped, all triggers share the Compose operation lock, and bounded redacted evidence plus a required alert-helper invocation own failure visibility. The operator runbook owns the full installation and recovery contract.
...
As of 0.33.28.5, the constrained Compose SSH deployment helper is tracked with an explicit LF-only Git attribute and release coverage requires that checkout policy. This preserves the host helper's shebang and Bash syntax across Windows development checkouts without changing runtime environment variables or its metadata-only, root-owned privilege boundary. Its separate root-owned, non-writable `/etc/longtail-forge/compose-deploy-helper.env` contains only allowlisted helper settings, including the exact public origin, Compose paths, protected backup destination, optional Secure Notes recovery-key backup path, and immutable GHCR repository; it never contains application secrets or the host's pull-only registry credential.
...
| `LONGTAIL_PUBLIC_DEMO_PERIMETER_WINDOW_SECONDS` | `60` | Fixed-window duration for the demo-only request, mutation, and Search buckets. 1-3,600 seconds; rejected outside demo mode. |
| `LONGTAIL_PUBLIC_DEMO_GLOBAL_REQUEST_LIMIT` | `2400` | All non-probe requests admitted per window across the one demo process. 1-100,000; cannot be disabled. |
| `LONGTAIL_PUBLIC_DEMO_CLIENT_REQUEST_LIMIT` | `600` | Non-probe requests admitted per trusted client-IP bucket per window. Must not exceed the global limit. |
| `LONGTAIL_PUBLIC_DEMO_MUTATION_LIMIT` | `120` | Mutations admitted per hashed session bucket per window; login always uses trusted client IP. Must not exceed the client limit. |
| `LONGTAIL_PUBLIC_DEMO_SEARCH_LIMIT` | `60` | Search reads admitted per hashed session bucket per window. Must not exceed the client limit. |
| `LONGTAIL_PUBLIC_DEMO_MAX_BODY_BYTES` | `131072` | Demo application declaration ceiling before route parsing. 1 KiB-8 MiB. Streaming parsers and the 8 MB Caddy ceiling remain independent backstops. |
...
| `LONGTAIL_SANITIZED_DEMO_ROLE_CREDENTIALS_FILE` | `./.local/sanitized-demo-role-credentials.json` | Explicit seed/journey-only credential path. Local fixture mode requires the version 1 seven-private-password contract. Exact-bound public-demo mode requires the version 2 operator-only contract and supplies six public visitor values from source. The file must be a protected regular non-symlink and is refused outside explicit local seed/journey tooling; it is not normal application configuration or diagnostics. |
...
| `disabled` | `accounts.shared_identity_mutation`, `administration.accounts`, `administration.extensions`, `administration.installation`, `administration.integrations`, `administration.invitations`, `administration.role_management`, `administration.workspace_lifecycle`, `api_keys`, `backups.installation`, `backups.workspace`, `exports.account`, `exports.audit`, `exports.workspace`, `files.ingress`, `imports.workspace`, `outbound.analytics`, `outbound.email`, `outbound.feedback`, `outbound.integrations`, `outbound.interest_capture`, `outbound.url_fetch`, `outbound.webhooks`, `private_feeds`, `restores.installation`, `restores.workspace`, `secure_notes.catalog_security`, `secure_notes.recovery`, `support_view` |

As of 0.33.31.13, public-demo outbound and infrastructure isolation expands that disabled family with `outbound.analytics`, `outbound.feedback`, `outbound.integrations`, and `outbound.interest_capture`. Email, webhook, and URL-fetch remain disabled. These are currently absent server surfaces, not placeholders. Demo mode accepts only reviewed application configuration names and rejects S3/provider credentials, unsafe overrides, known outbound-provider environment, and future undeclared `LONGTAIL_*` settings. Normal mode does not apply that allowlist.

The complete redacted exact-host profile is [public-demo-compose.env.example](public-demo-compose.env.example). It is an operator template, not another defaults file: `DEMO_MODE=false` remains authoritative for development, Friends-and-Family Preview, ordinary self-hosted production, and future SaaS/default configuration. Analytics, feedback, newsletter, mailing-list, and other interest-capture settings remain absent and are refused in the exact demo.

The root-owned `longtail-forge-public-demo-isolation` host helper is the matching infrastructure authority. It requires exact dedicated Docker network/volume identities, a non-masquerading IPv4 bridge, a loopback-only resolver, the realized `127.0.0.1:8001` publication, and first-position host-input/forwarding chains. Both chains permit established/related return traffic for the loopback Caddy request path; forwarding rejects every new connection, and host input additionally permits only the ClamAV gateway/port. The helper accepts the Dockerfile-owned `NODE_ENV=production` key while retaining the narrower application/provider allowlist, skips only the formatting template's empty terminal line, and still rejects every other undeclared container key. It also requires one application network peer, disabled Docker auto-restart, the fixed SQLite/local-storage/inline-worker profile, exact persistent mounts, bounded local container logs, failed external name resolution, and live denied host/public connection probes. Protected deploy and hourly-reset helpers enforce and recheck it; a reboot leaves the exact demo stopped until a protected lifecycle operation restores isolation before startup. This is never an application setting or a general Compose behavior.

As of 0.33.31.9, `files.ingress` denies every Files creation path before JSON or multipart parsing, including single/batch uploads, streamed bytes, direct service callers, and attach-existing relinking. The browser app shell exposes only a safe availability boolean: the shared attachment panel omits its chooser/drop/upload controls and the temporary File quick action is absent, while authorized seeded attachment listing, preview, content, and download continue through ordinary permission, scanner, and safe-preview checks. As of 0.33.31.14.5, routed downloads reuse the first attachment the session can already read and preserve that attachment's Client/Project scope for the `files.download` check and safe lifecycle/audit attribution, so scoped visitor roles retain the same authorized seeded-file access proven by preview. This adds no environment variable or Admin setting and changes nothing when `DEMO_MODE=false`, including local `127.0.0.1` development.

As of 0.33.31.11, exact marked public visitors also pass through fixed server-owned durable growth, JSON-input, and query-shape budgets before authenticated routes execute. The thresholds are reviewed application constants rather than environment variables or Admin settings; runtime diagnostics expose their fixed values and catalog size but no live usage. Successful mutation units persist in SQLite until the external hourly baseline replacement, failed responses release reservations, and `DEMO_MODE=false` bypasses the boundary completely, including local `127.0.0.1` development.
...
As of 0.33.31.7, `npm run demo:baseline:candidate -- build|validate|active ...` is the reviewed data verifier inside the immutable Compose image. Build and validate require the exact demo target/profile, canonical container data root, explicit anchor, and mounted protected role document; write only a fresh same-filesystem build child and fixed inactive candidate; and never stop, open, rename, or replace active database/Files state. The root-owned reset helper alone calls the packaged activation primitive after quiescence, then uses `active` to re-prove the promoted unit before restart. See the dedicated operator runbook for the complete activation, recovery, output, and refusal contract.
...
| Variable | Default | Notes |
| --- | --- | --- |
| `LONGTAIL_SUPPORT_VIEW_ENABLED` | `false` | Enables the complete install-level Support View entry, read-only enforcement, end/logout, and audit workflow. Enable it only when operators intentionally need the capability. |
| `LONGTAIL_SUPPORT_VIEW_TTL_SECONDS` | `900` | Maximum Support View lifetime, from 60 through 3600 seconds. A support session also cannot outlive the administrator's underlying browser-session expiry. |

Support View is disabled by default for self-hosted installations. When enabled, only an installation Super Admin with `support_view.enter` can open the administrator entry page, select an active user/workspace using readable labels, reauthenticate with the current password, record a bounded reason/reference, confirm the read-only warning, and start the configured short-lived session. Actor and effective-user identity remain separate, one effective workspace is fixed for the lifetime, ordinary reads use the target's live permissions, and mutations plus protected-secret reads fail closed. A persistent banner names the target, workspace, actor, and remaining time on every protected page; its immediate **End Support View** action rotates back to the actor and restores a safe prior landing. Settings **Log Out** instead ends the support state and expires authentication without creating a replacement actor session.

The separate Support View Audit page is available only to a normal, authorized Super Admin session. It filters append-only lifecycle and action-attempt records by actor, target, workspace, event, outcome, and bounded date range; exports at most 1,000 safe rows per CSV; and prunes ended sessions plus their events after 365 days. Support View itself receives generic not-found for the entry target catalog, audit page/API, and export. Entry, explicit end, logout, expiry, runtime-gate disablement, target deactivation/revocation, or administrator authority loss rotates or invalidates the opaque browser session as appropriate.
...
Support View browser rows remain ordinary actor-owned `sessions` records linked by an opaque non-secret support-session ID. The durable support tables store actor, effective user, effective workspace, bounded reason/reference, start/expiry/end times, request IDs, outcome, and safe event metadata. They never store the browser session credential, password, cookie, request or response body, or secure content. Entry and explicit End replace the browser session ID; automatic expiry or revocation does the same only when the actor still qualifies for a normal active-workspace session. Log Out ends and removes the support session without replacement. The request context keeps `user_id` as the immutable actor and exposes `actor_user_id`, `effective_user_id`, and `effective_workspace_id` separately. Workspace switching and nested or recovery-mode entry are rejected.
...
As of 0.33.32.26, scheduled-poll and active-run shutdown failures use the same safe warning summary as persisted handler failures. Warnings are always strings, whitespace-normalized, capped at 1,000 characters, and generic for nullish or unsupported object rejections; raw rejected objects and their payload fields are never passed to the logger. Shutdown still clears the poll timer, awaits the active run, and finishes with truthful stopped/non-running/timer-inactive status.
