...
As of 0.33.5.22.6, this contract records active runtime settings plus future reserved settings. Worker settings, job retention settings, local Files upload storage-provider selection, local storage provider diagnostics, the local storage streaming write contract, single-file multipart uploads, streamed multipart batch uploads, the attachment-helper streamed batch path, and streamed upload error hardening are now active; PostgreSQL, scanner adapter, hosted proxy, and alternate storage-provider settings remain reserved until their roadmap slices wire behavior.
