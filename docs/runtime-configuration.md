...
As of 0.33.26.9, [Development and Demo Data](development-and-demo-data.md) owns the local seed/reset environment boundary. Those commands require `LONGTAIL_ENV=development` and an explicitly marked and contained data directory. The fat development profile retains its unique `SUPER_ADMIN_PASSWORD` operator and reads seven different strong role values from the ignored `LONGTAIL_SANITIZED_DEMO_ROLE_CREDENTIALS_FILE`; sanitized-demo reuses its bootstrap identity for the fixture Super Admin. Role fixtures additionally require no release branch and an empty or loopback public URL. The commands refuse production/release/live/customer targets, clear Secure Notes key variables, never accept passwords in command arguments, and never change the production runtime contract or seed a normal startup.

As of 0.33.26.8, [Demo Host Data Provisioning and Reset](demo-data-operations.md) owns a separate operator-only contract for the named `rt-ltf-demo` installation. Its root-owned helper configuration contains exact host/service/path identity and only the path `LTF_ROLE_CREDENTIALS` to a second root-owned `0600` credential document; neither helper key is an application setting. The helper refuses the wrong host/target/origin before reading either protected source, never prints or copies their values, and gives the candidate seed only a minimal environment containing the protected role-file path. It never runs from app/worker startup or ordinary Nightly deployment and is not a general production seed escape hatch.
...
| `LONGTAIL_SANITIZED_DEMO_ROLE_CREDENTIALS_FILE` | `./.local/sanitized-demo-role-credentials.json` | Pretty development/demo seed and journey-only local path to the exact seven-password JSON contract. It is read only by explicit seed/journey tools, must be a protected regular non-symlink file, and is refused in production, release/deployment, or non-loopback use. It is not loaded into normal application configuration or diagnostics. |
