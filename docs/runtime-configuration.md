...
As of 0.33.5.22.2, this contract records active runtime settings plus future reserved settings. Worker settings, job retention settings, local Files upload storage-provider selection, and local storage provider diagnostics are now active; PostgreSQL, scanner adapter, hosted proxy, and alternate storage-provider settings remain reserved until their roadmap slices wire behavior.
...
As of 0.33.5.22.2, `LONGTAIL_STORAGE_PROVIDER` is consumed by Files upload writes and storage provider diagnostics are active. The only built-in provider is still `local`; unknown providers fail clearly for writes and report unavailable health in diagnostics instead of falling back to local. Existing files continue to read through their stored `files.storage_provider` value, so the setting affects new writes only. S3-compatible providers, streamed upload routes, and scanner adapter selection remain later 0.33.5.22 slices.
