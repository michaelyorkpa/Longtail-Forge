...
As of 0.33.5.23.4, this contract records active runtime settings plus future reserved settings. Worker settings, job retention settings, local Files upload storage-provider selection, local storage provider diagnostics, the local storage streaming write contract, single-file multipart uploads, streamed multipart batch uploads, the attachment-helper streamed batch path, streamed upload error hardening, scanner mode resolution, safe scanner health diagnostics, the optional `clamscan` executable scanner adapter, the optional `clamd` TCP scanner adapter, S3-compatible provider selection/configuration, the mocked S3 object-operation proof, safe S3 diagnostics, and the signed URL boundary plan are now active; PostgreSQL, hosted proxy, Unix-socket scanning, direct-transfer behavior, provider-specific S3 client rollout, and actual signed URL routes remain reserved until their roadmap slices wire behavior. ClamAV setup guidance for Linux, Windows, and macOS is documented in [file-scanner-setup.md](file-scanner-setup.md). The 0.33.5.23.4 parameter-binding branch closeout changes database docs, regression wiring, and version metadata only and does not add active runtime configuration.

Storage Provider and Scanner Runtime branch is complete as of 0.33.5.22.15. The live storage/scanner keys are `LONGTAIL_STORAGE_PROVIDER`, `LONGTAIL_LOCAL_STORAGE_ROOT`, `LONGTAIL_S3_BUCKET`, `LONGTAIL_S3_REGION`, `LONGTAIL_S3_ENDPOINT`, `LONGTAIL_S3_ACCESS_KEY_ID`, `LONGTAIL_S3_SECRET_ACCESS_KEY`, `LONGTAIL_FILE_SCANNER`, `LONGTAIL_CLAMD_HOST`, `LONGTAIL_CLAMD_PORT`, and `LONGTAIL_CLAMSCAN_PATH`. `LONGTAIL_CLAMD_SOCKET` is not active, no runtime key enables direct/presigned S3 upload or download routes, and no provider-specific S3 SDK/client setting exists yet. PostgreSQL settings remain reserved for the 0.40.0 database extraction layer; 0.33.5.23 is SQL parameter-binding migration and does not make PostgreSQL settings live.
...
- `LONGTAIL_STORAGE_PROVIDER` defaults to `local`; selecting `s3` requires the S3 provider settings and a provider-specific client before writes can proceed.
