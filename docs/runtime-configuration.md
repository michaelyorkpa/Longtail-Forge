...
As of 0.33.26.7, [Development and Demo Data](development-and-demo-data.md) owns the local seed/reset environment boundary. Those commands require `LONGTAIL_ENV=development` and an explicitly marked and contained data directory. The ordinary development profile requires a unique `SUPER_ADMIN_PASSWORD`; the explicit local sanitized-demo profile instead reads exactly seven unique strong values from the ignored `LONGTAIL_SANITIZED_DEMO_ROLE_CREDENTIALS_FILE` JSON source. Role fixtures additionally require no release branch and an empty or loopback public URL. The commands refuse production/release/live/customer targets, clear Secure Notes key variables, never accept passwords in command arguments, and never change the production runtime contract or seed a normal startup.
