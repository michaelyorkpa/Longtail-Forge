...
As of 0.33.31.6, [Public Demo Baseline Candidate and Historical Host Reset](demo-data-operations.md) owns the operator-only data contract for the named `rt-ltf-demo` installation. A root-run one-off container under the exact production Compose demo profile mounts the separate root-owned `0600` version 2 role document read-only, gives the repository seed only a minimal development/SQLite environment plus that protected path, and builds under the canonical `/var/lib/longtail-forge` volume without opening or replacing the active database/Files. The six public visitor credentials remain source-owned fixture data valid only under the exact binding, while the protected document contains only the private operator password. Dry run and validation refuse the wrong target/profile/root, reused or partial paths, marker/anchor/version/migration/fingerprint/count/role/scope/session/Files mismatch, protected persistence, symlinks, and plaintext secret material. The completed candidate remains inactive at `.longtail-public-demo-candidate`; activation/recovery and scheduling belong to later dedicated slices. Neither command runs from app/worker startup, ordinary deployment, or Nightly, and the earlier systemd host helper is historical rather than the supported Compose activation path.
...
As of 0.33.31.6, `npm run demo:baseline:candidate -- build|validate ...` is the reviewed non-activating data command inside the immutable Compose image. It requires the exact demo target/profile, canonical container data root, explicit anchor, and mounted protected role document; writes only a fresh same-filesystem build child and the fixed inactive candidate; and never stops, opens, renames, or replaces active database/Files state. See the dedicated operator runbook for the complete dry-run, build, validation, output, and refusal contract.
