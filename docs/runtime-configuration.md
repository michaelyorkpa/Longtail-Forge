...
The root-owned `longtail-forge-public-demo-isolation` host helper is the matching infrastructure authority. It requires exact dedicated Docker network/volume identities, an internal IPv4 bridge, a loopback-only resolver, first-position default-deny host-input and forwarding chains with only the ClamAV gateway/port allowed, one application network peer, disabled Docker auto-restart, the fixed SQLite/local-storage/inline-worker profile, exact persistent mounts, bounded local container logs, failed external name resolution, and live denied host/public connection probes. Protected deploy and hourly-reset helpers enforce and recheck it; a reboot leaves the exact demo stopped until a protected lifecycle operation restores isolation before startup. This is never an application setting or a general Compose behavior.
