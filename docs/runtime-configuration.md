...
As of 0.33.31.4, [Demo Host Data Provisioning and Reset](demo-data-operations.md) owns a separate operator-only contract for the named `rt-ltf-demo` installation. Its root-owned helper configuration contains exact host/service/path identity and only the path `LTF_ROLE_CREDENTIALS` to a second root-owned `0600` version 2 document containing the private operator password; neither helper key is an application setting. The helper refuses the wrong host/target/origin before reading either protected source, never prints or copies the private value, and gives the candidate seed only a minimal environment containing the protected role-file path. The six public visitor credentials are source-owned fixture data valid only under that exact binding. The helper never runs from app/worker startup or ordinary Nightly deployment and is not a general production seed escape hatch.
