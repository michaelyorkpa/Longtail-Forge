...
As of 0.33.5.22.11, this contract records active runtime settings plus future reserved settings. Worker settings, job retention settings, local Files upload storage-provider selection, local storage provider diagnostics, the local storage streaming write contract, single-file multipart uploads, streamed multipart batch uploads, the attachment-helper streamed batch path, streamed upload error hardening, scanner mode resolution, safe scanner health diagnostics, the optional `clamscan` executable scanner adapter, and the optional `clamd` TCP scanner adapter are now active; PostgreSQL, hosted proxy, Unix-socket scanning, and alternate storage-provider settings remain reserved until their roadmap slices wire behavior. ClamAV setup guidance for Linux, Windows, and macOS is documented in [file-scanner-setup.md](file-scanner-setup.md).
...
| File storage | `LONGTAIL_STORAGE_PROVIDER`, `LONGTAIL_LOCAL_STORAGE_ROOT` | 0.33.5.22 storage provider runtime. `LONGTAIL_STORAGE_PROVIDER=local` is consumed for new Files upload writes and diagnostics. `LONGTAIL_LOCAL_STORAGE_ROOT` sets the local provider root and is shown in diagnostics only as a safe app-root/data-root relative or redacted label. The local provider supports buffered `save()` writes, streamed `saveStream()` writes, single-file multipart uploads, streamed multipart batch uploads through the shared attachment helper, and partial-file cleanup for failed local streamed writes where practical. Alternate providers remain later slices. |
...
Reserved settings may appear in `config` for readout consistency, but this slice does not implement PostgreSQL, alternate storage providers beyond the registered local provider, hosted proxy behavior, Unix-socket scanning, or runtime settings editing.
