...
| Variable | Default | Notes |
| --- | --- | --- |
| `LONGTAIL_SUPPORT_VIEW_ENABLED` | `false` | Enables the install-level Support View session contract. Keep it false unless the full 0.33.30 server read-only gate and administrator UX are present and intentionally configured. |
...
Support View is disabled by default for self-hosted installations. Version 0.33.30.2 adds the internal server read-only gate but still does not expose a browser entry route or UI. Starting the lifecycle requires the dedicated `support_view.enter` permission, which is seeded only for the installation `super_admin` role, plus current-password reauthentication through the shared trusted-IP/account throttle. Actor and effective-user identity remain separate, one effective workspace is fixed for the lifetime, ordinary reads use the target's live permissions, and mutations plus protected-secret reads fail closed. Entry, explicit exit, expiry, runtime-gate disablement, target deactivation/revocation, or administrator authority loss rotates or invalidates the opaque browser session as appropriate.
