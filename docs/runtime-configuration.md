...
As of 0.33.5.22.3, this contract records active runtime settings plus future reserved settings. Worker settings, job retention settings, local Files upload storage-provider selection, local storage provider diagnostics, and the local storage streaming write contract are now active; PostgreSQL, scanner adapter, hosted proxy, and alternate storage-provider settings remain reserved until their roadmap slices wire behavior.
...
As of 0.33.5.22.3, `LONGTAIL_STORAGE_PROVIDER` is consumed by Files upload writes and storage provider diagnostics are active, and the storage adapter contract supports streamed local writes through `saveStream(readable, options)`. The only built-in provider is still `local`; unknown providers fail clearly for writes and report unavailable health in diagnostics instead of falling back to local. Existing files continue to read through their stored `files.storage_provider` value, so the setting affects new writes only. The current browser upload route remains base64-in-JSON until the multipart route slice. S3-compatible providers, upload UI changes, and scanner adapter selection remain later 0.33.5.22 slices.
...
| File storage | `LONGTAIL_STORAGE_PROVIDER`, `LONGTAIL_LOCAL_STORAGE_ROOT` | 0.33.5.22 storage provider runtime. `LONGTAIL_STORAGE_PROVIDER=local` is consumed for new Files upload writes and diagnostics. `LONGTAIL_LOCAL_STORAGE_ROOT` sets the local provider root and is shown in diagnostics only as a safe app-root/data-root relative or redacted label. The local provider supports buffered `save()` and streamed `saveStream()` writes. Alternate providers remain later slices. |
