...
As of 0.33.5.22.4, this contract records active runtime settings plus future reserved settings. Worker settings, job retention settings, local Files upload storage-provider selection, local storage provider diagnostics, the local storage streaming write contract, and the first single-file multipart upload route are now active; PostgreSQL, scanner adapter, hosted proxy, and alternate storage-provider settings remain reserved until their roadmap slices wire behavior.
...
As of 0.33.5.22.4, `LONGTAIL_STORAGE_PROVIDER` is consumed by Files upload writes and storage provider diagnostics are active, the storage adapter contract supports streamed local writes through `saveStream(readable, options)`, and `POST /api/files/upload` accepts one multipart file in local/self-hosted mode. The multipart upload route uses Busboy, expects `moduleId`, `targetType`, and `targetId` metadata fields before the `file` part in this first slice, enforces upload size while streaming, and then uses the same file-row, `file.scan` job, and attachment lifecycle as `POST /api/files`. The existing JSON/base64 route remains available. The only built-in provider is still `local`; unknown providers fail clearly for writes and report unavailable health in diagnostics instead of falling back to local. Existing files continue to read through their stored `files.storage_provider` value, so the setting affects new writes only. S3-compatible providers, upload UI changes, streamed batch uploads, and scanner adapter selection remain later 0.33.5.22 slices.
