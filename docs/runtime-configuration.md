...
As of 0.33.28.1, [Compose Production Support and Bare-Metal Transition](preview-deployment.md) makes Compose the sole supported production/self-hosted shape; the staged-release material remains only for development, testing, advanced experimentation, and protection of the two existing hosts during cutover. Compose variables such as image digest/tag, loopback host port, volume name, network range, and backup directory configure orchestration; they do not become application settings. Runtime secrets and application variables still come from a protected external environment file, while the container fixes database and Files paths to its one local persistent data volume.
