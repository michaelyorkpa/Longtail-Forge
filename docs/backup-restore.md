...
Restore normalizes the promoted Files root to owner-only mode `0700` on POSIX so the recovered installation can satisfy startup readiness. The standalone restore CLI does not guess an installation's runtime account; an operator running it directly for development or experimentation must restore the reviewed ownership and private modes before startup. Docker Compose is the sole supported production/self-hosted recovery path; its volume and helper contract return the complete data tree to UID/GID 10001 and keep the SQLite database plus WAL/SHM sidecars private. Follow the complete [Compose backup-first upgrade and rollback procedure](preview-deployment.md#docker-backup-first-upgrade-and-rollback).
...
As of 0.33.27.6, the drill also snapshots a legacy UUIDv4 Client and File alongside UUIDv7 Project, Note, and audit records. It proves their relationship IDs, protected Files object key and path, record URL, and embedded audit JSON are unchanged after restore, then requires both `PRAGMA integrity_check` and `PRAGMA foreign_key_check` to pass. Whole-instance recovery never normalizes identifier versions or rewrites references.

As of 0.33.32.32, the drill's destructive tamper fixture uses the same local-archive command boundary as production backup creation: tar receives only the archive basename from the archive directory, while the temporary staging directory is a relative forward-slash operand. This keeps `npm run backup:drill` equivalent under native PowerShell/Windows tar, Git Bash/GNU tar, and Linux without changing the archive layout or relaxing traversal, checksum, manifest, restore, or rollback validation.

The named demo-host provision/reset operation reuses this exact whole-instance creator and inspector before replacing any demo data. It then stages and promotes the fictional database plus Files tree together and retains both the verified archive and the prior data root through observation. That special operation is documented in [Demo Host Data Provisioning and Reset](demo-data-operations.md); it does not create a second backup format or authorize seeding any preview/customer installation.
