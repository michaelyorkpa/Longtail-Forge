...
Restore normalizes the promoted Files root to owner-only mode `0700` on POSIX so the recovered installation can satisfy production startup readiness. When the transition-only root-owned bare-metal deployment helper invokes this restore on either existing preview host, it must also return the complete data root to the dedicated application account before startup and keep the SQLite database plus WAL/SHM sidecars at mode `0600`. The standalone restore CLI does not guess an installation's service account; an operator running it directly must restore the reviewed ownership and private modes required by that installation before starting the service. Docker Compose is the sole supported production/self-hosted recovery path; follow the complete [Compose backup-first upgrade and rollback procedure](preview-deployment.md#docker-backup-first-upgrade-and-rollback).
