...
Only on the exact `rt-ltf-demo` host, install the same reviewed release's `scripts/release/longtail-forge-public-demo-reset-host.example` as `/usr/local/sbin/longtail-forge-public-demo-reset`, root-owned and non-writable. It consumes that same protected helper environment plus the optional root-only `LTF_DEMO_ROLE_CREDENTIALS` path, and shares `/var/lib/longtail-forge-deploy/compose-operation.lock` with deploy and rollback so two lifecycle promoters cannot overlap. A root operator invokes its exact-confirmation `reset` action; it is not available to the constrained deployment SSH account, app/worker startup, or an in-process timer. External hourly scheduling remains a separate reviewed operation.
