...
Only on the exact `rt-ltf-demo` host, install the same reviewed release's reset, scheduler, and alert helpers as `/usr/local/sbin/longtail-forge-public-demo-reset`, `/usr/local/sbin/longtail-forge-public-demo-reset-scheduler`, and `/usr/local/sbin/longtail-forge-public-demo-reset-alert`, all root-owned and non-writable. Install the reviewed oneshot service and timer under `/etc/systemd/system/`, and the root-only scheduler environment example as `/etc/longtail-forge/public-demo-reset-scheduler.env`. The reset consumes the protected Compose helper environment plus the optional root-only `LTF_DEMO_ROLE_CREDENTIALS` path and shares `/var/lib/longtail-forge-deploy/compose-operation.lock` with deploy and rollback so two lifecycle promoters cannot overlap. None of these boundaries is available to the constrained deployment SSH account, app/worker startup, or an in-process timer.
