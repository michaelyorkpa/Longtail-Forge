...
This document is the public-edge operational-security contract. The sole supported Compose production path and controlled runtime-artifact payload are defined in [Compose Production Support](preview-deployment.md), and the tested whole-instance recovery path is in [Baseline Backup and Restore](backup-restore.md). Native Docker acceptance, CI, release operations, and the manual review in [Operational Security](operational-security.md#manual-security-review-before-invitations) remain prerequisites; do not send friends-and-family invitations until they pass.
...
The supported Compose procedure is defined in [Compose Production Support](preview-deployment.md), and the backup/restore commands it consumes are defined in [Baseline Backup and Restore](backup-restore.md). Upgrades remain maintainer-operated and invitations remain blocked on the other preview gates:
...
2. Keep the currently running digest and data volume untouched. Verify the selected protected-`main` schema-2 release metadata, GHCR image-index digest, native `linux/amd64` proof, runtime-artifact checksum, and attached attestations before dispatch. Do not rebuild or relabel the image on the host.
3. For a manual hold, assert the operator marker and confirm an ordinary public request receives the reviewed curtain while all three diagnostics still report the current container. The constrained Compose helper may instead assert its independent deployment marker. Keep both public and private proxies running; the helper stops only the Compose app, creates and inspects the stopped-app backup, selects the immutable digest, and restarts the container behind the curtain.
...
5. If manual verification fails, leave the operator hold active and stop the candidate. The Compose helper restores the recorded database and Files backup together, reselects the prior digest, and verifies the exact known-good identity directly and publicly before clearing only the deployment marker. Failed restore/recovery or interruption remains curtained with protected evidence and recovery artifacts. Never mix an old database with new Files data, reverse applied migrations by hand, or clear a deployment hold merely because the workflow exited.
