...
This document is the public-edge operational-security contract. The sole supported Compose production path, controlled runtime-artifact payload, and bounded bare-metal transition safety are defined in [Compose Production Support and Bare-Metal Transition](preview-deployment.md), and the tested whole-instance recovery path is in [Baseline Backup and Restore](backup-restore.md). Native Docker acceptance, CI, release operations, and the manual review in [Operational Security](operational-security.md#manual-security-review-before-invitations) remain prerequisites; do not send friends-and-family invitations until they pass.
...
The supported Compose direction and transition-only bare-metal procedures are defined in [Compose Production Support and Bare-Metal Transition](preview-deployment.md), and the backup/restore commands they consume are defined in [Baseline Backup and Restore](backup-restore.md). Upgrades remain maintainer-operated and invitations remain blocked on the other preview gates:
