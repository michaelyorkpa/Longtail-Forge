...
As of 0.33.27.1, UUID-shaped application identity has one framework-owned authority in `src/core/identifiers.js`. `createRecordId()` supplies UUIDv7 for forward-created ordinary persistent records, while `createOpaqueId()` supplies random UUIDv4 for non-secret request, lock, storage, backup, purge-fence, and operator identity. Bearer credentials and cryptographic material stay with their dedicated security helpers. This is an intrinsically framework-wide Two-Module Rule exception, not a module contribution or a secrets service. The exact temporary production bypass inventory is machine-enforced and will shrink through the remaining 0.33.27 conversion slices; existing UUIDv4 data remains valid and explicit timestamp/order fields remain authoritative. The complete classification and storage compatibility contract live in [database.md](database.md#identifier-authority-and-forward-uuidv7-policy).
