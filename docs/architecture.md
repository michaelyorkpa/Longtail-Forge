...
As of 0.33.32.20, the app-shell bootstrap producer is checked against the shared framework-owned `AppShellBootstrap`, while `public/js/shared/app-shell-bootstrap.js` is the checked browser boundary that normalizes an `unknown` response into safe current fields. `navigation.js` remains outside whole-file checking and reads the response only through that adapter. The adapter joins the injected framework preamble after error/recovery handling and before page assets, preserving classic-script execution, workspace-context fallbacks, and the existing navigation recovery path. The complete checked inventory floor is 97 files.
