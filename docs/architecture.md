...
As of 0.33.30.1, Support View is a separate framework-owned identity contract, not a user switch or general impersonation mode. The normal session remains owned by an immutable installation-administrator actor while request context carries a distinct effective user and one effective workspace. A default-off runtime gate, Super-Admin-only `support_view.enter` permission, current-password throttle, active target membership, opaque session rotation, short expiry, and live revocation checks govern the lifecycle. Durable `support_sessions` and append-only `support_view_events` contain only bounded reason/reference, attribution, request IDs, timestamps, outcomes, and allowlisted metadata; credentials, browser session IDs, bodies, and secure content are excluded. No browser entry flow ships in this slice, and the later 0.33.30 server gate remains responsible for permission-shaping reads and authoritative mutation denial before Support View becomes usable.
