...
As of version 0.33.3.4, Longtail Forge has an active first-party module architecture with display-only workspace-aware terminology for framework/module-registry surfaces, a framework-owned search service contract with backend adapter capability detection, canonical search metadata storage, active searchable type discovery, backend-neutral permission-safe search request shaping, formal single-record indexing/removal/re-indexing methods, first module-owned indexers for Tasks, Time Entries, Clients, and Projects, framework-owned Help article indexing with `record_type = help_article` and `source = Help`, service-owned event synchronization for initial module records, rebuild tooling for workspace/module/local app-wide scopes, adapter-owned SQLite FTS repair, SQLite FTS5/indexed-LIKE search behavior behind the adapter boundary, a protected browser `GET /api/search` route that calls the framework search service and returns permission-shaped browser results, a compact authenticated-shell search entry powered by active searchable type declarations, a framework-owned `search.html` results page that reads URL filters and renders permission-safe results, icon-triggered shell controls for search and notifications, a framework-owned Help Center surface/API backed by validated framework and module help declarations plus a baseline framework Help article set, and a Notes first-party module with module-owned schema for notes, links, Library buckets, Library collections, note revisions, detected wiki links, Markdown safety/rendering helpers, a replaceable plain-textarea editor boundary, backend note permissions, Library-aware access-policy helpers, note audit/event declarations, access/import lookup indexes, OneNote/import metadata fields, module-owned Notes service/repository/browser routes, safe note lifecycle events, safe search-document payload preparation, permission-shaped browser API reads/writes, a protected Notes Library UI with list/detail/create/edit/archive/restore/collection workflows, integration closeout coverage for framework permissions/tags/search/files/audit/events/notifications/Help/module lifecycle boundaries, permission-safe Library and collection counts, current-state Notes developer documentation in `docs/notes-module.md`, current-state Notes product Help pages for usage, Library, collections, Library buckets, Archive, Markdown, links, revisions, secure notes, files, and search, 0.33.3.4 import planning in `docs/notes-import-planning.md`, and a secure-note encryption-at-rest foundation whose browser/API responses strip encryption envelope fields, keep secure revision history metadata-only, expose a safe admin health/configuration check, and render locked/safe-error states in the browser.
