...
As of 0.33.27.4, UUID-shaped application identity has one framework-owned authority in `src/core/identifiers.js`. `createRecordId()` supplies UUIDv7 for forward-created ordinary persistent records, while `createOpaqueId()` supplies random UUIDv4 for non-secret request, lock, storage, backup, purge-fence, operator-operation, and temporary-path identity. Framework-created records and every audited server-side Clients/Projects, Tasks, Time Tracking, Notes, and Lists record family use the record operation; every audited framework/operator operational generator uses the opaque operation. One authoritative module service or repository owns each new ID, accepted caller-supplied UUIDv4/UUIDv7 values remain unchanged, and relationship plus explicit timestamp/due-date/sequence/paging contracts remain authoritative. Bearer credentials and cryptographic material stay with their dedicated security helpers. This is an intrinsically framework-wide Two-Module Rule exception, not a module contribution or a secrets service. The exact temporary production bypass inventory is machine-enforced and now contains only the Clients/Projects browser generator assigned to 0.33.27.5. The complete classification and storage compatibility contract live in [database.md](database.md#identifier-authority-and-forward-uuidv7-policy).
