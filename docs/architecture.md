...
As of 0.33.27.3, UUID-shaped application identity has one framework-owned authority in `src/core/identifiers.js`. `createRecordId()` supplies UUIDv7 for forward-created ordinary persistent records, while `createOpaqueId()` supplies random UUIDv4 for non-secret request, lock, storage, backup, purge-fence, operator-operation, and temporary-path identity. Framework-created Workspaces, Users, memberships and assignments, permission and Tag relationships, Notifications and subscriptions, audit events, durable jobs, work-resume state, API-key rows, Files rows, purge tombstones, and recovery audit rows use the record operation; every audited framework/operator operational generator uses the opaque operation. Bearer credentials and cryptographic material stay with their dedicated security helpers. This is an intrinsically framework-wide Two-Module Rule exception, not a module contribution or a secrets service. The exact temporary production bypass inventory is machine-enforced and now contains only the module and Clients/Projects browser generators assigned to 0.33.27.4 and 0.33.27.5; existing UUIDv4 data remains valid and explicit timestamp/order fields remain authoritative. The complete classification and storage compatibility contract live in [database.md](database.md#identifier-authority-and-forward-uuidv7-policy).
