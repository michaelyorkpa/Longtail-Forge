...
As of 0.33.22.9, the framework private-feed store is a workspace collection of independently identified, named, hash-only credentials. Each row is creator-bound and carries one workspace, Client, or Project entitlement scope. The admin collection API requires `workspace_settings.manage`; administrators may list safe metadata and revoke any workspace row, but creation always belongs to the authenticated actor and only that owner may rotate and receive a new one-time URL. Public authentication retains the stable route, constant-time digest check, trusted-IP throttle, and generic rejection while additionally requiring an active owner, membership, workspace, Tasks module, active target, and exact current `tasks.view` scope. Canonical user, membership, role, module, Client, and Project lifecycle paths reconcile invalid rows, with a bounded startup repair and authoritative read-time denial preventing orphan access. Tasks remains the only content provider. Workspace subscriptions dispatch normally; Client/Project credentials deliberately fail closed until the Tasks-owned content filter is added in `0.33.22.9.1`, so framework code still does not query Tasks storage.

As of 0.33.22.8, the framework-owned User Settings host exposes that same lifecycle as a specialized immediate action rather than a generic saved setting. Status reads return lifecycle state only; a raw bearer URL exists in browser memory only after generation or rotation, so losing it requires rotation and rotation or disablement revokes the former URL immediately. The UI remains provider-neutral and read-only. This closes the Two-Module review with authentication, secret lifecycle, and Settings-host anatomy as the explicit framework-wide exception, while Tasks continues to own permission-shaped RFC 5545 content until a second real content consumer establishes a shared contract. OAuth, provider APIs, write-back, and two-way editing remain deferred.
