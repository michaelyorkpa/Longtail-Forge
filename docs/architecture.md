...
As of 0.33.32.31, the protected Search route's plain-object guard explicitly depends on the application-owned Express `extended` query parser configured by `createApp()`. Repeated scalar keys remain arrays, bracketed nested values remain objects rejected by the Search query guard, and configuration drift is pinned beside the live route behavior. The Search service consumes the shared request-session contract, refines it only after proving an active workspace, and gives record-reference normalization a required camelCase result contract while preserving the live snake_case adapter aliases.

As of 0.33.32.36, active Search declaration normalization is an explicit checked producer and the rebuild service consumes that complete declaration plus the canonical camelCase rebuild reference. `src/types/search-rebuild-contracts.d.ts` names rebuild scopes/options, workspace-bound audit identity, progress counts, per-target errors and backend-repair detail, and the aggregate result returned to the durable job handler. Database result rows and unknown indexer failures are narrowed at the service edge; the inventory floor is 109 files. This boundary does not change active-module eligibility, rebuild scope, stale cleanup, Search permissions, canonical rows, FTS/LIKE adapter behavior, job retry/failure semantics, or workspace isolation.
...
As of 0.33.32.15, the checked Jobs chain extends through enqueue, handler registration, persisted claims, execution context, worker state, retries, completion, and dead-letter transitions, while checked event summaries consume the normalized internal-event and declared summary-resolver shapes. The inventory floor is 73 files. Type declarations retain the queue's accepted casing aliases and live handler envelope; the database, worker, module services, and notification service keep their existing ownership and runtime behavior.

As of 0.33.32.16, the checked identity chain extends through authentication login/logout/session/workspace/password operations and API-key management/authentication at a 75-file inventory floor. The services consume the shared request-session and active-key contracts, make post-verification user and workspace-session invariants explicit, and preserve nullable recovery/session/key reads. Repository, security, permissions, module-scope, audit, and public-demo owners retain their existing runtime authority and behavior.

As of 0.33.32.17, the checked permission boundary requires every `PermissionResource` to carry a concrete workspace ID. The permission service narrows a live request session before evaluating that resource, and a small framework constructor owns the existing workspace, Client, Project, and operation projections used by Audit, Search-result, and Search-index routes. The inventory floor is 77 files; role assignments, operation overrides, Support View effective identity, module/resource catalogs, denial events, and route responses keep their existing runtime owners and behavior.

As of 0.33.32.18, bounded clean-file passes are a retained part of the checked-seam architecture rather than an unreviewable bulk pragma rollout. `scripts/typecheck-clean-file-passes.json` records each exact path list and ownership tier before source opt-in; the release guardrail caps every pass at 40 unique paths, prevents overlap, and requires every selected path to remain in the complete checked inventory. The first 12-file framework core leaf-utility pass raises the inventory floor to 89 files with runtime behavior unchanged; files that exposed substantive nullable-record or subprocess-error fallout remain outside this mechanical tier.

As of 0.33.32.38, `src/types/route-contracts.d.ts` supplies the structural request, response, next, and async-handler vocabulary for checked Express adapters without adding runtime TypeScript or depending on ambient Express declarations. The shared authenticated, active-workspace, and API-key async adapters keep rejected-Promise forwarding and add defensive context refinement before handler dispatch. Three exact framework route tiers cover 20 sub-150-line adapters; the checked final error middleware retains unknown thrown values, headers-already-sent delegation, request correlation, safe diagnostics, and path-owned API/browser classification. The checked inventory floor is 132 files. Search, private feeds, Files, the large Users adapter, and Work Resume retain separate owners; Work Resume's nullable post-dismiss result is explicitly deferred to `0.33.32.38.1`.

As of roadmap slice 0.33.32.38.1 (package 0.33.32.44.1), the Work Resume route joins the checked program at the 150-file floor. It consumes the shared active-workspace route adapter, shapes established browser-safe fields from explicitly narrowed records, and relies on an exact non-null dismissal projection. The service preserves workspace/user predicates and handles disappearance between update and reread as the existing generic 404. Candidate filtering, read pruning, ranking, action construction, and lifecycle policy remain service-owned; no route, schema, public API, permission, or Workbench workflow was added.

As of 0.33.32.39, `files.routes.js` consumes the active-workspace route contract and raises the checked inventory floor to 133 files. JSON payloads governed by existing Files Zod schemas remain `unknown` across the route-to-service call; the three legacy open-object mutations reject arrays, scalars, and `null` through the shared object-body reader. Request/response stream structure covers Busboy input plus authenticated download and preview piping without importing runtime TypeScript. Multipart state and ordered partial-result variants are explicit, arbitrary failures cannot leak provider/path copy, and service-owned permissions, quotas, storage, scanning, lifecycle, audit, and response shapes remain unchanged.

As of 0.33.32.41, the checked server program also owns the password-security and application lifecycle cores at a 147-file inventory floor. Password parsing returns an explicit Argon2/PBKDF2 union and proves decoded buffers before verification while retaining every established policy parameter, compatibility bound, dummy-hash, asynchronous-derivation, and timing-safe-comparison behavior. `createApp()` and `startServer()` name the HTTP server, startup-phase event, signal, timer, queue-result, and unknown-failure boundaries. Startup configuration, data paths, storage, scanning, database initialization, queue creation, module startup, listen/readiness timing, inline-worker launch, shutdown registration, middleware/router order, database close, and exit sequencing remain unchanged.

As of 0.33.32.42, `src/core/markdown/markdown.service.js` is checked against Markdown-it's parser, renderer, inline-state, token, options, and environment declarations plus the application-owned render-mode, preference, URL, and safe-output contracts. The inventory floor is 148 files. The boundary narrows nullable/string-or-number token attributes without changing the shared CommonMark/table/task-list/underline syntax, raw-HTML and unsafe-scheme rejection, image opt-in, document/user-authored line behavior, source normalization, plain-text/excerpt output, or Notes, Help, Files, and future module ownership.

As of 0.33.32.19, `npm run typecheck` also runs a separate DOM-only browser program from `tsconfig.public.json`. Its first checked tier is the six shared classic scripts for API requests, error parsing, record helpers, formatting, cached fetches, and page controllers. `src/types/browser-contracts.d.ts` describes their shared `window.LongtailForge` namespace while importing the framework-owned `ApiErrorEnvelope`; `error-contract.js` remains the sole browser envelope parser and `api-client.js` delegates to it. The program deliberately has no Node ambient types, runtime imports, module conversion, bundling, or load-order change, and raises the complete checked inventory floor to 95 files.

As of 0.33.32.30, the API client no longer carries a degraded standalone error fallback. It captures the canonical parser at initialization and fails with fixed safe copy when the parser is absent, while the framework preamble remains responsible for loading `error-contract.js` before page assets. The executable browser contract proves missing/reversed order fails before an API request and normal order preserves safe message, code, request ID, and status without exposing raw response text or server details.

As of 0.33.32.21, the app-shell bootstrap producer is checked against the shared framework-owned `AppShellBootstrap`, while `public/js/shared/app-shell-bootstrap.js` is the checked browser boundary that normalizes an `unknown` response into safe current fields. Declarative list descriptors similarly name their response envelope through `dataSource.recordsKey`; `public/js/shared/view-response-records.js` prefers that declaration and contains the measured compatibility fallback for older/external descriptors. `navigation.js` and `view-renderer.js` remain outside whole-file checking and consume unknown responses only through their checked adapters. Both adapters join the injected framework preamble after error/recovery handling and before page assets, preserving classic-script execution and existing recovery paths. The complete checked inventory floor is 98 files.

As of 0.33.32.22, the declarative browser path has one additional checked seam: `public/js/shared/view-surface-descriptor.js` projects delivered `unknown` data into the precise shared descriptor family before `view-renderer.js` reads it. `view-builder.js` exposes the handoff without becoming a checking target. The server manifest validator and browser projection now share the rendered summary-panel and action-confirmation vocabulary, malformed or renamed supported fields fail with their descriptor path, and explicitly open detail/empty-state payloads remain the only forward-compatible object extensions. The complete checked inventory floor is 99 files.

As of 0.33.32.23, `src/routes/search.routes.js` is checked against explicit permission-safe request, canonical raw-result, and browser-result shapes. The Express query boundary accepts absent strings and repeated string values, rejects nested objects/arrays through the existing generic filter error, and prevents cursors from bypassing the established bounded page range. Raw Search rows remain workspace- and permission-pruned before visible offset accounting; only the readable page is shaped and batch-enriched. Canonical filter composition, target discovery, ranking, backend fallback, and authorization remain with their existing Search, module, and permission owners. The complete checked inventory floor is 100 files.

As of 0.33.32.25, the sessions service is the checked owner of exception-scoped revocation input and result shapes. Password change calls that real service contract directly and supplies one required `preservedSessionId`; a missing value fails closed rather than silently selecting full-user revocation. The sessions repository declares the stored list projection consumed by revocation, and touched permission decisions use explicit assignment and override shapes instead of wildcard parameters. The complete checked inventory floor is 101 files; revocation behavior, permission grants, role ceilings, workspace scoping, Support View identity, and public-demo enforcement remain runtime-authoritative.

As of 0.33.32.26, the checked Jobs runner uses one bounded safe-error summary for persisted handler failures, scheduled poll failures, and active-run failures observed during shutdown. Unknown rejection values are narrowed without `any`: strings and guarded string messages are normalized and capped, while unsupported objects and nullish values become generic text rather than raw logger payloads. The worker logger contract accepts string warnings only; claim, retry, dead-letter, public-demo, handler, and worker-mode behavior is unchanged.

As of 0.33.32.27, server database types live in `src/types/database-contracts.d.ts`, outside the browser-consumed framework declaration graph. The contract imports Node's buffer type explicitly, exposes generic rows as unknown-valued records, and keeps `DatabaseAdapter` lifecycle/transaction authority distinct from callback-scoped `TransactionClient` query/get/run authority. Checked consumers project named rows at their owning boundary before field use. The fully annotated `src/db/provider.js` retains nullable reads, parameter kinds, health/dialect access, compatibility aliases, and runtime exports. Server and browser negative compile fixtures reject unchecked row access, nested transaction creation, and database reachability through shared browser contracts; the checked inventory floor is 103 files.

As of 0.33.32.40, the framework repository tier is fully checked at the 145-file inventory floor. The final 12 repositories are recorded in `scripts/typecheck-repository-passes.json` as three non-overlapping owner passes: credential/session, permission/audit/activity, and workspace lifecycle. Their named projections refine unknown database values only at selected-row boundaries, nullable `get` results remain visible to callers, and helpers participating in atomic work accept `TransactionClient` without gaining adapter lifecycle authority. This pass changes no SQL, schema, permission, workspace scope, uniqueness, or lifecycle semantics.

As of 0.33.32.28, Time Tracking's Zod parser is generic over the supplied schema, so checked consumers retain exact manual, sourced, status, finalize, browser-entry, and public-API payload shapes instead of falling through `any`. Sourced active-timer saves validate once at the module edge and preserve only the declared timer fields plus opaque object metadata; trusted finalization records are not re-parsed. The canonical timer billable primitive recognizes boolean and string intent before workspace capability forces Personal/Family writes non-billable. A negative compile fixture and the executable sourced/Task Timer regression keep that boundary in the 104-file checked inventory.

As of 0.33.32.29, Time Tracking billing owns one checked persisted-session timezone boundary before the shared calendar helpers. It retains supported IANA values and applies the canonical `America/New_York` fallback to missing or unsupported session data; low-level timezone utilities continue to throw when a caller directly supplies an invalid named zone. Billing compares unchanged absolute Time Entry instants against local-calendar bounds converted to UTC, so no historical row is rewritten or reinterpreted. The focused isolated regression exercises the real dashboard service fallback plus current, last, and custom report bounds over both DST transitions without changing project-period, hierarchy, rounding, or rate ownership.
...
As of 0.33.32.33, the refinement is a real normal-or-Support-View request-session union. Normal sessions explicitly exclude actor/effective fields; Support View sessions require the complete actor, target, workspace, and public support-session projection. The checked Support View service carries named nullable eligibility, stored-session, audit, event, membership, and rotation shapes through callback-scoped transactions. The central gate now reaches effective identity only after narrowing `support_view`, with no assertion cast and no change to runtime authorization or lifecycle behavior. The complete checked inventory floor is 105 files.
...
As of 0.33.32.37, `src/types/private-feed-contracts.d.ts` names the framework edge from selected token rows through scope eligibility, the secret-free provider descriptor, lifecycle inputs/results, and public responses. `RequestSession` remains the browser normal-or-Support-View union; a separate `PrivateFeedAuthorizationSession` enters only the shared permission evaluator after successful bearer verification and live eligibility checks. The checked service narrows raw lifecycle JSON and nullable rows, and the checked route requires an active-workspace browser session for management while preserving the single public missing response. The inventory floor is 111 files. Tasks still independently reauthorizes the descriptor and owns all task selection, recurrence, timezone, escaping, and calendar serialization behavior.
...
As of 0.33.32.20, `/api/app-shell/bootstrap` is described by the framework-owned `AppShellBootstrap` type at the checked producer. The browser treats fetched JSON as unknown and passes it through `LongtailForge.appShellBootstrap.normalize()` before navigation reads shell fields. Malformed optional fields become safe empty values, and enabled modules, quick actions, and view surfaces retain the existing workspace-context fallback. The adapter is a framework preamble dependency, not a new module or page workflow.
