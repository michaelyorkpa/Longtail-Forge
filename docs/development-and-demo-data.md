...
As of 0.33.31.4, the separate named-host-only operation for `rt-ltf-demo` reuses the same `sanitized-demo` scenario and role/scope definition but activates an additional public profile: six deterministic public visitor credentials plus one private operator credential from a root-owned, exact-target-bound version 2 document. The host file cannot override or add visitor values, and the public mode is refused without the exact binding. That operation does not relax this local CLI's development-environment and marked-directory refusals. Its reviewed source and runbook are tracked, while generated demo data, Files objects, markers, backups, logs, and the private operator credential remain host-only. See [Demo Host Data Provisioning and Reset](demo-data-operations.md).
