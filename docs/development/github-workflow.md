...
Longtail Forge uses two permanent branches because they answer different questions. `nightly` asks, “Is the current integration state healthy enough to promote?” `main` asks, “Is this exact revision known-good and eligible for a deliberate friends-and-family release?” Keeping those responsibilities separate allows active work to integrate without making every merge a deployment.
...
2. Make one scoped change, run its narrow checks, commit intentionally, and push the branch.
   ...
   On a version-wide checkpoint branch, the checkpoint pull request keeps base `nightly` exactly like any other change — a pull request whose base is the version-wide `agent/*` branch itself runs none of the required checks and does not complete a checkpoint. Put the completed `ROADMAP.md` to `ROADMAP-ARCHIVE.md` handoff in the same pull request as the implementation, as its final bookkeeping commit. Keep `LTF-Checkpoint`, `LTF-Summary`, and `LTF-Docs` contiguous in one final commit-message paragraph. Because roadmap files are ceremony/bookkeeping rather than documentation for the trailer, use exactly `LTF-Docs: No docs change needed: completed checkpoint moved to roadmap archive.` Then run `npm run checkpoint:validate` before the first push and after any commit-message amend. The archive handoff becomes authoritative only when the protected pull request merges; do not create a separate archive-only pull request.
...
`Browser smoke and accessibility` remains an always-present required job. It succeeds through an explicit GitHub-only documentation message without checking out the application, installing npm dependencies or Playwright browsers, or running the smoke/accessibility suite. `Complete maintenance release rehearsal` skips the same GitHub-only documentation class and otherwise installs distribution Nginx/OpenSSL plus a checksum-pinned official Caddy binary, then runs the fail-fast host-asset, direct-Caddy, real-Nginx/private-Caddy, and Compose maintenance/recovery guardrail stages through `npm run maintenance:rehearse`. `Dependency review` remains enabled for every pull request. CodeQL retains PR and scheduled analysis; redundant post-merge push scans are retired only after live protection confirmed the PR context is required on both permanent branches. The workflow deliberately does not use pull-request `paths-ignore`, because an omitted required workflow can leave branch protection waiting forever.
...
A GitHub-only documentation push to `nightly` completes through explicit successful no-runtime jobs. It does not build or upload a runtime artifact.
...
Nightly artifacts are integration and promotion evidence, not a supported production transport. The former direct-artifact demo deployment job and `demo-development` environment handoff are retired. The demo host remains on an explicitly selected immutable `main` Compose release until a future supported development-image lane is designed. Changes under `help/**` remain runtime changes and therefore run the full application/browser validation and artifact-proof path even though no Nightly deployment follows.
...
The `friends-and-family-preview` environment has separate data, application secrets, Secure Notes key, SSH credential, known-hosts pin, URL, and deployment account. The Compose replacement and restored rollback were proven in 0.33.28.4; keep the environment disabled whenever any exact host, credential, URL, scanner, backup, restore, or recovery prerequisite is no longer satisfied. The Compose host helper keeps its exact public origin, GHCR repository, matching maintenance-state root, and optional Secure Notes recovery-key backup path in the separate root-owned `/etc/longtail-forge/compose-deploy-helper.env`; reinstalling the reviewed helper must preserve that file. Root's Docker client holds a separate pull-only package credential that the deployment account and workflow cannot read. Rotate it by adding the replacement, proving one exact digest pull, and revoking the old value; revoke immediately on suspected disclosure and disable deployment until replacement proof passes.

`Manual protected-main image candidate` exists for a bounded live qualification that must compare two real immutable digests before a version can close. It accepts only one exact commit reachable from protected `main`, requires `PUBLISH CANDIDATE <full-sha>`, runs the release gates, publishes the same attested `linux/amd64` SHA-tagged image contract, and retains schema-2 metadata plus the controlled payload for 30 days. It does not create or replace a Git tag or GitHub Release, is not a mutable prerelease channel, and does not authorize deployment, announcement, or invitations.
...
A failed deploy retains root-only operation evidence. Candidate start/readiness or identity failure restores the recorded whole-instance backup and prior digest before verifying that known-good identity. A verified recovery clears only the deployment marker and still reports the candidate deployment as failed. Stop, backup, restore, recovered-current verification, or interruption failure leaves the marker, images, backup/pre-restore artifacts, metadata, and evidence intact for operator review. Do not clear the deployment marker manually to make a failed workflow appear green.
...
After an interrupted or failed rollback, inspect the retained current/previous state, backups, metadata, and operation directory before another dispatch. A mismatched target is refused, and the operator maintenance `off` command cannot remove deployment-owned evidence. Do not delete either image, either backup, sidecars, operation records, or Secure Notes recovery material until the observation period passes.
