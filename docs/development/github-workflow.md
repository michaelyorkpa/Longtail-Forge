...
3. Open a pull request into `nightly`. On GitHub choose **Pull requests > New pull request**, set base to `nightly`, compare to the short-lived branch, and describe the outcome and proof. The required checks are named exactly `Development gate`, `Browser smoke and accessibility`, `Dependency review`, and `CodeQL JavaScript analysis`; all four must report a conclusive successful result. Resolve every review conversation, even when the resolution is “no change” with a documented reason.

The Development workflow first classifies the complete rename-aware pull-request diff from the exact PR base SHA through the proposed head. A narrow docs-only pull request may contain only root-level `*.md` files, ordinary documentation files beneath `docs/`, and the explicitly reviewed non-runtime `LICENSE` file. Checked-in Caddy, Nginx, environment, systemd, YAML, script, SQL, and similar deployment/runtime configuration contracts beneath `docs/` are explicitly excluded from the fast path. Both the old and new paths of a rename must qualify. Changes beneath `.github/`, or changes to package manifests/lockfiles, source, public assets, scripts, tests, deployment configuration, containers, migrations, runtime configuration, or any unknown path always retain the full path. An unavailable or failed classification also falls back to full Development and browser validation.

For a classified docs-only pull request, `Development gate` still installs the dependencies required by the closeout and selected regressions, runs `npm run closeout`, and runs `npm run test:regressions:changed:ci` against that same complete PR diff. It skips application typecheck, unit tests, and lint. `ROADMAP.md`, `ROADMAP-ARCHIVE.md`, and `CHANGELOG.md` retain their existing `release` regression owner with `fullCheck: false`; in this context `release` means release-regression ownership only. A bookkeeping-only change does not require a GitHub Release, deployment, runtime artifact rebuild, application build, or Longtail Forge version bump.

`Browser smoke and accessibility` remains an always-present required job. It succeeds through an explicit docs-only message without checking out the application, installing npm dependencies or Playwright browsers, or running the smoke/accessibility suite. `Dependency review` remains enabled for every pull request, and CodeQL is unchanged. The workflow deliberately does not use pull-request `paths-ignore`, because an omitted required workflow can leave branch protection waiting forever.

For every non-docs-only pull request, the Development gate reports context/setup, closeout, typecheck/unit/lint, and regression stage timings. Changed-area full escalation uses the regression-only path only after fast checks pass in that same job, so it does not repeat them. Nightly, promotion, preview, and release workflows retain their complete regression, permission, browser, and packaging gates and report those stage timings separately.
