...
| `task-focus-exit-capture.spec.mjs` | Desktop/mobile timed Task Focus holds Change Focus and a real app-shell Dashboard link for the shared single-line resume-note capture, proves the Yes write targets the focused Task, and proves No continues without a write |
