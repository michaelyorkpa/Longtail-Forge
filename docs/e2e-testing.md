...
| `login.spec.mjs` | An unauthenticated visit renders only the login form; ordinary remembered-login alignment, keyboard use, explicit boolean submission, required-password transition, and safe preferred landing remain pinned. Public-demo interception additionally proves all six exact role/scope choices, the native select accessible name and keyboard focus order, live guidance/status, selection and credential fill without authentication, ordinary Log In as the sole request, generic failure recovery, responsive desktop/mobile behavior, and complete helper absence with unchanged ordinary focus/autocomplete when the optional catalog is unavailable. |
...
| `support-view.spec.mjs` | Desktop managed-server journey for password-confirmed entry, persistent actor/target/expiry state, axe-clean entry/audit states, dynamic write-control suppression, central mutation and sensitive-read denial, immediate exit/focus restoration, and retained safe audit review |
...
| `client-projects-restricted-descriptor.spec.mjs` | Desktop/mobile Clients and Projects use permission-restricted server capability fixtures to prove both delivered descriptors render without unavailable top-level actions or page errors, while the seeded super admin retains Add Client and Add Project |
