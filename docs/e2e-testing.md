...
| `calendar-subscription-settings.spec.mjs` | Desktop/mobile Admin Calendar coverage pins Settings navigation and permission placement, Workspace-first Client/Project hierarchy, the standalone danger-colored one-time warning, masked URL reveal/copy/clear behavior, multiple safe metadata rows including effective owner timezone, owner-only rotation, active-row revoke-and-remove, revoked-row Delete cleanup and focus recovery, disabled-Tasks recovery, official client guidance, User Settings removal, keyboard use, confirmations, and page/table overflow boundaries |
