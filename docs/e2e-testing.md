...
| `user-settings-appearance.spec.mjs` | User Settings keeps the Light/Auto/Dark control bounded and keyboard-operable, with Auto source independently bounded; rendered desktop/mobile coverage also pins Profile placement, full-width Calendar Subscription/Notifications/Leave/Workspace Creation anatomy, the private feed's masked one-time URL lifecycle and confirmations, official client guidance, initial disclosure state/caret, complete offset-labelled IANA timezone choices, and matching leave-membership warnings |
