...
| `calendar-subscription-settings.spec.mjs` | Desktop/mobile Admin Calendar coverage pins Settings navigation and permission placement, Workspace-first Client/Project hierarchy, one-time masked URL reveal/copy/clear behavior, multiple safe metadata rows, owner-only rotation, administrator revocation and focus return, disabled-Tasks recovery, official client guidance, User Settings removal, keyboard use, confirmations, and page/table overflow boundaries |
