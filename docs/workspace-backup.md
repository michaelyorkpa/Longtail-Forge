...
As of 0.33.27.6, that proof uses legacy UUIDv4 workspace, user, membership, assignment, and Client fixtures with UUIDv7 Project, Note, File, and audit records. The restored package must retain exact primary/foreign keys, Files storage keys and paths, audit URLs, JSON payloads, and version metadata, followed by clean `PRAGMA integrity_check` and `PRAGMA foreign_key_check` results. Workspace recovery does not regenerate or normalize identifiers.
