...
No runtime owner currently has counted literal-helper calls or direct helper-interpolated SQL operation sites. The startup and migration compatibility paths now keep any remaining exceptions to no-value startup maintenance, schema repair scripts, and migration SQL-file execution until the enforcement slices record the final dialect allowlist. New or touched value-bearing reads and writes must use named params; new helper interpolation is not allowed.
