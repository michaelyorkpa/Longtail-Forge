...
`npm run start:worker` starts the optional same-host separate worker from the same installed artifact. The supported Compose contract, native platform/provenance proof, persistence, upgrade/rollback direction, and host Caddy boundary are documented in [Compose Production Support](preview-deployment.md); this artifact does not expose the Node listener directly to the internet.

The artifact also carries the `backup:create`, `backup:inspect`, `backup:export`, and `backup:restore` whole-instance commands, `workspace-backup:inspect` and `workspace-backup:restore`, the explicit `workspace:purge` queue command, the non-activating `demo:baseline:candidate` Compose command, the stopped-volume public-demo activation primitive, and the historical guarded `demo:data:host` implementation. The candidate command reconstructs and validates its matching inactive database-and-Files unit from reviewed source. The activation primitive is callable only by the separately installed root-owned Compose reset helper after it holds the shared lifecycle lock and proves every SQLite user stopped; neither operation runs from app/worker startup or ordinary deployment. The external scheduler, alert, service, timer, and host-only environment examples are separate GitHub Release host assets, not runtime-artifact entrypoints. The redacted exact-demo Compose profile and operator runbook are included in both the artifact and GitHub Release so the immutable payload carries its reviewed operating contract. Demo tooling includes the reviewed deterministic seven-role identity/scope definition, but never the separately installed role credential document or any credential value. Their checksummed formats, Secure Notes key prerequisites, recovery procedures, irreversible deadline/fencing rules, and demo-only boundary are defined in [Baseline Backup and Restore](backup-restore.md), [Workspace Backup Package](workspace-backup.md), [Workspace Deletion Grace Period and Final Purge](workspace-deletion.md), [Public Demo Baseline Candidate and Historical Host Reset](demo-data-operations.md), and [Public Demo Operator Runbook](public-demo-operator-runbook.md). The disposable `backup:drill`, `workspace-backup:drill`, demo operation regressions, and purge regressions remain repository-only test tooling.
...
- Runtime entrypoints: `server.js` and `worker.js`; backup/workspace recovery commands; the inert-by-default non-activating Compose public-demo candidate command; its stopped-volume activation primitive for the root-owned reset helper; and the historical named-demo-host data command plus their shared deterministic builder. No demo operation is called by app/worker startup or normal deployment, and each requires its exact protected operator boundary.
...
- Runtime/operator contract files: `.env.example`, `.nvmrc`, `README.md`, `SECURITY.md`, `LICENSE`, this document, runtime configuration, operational security, preview deployment/Compose environment guidance, the redacted public-demo profile and operator runbook, the reference internet-deployment/Caddy files, and SQLite small-office guidance.
...
For a public release, the tarball and sidecar remain GitHub Release provenance assets because their exact checksum is embedded in the published image and schema-2 release metadata. They are not deployment transport. `npm run image:publish` accepts them only as Docker build inputs, publishes one reviewed `linux/amd64` GHCR image, and binds the protected `main` commit, artifact checksum, image-index digest, platform-manifest digest, native `better-sqlite3` execution, and registry-attached SPDX/SLSA attestations. The supported host pulls the recorded `repository@sha256:...`; it never extracts the tarball into a production application tree.
