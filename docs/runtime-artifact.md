...
- Runtime entrypoints: `server.js` and `worker.js`; backup/workspace recovery commands; the inert-by-default non-activating Compose public-demo candidate command; and the historical named-demo-host data command plus their shared deterministic builder. Neither demo command is called by app/worker startup or normal deployment, and both require exact protected operator configuration.
