...
SQLite small-office mode continues to default to local file storage through `LONGTAIL_STORAGE_PROVIDER=local` and `LONGTAIL_LOCAL_STORAGE_ROOT`. As of 0.33.5.25.1, S3 remains deferred scaffolding: the adapter keeps mocked proof coverage for object save, stream save, read, metadata, delete, and health operations, and `src/config.js` keeps the reserved S3 setting names stable, but `LONGTAIL_STORAGE_PROVIDER=s3` fails app and worker startup until a provider-specific client is wired. Local storage remains the supported self-hosted path. Local-vs-S3 deployment guidance: use `local` for SQLite small-office installs with local or attached storage, and reserve S3 for a later hosted/SaaS-style or explicitly managed object-storage rollout behind the Files adapter. S3 diagnostics and browser payloads must not expose bucket names, endpoints, credential values, storage keys, protected paths, raw provider responses, root locations, or signed URLs. No direct/presigned S3 upload or download route ships in this branch; future signed URL behavior must be permission-checked, expiring, and deliberately route-designed.

As of 0.33.5.25.2, Files workspace and per-user storage quotas are enforced for both buffered compatibility uploads and streamed multipart uploads. `NULL` limits stay unlimited. When limits are set, quota usage counts internal bytes still occupying storage across pending, available, quarantined, and staged-deleted files, so local operators should size quota limits with backups, review queues, and staged-delete retention in mind.

As of 0.33.5.25.3, local Files installs pre-check storage metadata before download and preview-content streaming. If the database still references a file whose local object was moved, deleted, or rotated outside Longtail Forge, the route returns a clean 404 instead of starting an aborted stream. Streamed uploads also reject wrong-type sampled content earlier where practical and await/log cleanup for rejected stored objects.

As of 0.33.5.25.4, local streamed multipart batch uploads keep one malformed file part as a per-file failure when the rest of the request remains valid, so other file parts can still attach. The local storage adapter no longer exposes an unused `quarantine()` method; quarantine is a database lifecycle state only, and stored-object relocation remains future explicit work.
