...
SQLite small-office mode continues to default to local file storage through `LONGTAIL_STORAGE_PROVIDER=local` and `LONGTAIL_LOCAL_STORAGE_ROOT`. As of 0.33.5.22.15, `LONGTAIL_STORAGE_PROVIDER=s3` can select the registered S3-compatible provider, read S3 bucket, region, endpoint, and credential settings from server runtime configuration, and exercise a mocked S3 client proof for object save, stream save, read, metadata, delete, and health operations. Local storage remains the supported self-hosted path; S3 installs still need a provider-specific client rollout before real object storage can be used. Local-vs-S3 deployment guidance: use `local` for SQLite small-office installs with local or attached storage, and reserve `s3` for hosted/SaaS-style deployments or explicit operator-managed object storage where the provider client is installed behind the Files adapter. Missing S3 settings fail clearly as configuration errors, and missing S3 clients fail safely instead of partially writing files. S3 diagnostics expose only provider id and safe availability; diagnostics and browser payloads must not expose bucket names, endpoints, credential values, storage keys, protected paths, raw provider responses, root locations, or signed URLs. No direct/presigned S3 upload or download route ships in this branch; future signed URL behavior must be permission-checked, expiring, and deliberately route-designed.
