...
| Workbench | As of 0.33.6.11b, `views/protected/workbench.html` is a minimal `data-workbench-host` shell and `public/js/workbench.js` builds the guided Workbench page through `LongtailForge.view` primitives while loading focus modes/candidates and contribution source payloads from protected routes. Pick up where I left off consumes existing resume-state candidates first, ranks running timers, paused timers, resume-note tasks, and In Progress tasks ahead of weaker candidates, falls back to ranked candidate buckets only when no resume rows are available, and lets users dismiss resume-backed candidates through the protected resume API. Normalized Task candidates open the registered `tasks.edit` action in place; Note and List focus candidates can open their existing `notes.edit` and `lists.edit` module actions when available; other candidate types keep an explicit temporary page fallback through their safe source URL. Due-focused modes request the shared due-datetime candidate sort, Review blocked work resolves only genuinely blocked candidates, and split client/project focus filters apply exact scopes to every focus mode until the 0.33.6.13 hierarchy standard lands. "More in this focus" starts collapsed, Timers starts open only when active or paused timers exist unless the user toggles it during the current session, and Workbench summaries show a consistent caret/focus affordance while retaining native `<summary>` toggle semantics. The wide layout includes a subordinate right-side Inspector that renders permission-safe focus-candidate titles/context and opens Task, Note, and List titles through existing module actions without adding an embedded viewer pane. The Workbench must not render a full Tasks index, task filter bar, task sort control, task-list actions, `taskItems` browser state, an empty bootstrap `taskItems` compatibility field, Quick Notes section, embedded preview pane, or unsafe raw-ID labels. | Page header, header-slot status state, guided focus questions, split focus filter row, one recommended-action card, useful empty states, collapsed curated overflow list, active timer card, Workbench Inspector, accessible collapsible summaries, inline candidate/timer action rows, dismiss action, badges. | Keep Workbench framework-owned. The browser host owns focus selection, client/project filter visibility and persistence, one-candidate recommendation emphasis, page anatomy, right-side Inspector layout, relocated header status slot, collapsible-section default state, registered module-action dispatch, focus return, dismissal dispatch, raw-ID fallback labels, and temporary page fallback handoff; QAC owns quick note capture; focus/candidate services own deterministic ranking/filtering/sorting, resume-first strategy execution, recurring-created exclusion, and permission/workspace/module scoping; module work-item/timer providers keep record meaning and source routes. |
