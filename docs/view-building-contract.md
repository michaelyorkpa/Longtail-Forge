...
| Workbench | As of 0.33.6.6f, `views/protected/workbench.html` is a minimal `data-workbench-host` shell and `public/js/workbench.js` builds the guided Workbench page through `LongtailForge.view` primitives while loading focus modes/candidates and contribution source payloads from protected routes. Normalized Task candidates open the registered `tasks.edit` action in place instead of navigating away; non-modal candidate types keep an explicit temporary page fallback through their safe source URL. Due-focused modes request the shared due-datetime candidate sort, Review blocked work resolves only genuinely blocked candidates, and split client/project focus filters apply exact scopes to every focus mode until the 0.33.6.13 hierarchy standard lands. "More in this focus" starts collapsed, Timers starts open only when active or paused timers exist unless the user toggles it during the current session, and Workbench summaries show a consistent caret/focus affordance while retaining native `<summary>` toggle semantics. | Page header, header-slot status state, guided focus questions, split focus filter row, recommended-action card, right-aligned icon-only cycling controls, useful empty states, collapsed subordinate overflow/card lists, accessible collapsible summaries, inline item/action rows, chips. | Keep Workbench framework-owned. The browser host owns focus selection, client/project filter visibility and persistence, top-five recommendation cycling, page anatomy, relocated header status slot, collapsible-section default state, registered module-action dispatch, focus return, and temporary page fallback handoff; focus/candidate services own deterministic ranking/filtering/sorting and permission/workspace/module scoping; module work-item/timer providers keep record meaning and source routes. |
