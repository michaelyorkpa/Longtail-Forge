...
| Workbench | As of 0.33.6.6, `views/protected/workbench.html` is a minimal `data-workbench-host` shell and `public/js/workbench.js` builds the guided Workbench page through `LongtailForge.view` primitives while loading focus modes/candidates and contribution source payloads from protected routes. | Page header, status, guided focus questions, recommended-action card, useful empty states, subordinate candidate/card lists, inline item/action rows, chips. | Keep Workbench framework-owned. The browser host owns focus selection and page anatomy; focus/candidate services own deterministic ranking/filtering; module work-item/timer providers keep record meaning and source routes. |
