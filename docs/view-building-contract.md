...
| Workbench | As of 0.33.6.6e, `views/protected/workbench.html` is a minimal `data-workbench-host` shell and `public/js/workbench.js` builds the guided Workbench page through `LongtailForge.view` primitives while loading focus modes/candidates and contribution source payloads from protected routes. Normalized Task candidates open the registered `tasks.edit` action in place instead of navigating away; non-modal candidate types keep an explicit temporary page fallback through their safe source URL. Due-focused modes request the shared due-datetime candidate sort, Review blocked work resolves only genuinely blocked candidates, and split client/project focus filters apply exact scopes to every focus mode until the 0.33.6.13 hierarchy standard lands. | Page header, header-slot status state, guided focus questions, split focus filter row, recommended-action card, right-aligned icon-only cycling controls, useful empty states, subordinate overflow/card lists, inline item/action rows, chips. | Keep Workbench framework-owned. The browser host owns focus selection, client/project filter visibility and persistence, top-five recommendation cycling, page anatomy, relocated header status slot, registered module-action dispatch, focus return, and temporary page fallback handoff; focus/candidate services own deterministic ranking/filtering/sorting and permission/workspace/module scoping; module work-item/timer providers keep record meaning and source routes. |
