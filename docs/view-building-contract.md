...
| Workbench | As of 0.33.6.6b, `views/protected/workbench.html` is a minimal `data-workbench-host` shell and `public/js/workbench.js` builds the guided Workbench page through `LongtailForge.view` primitives while loading focus modes/candidates and contribution source payloads from protected routes. | Page header, header-slot status state, guided focus questions, recommended-action card, right-aligned icon-only cycling controls, useful empty states, subordinate overflow/card lists, inline item/action rows, chips. | Keep Workbench framework-owned. The browser host owns focus selection, top-five recommendation cycling, page anatomy, and the relocated header status slot; focus/candidate services own deterministic ranking/filtering and permission/workspace/module scoping; module work-item/timer providers keep record meaning and source routes. |
