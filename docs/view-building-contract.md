...
| Workbench | As of 0.33.6.6a, `views/protected/workbench.html` is a minimal `data-workbench-host` shell and `public/js/workbench.js` builds the guided Workbench page through `LongtailForge.view` primitives while loading focus modes/candidates and contribution source payloads from protected routes. | Page header, status, guided focus questions, recommended-action card, right-aligned icon-only cycling controls, useful empty states, subordinate overflow/card lists, inline item/action rows, chips. | Keep Workbench framework-owned. The browser host owns focus selection, top-five recommendation cycling, and page anatomy; focus/candidate services own deterministic ranking/filtering and permission/workspace/module scoping; module work-item/timer providers keep record meaning and source routes. |
...
| Reporting | `views/protected/reporting.html` and `public/js/reporting.js` currently own hard-coded report host tables and filters. | Filter panel, data table with overflow, status/empty state. | Future 0.33.8 reporting work should consume the helper boundary when it builds the framework reporting host. |
