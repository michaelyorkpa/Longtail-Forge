...
`public/js/shared/file-attachments.js` now centralizes old/no-helper element fallback in `createAttachmentElement()` while using shared list shell, empty-state, action strip, and action button helpers when available. The strict guardrail allows that single centralized fallback plus Files-owned file reads, base64 upload payloads, accepted categories, attachment refresh, host callbacks, remove/report/review/delete/restore routes, download links, and recovery-state wording. It still forbids turning the shared attachment panel into inline File Context, Preview, Metadata, or Inspector UI.
