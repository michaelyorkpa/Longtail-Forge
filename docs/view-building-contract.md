...
| Workbench | As of 0.33.6.12c-2, `views/protected/workbench.html` is a minimal `data-workbench-host` shell and `public/js/workbench.js` builds the guided Workbench page through `LongtailForge.view` primitives while loading focus modes/candidates and contribution source payloads from protected routes. Workbench has explicit `focus-selection` and `task-focus` browser states exposed through host data attributes and the page-controller snapshot. Focus Selection keeps the current focus questions, split Client/Project filters, one recommended-action card cycling through the top five ranked candidates, active timer card, and a bounded right-side "More in this focus" Inspector overflow list sourced from candidates after the top-five window. Primary actions on normalized Task candidates are labeled `Focus task` and enter Task Focus by recording the active task focus; `Change Focus` exits Task Focus without changing the selected focus mode or filters. Candidate dismissal controls are removed. Focus Selection Inspector title clicks use the same primary candidate path, while non-task primary candidates use an explicit temporary page fallback until their owning module has a Task Focus-equivalent target view. Task Focus shows a selected-task action strip, summary, read-only collapsed Task Details, and a Task Focus Checklist check-only execution surface while hiding Focus Selection panels and candidate overflow; task-linked timer surface and selected-task Inspector context/action behavior remain later 0.33.6.12 slices. The Workbench must not render a full Tasks index, task filter bar, task sort control, task-list actions, `taskItems` browser state, an empty bootstrap `taskItems` compatibility field, Quick Notes section, main-column "More in this focus" section, embedded preview pane, or unsafe raw-ID labels. | Page header, header-slot status state, `Change Focus` state action, guided focus questions, split focus filter row, one recommended-action card, useful empty states, right-side overflow Inspector, active timer card, Task Focus action strip, selected-task summary, read-only collapsed Task Details, Task Focus Checklist check-only section, accessible collapsible summaries, inline candidate/timer action rows, badges. | Keep Workbench framework-owned. The browser host owns focus selection, view-state tracking, active task-focus selection, client/project filter visibility and persistence, top-five recommendation cycling, page anatomy, right-side Inspector overflow layout, Task Focus selected-task shell, relocated header status slot, collapsible-section default state, focus return, raw-ID fallback labels, and temporary page fallback handoff; QAC owns quick note capture; focus/candidate services own deterministic ranking/filtering/sorting, resume-first strategy execution, recurring-created exclusion, and permission/workspace/module scoping; Tasks owns task edit, completion, blocked-status validation, checklist/timer mutation, audit/event/search side effects, and readable task payloads; module work-item/timer providers keep record meaning and source routes. |
