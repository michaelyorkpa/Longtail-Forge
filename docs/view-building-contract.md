...
| Tasks | `views/protected/tasks.html` is now a minimal host for `tasks.workspace`. `public/js/tasks.js` renders the descriptor-backed page shell, mounts a non-collapsible Saved Task Views dropdown panel, mounts a collapsed Sorting and Filters panel, sends selected views through the Tasks-owned `task_view` query contract, and mounts the existing task list through the descriptor `tasks-main-list` detail region, while `public/js/task-dialog.js` owns the reusable task dialog fallback markup. | Page header/status, slide-out filter sidebar, task view selector, collapsed filter controls, main task-list region, data table rows, dense row actions, modal shell/form/footer, field grids, chips. | Descriptor-backed shell/read path shipped in 0.33.5.18.7.1; filter-sidebar anatomy shipped in 0.33.5.18.7.2; task-view query contract shipped in 0.33.5.18.7.3; read-only list binding shipped in 0.33.5.18.7.4. Later Tasks slices should convert bulk actions and modal fields deliberately without moving the task list into the sidebar. |
