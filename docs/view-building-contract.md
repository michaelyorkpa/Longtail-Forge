...
| Admin and Settings | User Settings, Workspace Settings, Tasks Settings, Time Tracking Settings, and Files Settings are minimal `data-settings-host` pages built by `LongtailForge.settingsHost`; API Keys, Audit Log, Notifications, and User Admin retain their existing surface-specific hosts. | Settings page headers, field grids, contribution sections, attachment regions, action/status rows, operational readouts, modal shells; retained admin data tables. | The five Settings hosts use the permission-filtered catalog and shared renderer. Ordinary module settings come from manifest contributions plus optional owner-registered handlers/effects; framework Settings code must not import feature modules, name first-party setting IDs, or invent module-specific branches. Do not claim the broader Admin family is converted. |
