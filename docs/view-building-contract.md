...
As of 0.33.5.15.6, this document defines the first framework-owned view-building boundary, records the inventory that the 0.33.5.15 helper work should address, documents the initial helper implementation, records the first Lists protected-workspace pilot plus Client/Project modal adoption, documents the static guardrails for those converted surfaces, and closes the helper line with current-state module adoption guidance. Updated through 0.33.5.18.15, it also records the current helper and descriptor implementation, the converted Lists, Notes, Tasks, Files browse, Clients, and Projects protected-page surfaces, the strict guardrails for those converted surfaces, the completed Tasks conversion guidance, the finalized cross-module converted modal action ownership standard, Task/Notes stacked child-dialog parity, Notes/Tasks footer visual parity, the Notes notification follow-bell pass, the Files conversion branch, the Clients/Projects descriptor-readiness inventory, framework-rendered read anatomy boundary, action registration cleanup, related-region cleanup, bulk-toolbar/row-selection shell cleanup, service-owned hierarchy ordering/reparent safety boundary, strict filter/tag/action cleanup boundary, and the cross-surface 0.33.5.18 closeout inventory. The helper does not change module APIs, database schema, permissions, or business workflows. The exported field-factory and editable-grid contract is updated through 0.33.14.2.
...
Supported field types are text, number, select, multi-select, boolean/checkbox, toggle/switch, radio, textarea, date, and time. Existing descriptor compatibility types `search`, `hidden`, and `url` retain their shipped text-input behavior. Number metadata supports `min`, `max`, `step`, and `inputmode`; common metadata includes `field`/`id`, `label`, `required`, `options`, `optionsSource`, `default`, `placeholder`, `rows`, `autocomplete`, `hidden`, and `width`. The descriptor manifest contract rejects unrecognized field types and accepts `max` plus `inputmode` wherever `VIEW_FIELD_FIELDS` is used.

Scalar controls use an explicit label/control association. Radio groups use a `fieldset` and `legend`; multi-select values bind as collections; boolean controls preserve real boolean checked state; and toggle/switch controls use checkbox behavior with `role="switch"`. `createField()` owns this anatomy only. Callers still own option meaning and hydration, validation, conditional visibility, value semantics, save payloads, route calls, permissions, and refresh behavior.

As of 0.33.14.2, `createFieldGrid({ editable: true, fields })` marks a persistent grid as editable, and descriptor item forms may opt in with `itemForm.editable: true`; grids that do not opt in retain the shipped read-only persistent behavior. `renderDescriptorFieldGrid(descriptor, { editable, values })` binds caller-supplied values by field ID without learning record meaning. Every grid exposes `grid.viewParts.collectValues()`, backed by `LongtailForge.view.collectFieldValues(grid)`, which reads live controls into a flat payload: checkboxes/switches become booleans, numbers become numbers, multi-selects become arrays, radio groups become the selected scalar, and disabled controls are omitted by default.

Each field includes a hidden polite message channel exposed as `fieldElement.viewParts.message` and `fieldElement.viewParts.setMessage(message, { invalid, tone })`. The factory owns message placement plus `aria-describedby` and `aria-invalid` synchronization; the caller owns validation rules, message wording, when validation runs, and whether saving is allowed. A cleared message removes the description link and resets invalid state.

The descriptor renderer routes modal fields, filter fields, persistent read-only fields, and opt-in persistent editable grids through `createField()`. Reporting uses the primitive for its selector and filter controls. The shared Settings renderer uses `createField()` and `createFieldGrid()` for metadata rendering and delegates its nested module payload shape to the generic collector. These are the three real consumers required by the Two-Module Rule: the current renderer, Reporting, and Settings. No framework-wide exception is needed.

Framework-owned renderer, Reporting, and Settings code must not hand-build `input`, `select`, `textarea`, `label`, `fieldset`, or `legend` anatomy with raw `document.createElement` when `createField()` covers the control. Module-owned validation, save payloads, conditional visibility, option semantics, routes, and permissions remain outside this guardrail.

As of 0.33.15.4, `LongtailForge.settingsRenderer` builds each contribution as a titled `.view-settings-section` fieldset with a framework field grid and framework action row. Normal fields use the field factory, display-only `info` entries use the shared info panel, dependent visibility disables and omits hidden fields, and both native constraints and API field errors use the field primitive's message channel. The retired `settingsControls` and `settingsNormalizers` globals are not compatibility paths.

As of 0.33.15.8, Workspace, User, Tasks, Time Tracking, and Files Settings protected HTML are strict minimal `data-settings-host` mounts. `LongtailForge.settingsHost` creates framework-owned page headers, field sections, action/status regions, readouts, and modal shells through view primitives and exposes only standardized `data-settings-attachment` targets. Page adapters load filtered sections from `/api/settings/catalog`; placement, permission, capability, and enabled-module decisions stay server-owned. Module lifecycle controls are the explicit recovery exception that remain visible for disabled modules.

No first-party module setting ID belongs in `settings-host.js`, `settings-renderer.js`, `settings.service.js`, or `settings-catalog.service.js`. The real shared Settings consumers are Workspace Settings, User Settings, Tasks Settings, Time Tracking Settings, Files Settings, Create Workspace, and module-owned runtime readers through `settingsService.getValue(...)`; the intrinsically framework-wide exceptions are protected framework settings, module lifecycle state, identity/profile fields, audit/operations controls, and install/runtime configuration that cannot be module-overridden.

As of 0.33.5.28.2, view helpers and descriptors also must not learn database provider behavior. They can call protected routes and render route payloads, but canonical filtering, sorting, paging, permission pruning, database reads/writes, and dialect-sensitive SQL stay in server-side services and repositories that consume `src/core/database.js`, named params, and `db.dialect` seams.
...
| Clients/Projects | `views/protected/clients.html` and `views/protected/projects.html` are minimal hosts for strict read descriptors, and `public/js/clients-projects.js` renders those surfaces through `LongtailForge.view.renderSurface()` while keeping Clients/Projects behavior and editor bodies module-owned. Add/Edit Client and Add/Edit Project dialog shells use `LongtailForge.view` modal/form/footer helpers, descriptor/query Add/Edit dispatch goes through the shared module-action registry, related Client/Project read rows use shared list/table/action helpers, and Client/Project bulk toolbars use the shared bulk-toolbar shell from descriptor regions. | Add/Edit Client dialog, Add/Edit Project dialog, related-project/client tables, shared modal footer/action placement, slide-out filter controls, hierarchy display, row-selection checkbox shell, secondary tag chip rows, icon-only repeated table actions, bulk/action/status shells. | Modal helper adoption shipped in 0.33.5.15.4 at the shared dialog source. The 0.33.5.18.13.1 readiness pass added the reporting inventory and shared descriptor support for display-only hierarchy metadata, option-source hydration, and table display hooks. The 0.33.5.18.13.2 pass added separate `client-projects.clients` and `client-projects.projects` descriptors plus minimal hosts bound to `/api/clients` and `/api/projects`; the 0.33.5.18.13.3 pass moved page headers, filters, status/empty/loading/error states, table wrappers, hierarchy/chip display, and page/row action placement into descriptor/shared renderer paths while keeping dialogs, labels, option hydration, query-param openers, saves, routes, permissions, billing, task defaults, tags, and hierarchy rules module-owned. The 0.33.5.18.14.1 pass routes descriptor/query Add/Edit through `LongtailForge.moduleActions.open(...)`, publishes the module-owned `LongtailForge.clientProjectDialog` API, and removes duplicate adapter action registration plus the page-level Add Client compatibility form. The 0.33.5.18.14.2 pass moves related Project and Project-context read rows into shared list/table/action shells while keeping related-row shaping, summaries, tags, and route actions module-owned. The 0.33.5.18.14.3 pass moves Client/Project bulk toolbar chrome and row-selection checkbox anatomy into descriptor/shared renderer paths while keeping selected IDs, bulk options, payloads, granular route calls, workspace gating, partial failures, and refresh behavior module-owned. The 0.33.5.18.14.5 pass activates strict guardrails, moves filters into the left slide-out surface, moves tags into secondary table rows, converts repeated table edit actions to icon-only controls, and removes the legacy page table/filter/bulk-dialog fallback without changing routes, payloads, permissions, schema, or workflows. |
...
| Workbench | As of 0.33.6.12d-1, `views/protected/workbench.html` is a minimal `data-workbench-host` shell and `public/js/workbench.js` builds the guided Workbench page through `LongtailForge.view` primitives while loading focus modes/candidates and contribution source payloads from protected routes. Workbench has explicit `focus-selection` and `task-focus` browser states exposed through host data attributes and the page-controller snapshot. Focus Selection keeps the current focus questions, split Client/Project filters, one recommended-action card cycling through the top five ranked candidates, active/paused-only Timers card, and a bounded right-side "More in this focus" Inspector overflow list sourced from candidates after the top-five window. Primary actions on normalized Task candidates are labeled `Focus task` and enter Task Focus by recording the active task focus; `Change Focus` exits Task Focus without changing the selected focus mode or filters. Candidate dismissal controls are removed. Focus Selection Inspector title clicks use the same primary candidate path, while non-task primary candidates use an explicit temporary page fallback until their owning module has a Task Focus-equivalent target view. Task Focus shows a selected-task action strip, summary, read-only collapsed Task Details, a Task Focus Checklist check-only execution surface, and a default-open task-linked Task Timer section while hiding Focus Selection panels and candidate overflow. As of 0.33.6.12e-2, the right-side Inspector renders the selected-task related-context read model, defaults open in Task Focus, exposes a caret collapse control, groups linked Notes, task Files, linked Lists, same-project Tasks, and direct shared-tag records, and dispatches existing module actions or safe fallbacks. As of 0.33.6.12f, Pick up where I left off preserves running/paused timer precedence, then boosts the second-most-recent updated readable active task from the Tasks Workbench item source before the remaining resume/candidate list. As of 0.33.21.3.1, live timers are merged even when resume state exists; readable task-sourced timers are Task candidates ordered running before paused, manual timers retain Time Tracking identity, due In Progress Tasks remain eligible in "What's due next", and every mode except "Review blocked work" excludes Blocked candidates. As of 0.33.6.12g, opposite-state panels are fully removed from visible layout and focus order: Focus Selection never leaves a Task Focus action shell or task wrapper card behind, and Task Focus never exposes the guided-focus or recommended-action surfaces until `Change Focus` returns there. As of 0.33.6.12h, the `Same project tasks` Task Focus Inspector group is ordered by due-date proximity inside that group: due-today/overdue first, then future due dates nearest-to-farthest, then no-due tasks, with deterministic task-level tie-breakers. As of 0.33.6.12j, the selected-task summary shows one Client/Project path line and uses the summary chip row for status, priority, due date/time, and safe direct tags instead of duplicating the path in body copy. As of 0.33.6.12k, Task Focus keeps exactly one visible focused-task timer representation in the Task Timer section, while the lower `Other Active Timers` panel stays visible, excludes the focused task's running/paused timer, and shows only other running/paused task or manual timers. As of 0.33.6.12l, the Task Focus checklist mutation response refreshes the selected task status chip immediately: Open becomes In Progress after checking checklist work, eligible In Progress returns to Open after all checklist work is unchecked, and stronger lifecycle statuses remain unchanged. As of 0.33.6.12m, Task Focus linked Note rows open the Notes-owned `notes.view` read modal, which renders the canonical server Markdown HTML and offers an explicit Edit handoff without embedding note bodies in the Inspector. As of 0.33.6.12n, recurring linked notes propagated through `All Future` appear through the same related-context rows and `notes.view` action while Workbench still receives relationship metadata instead of note bodies. As of 0.33.6.13a, Focus Selection due/project/client/blocked modes include Tasks-owned active task candidates where needed so overdue scoped work is visible first in the shared recommended-action and Inspector overflow order while browser code still renders the service-owned candidate list. As of 0.33.21.21.1, every loaded Open or In Progress Task Focus without a Blocked Reason registers one bounded exit guard with the framework app-shell navigation-intent controller; timer and checklist state do not gate it. Blocked status or any nonblank Blocked Reason suppresses capture, including a stale race re-read before the canonical Tasks write. Successful focus entry still consumes the saved candidate handoff without changing status, Focus Selection labels a nonblank handoff as `Resume note:`, confirmed eligible capture moves the Task to In Progress through Tasks-owned lifecycle rules, and No performs no write. Interceptable Change Focus, links, Search, Notifications, workspace/account actions, scripted fallbacks, and supported history traversal still continue one exact destination once; reload/hard exit records only Task ID plus timestamp and revalidates readability, eligible lifecycle, blocked context, and current note on safe Workbench return. As of 0.33.6.14.1, the Workbench host guardrail is strict for this branch. The Workbench must not render a full Tasks index, task filter bar, task sort control, task-list actions, `taskItems` browser state, an empty bootstrap `taskItems` compatibility field, Quick Notes section, main-column "More in this focus" section, manual timer creation row, duplicate focused-task timer card, embedded preview pane, or unsafe raw-ID labels. | Page header, header-slot status state, `Change Focus` state action, guided focus questions, split focus filter row, one recommended-action card, useful empty states, right-side overflow Inspector, Focus Selection Timers card, Task Focus action strip, selected-task summary, read-only collapsed Task Details, Task Focus Checklist check-only section, default-open task-linked Task Timer section, selected-task `Other Active Timers` panel, selected-task related-context read model, linked-note read modal handoff, accessible collapsible summaries, inline candidate/timer/action rows, badges. | Keep Workbench framework-owned. The browser host owns focus selection, view-state tracking, active task-focus selection, client/project filter visibility and persistence, top-five recommendation cycling, page anatomy, right-side Inspector overflow layout and Task Focus collapse state, Task Focus selected-task shell, state-aware Other Active Timers filtering, relocated header status slot, collapsible-section default state, focus return, raw-ID fallback labels, bounded Task Focus exit eligibility and hard-exit marker lifecycle, and temporary page fallback handoff; the app shell owns generic navigation intent and exact destination continuation; QAC owns quick capture and dispatches Timer capture to the Time Tracking Create Timer modal; focus/candidate services own deterministic ranking/filtering/sorting, resume-first strategy execution, live-timer merging and Tasks lifecycle shaping, second-most-recent updated task boost, recurring-created exclusion, and permission/workspace/module scoping; the Task Focus related-context service owns selected-task aggregation through module service contracts plus same-project task due-ordering inside that read model; Notes owns the linked-note read modal, rendered Markdown body, secure/private body visibility, and Edit handoff; Tasks owns task edit, completion, blocked-status validation, checklist/timer mutation, resume-note consume/capture transitions, checklist-driven open/in-progress status transitions, audit/event/search side effects, canonical task `updated_at`, and readable task payloads; Time Tracking owns manual active timer storage, Create Timer modal behavior, sourced active timer storage, and timer finalization services; module work-item/timer providers keep record meaning and source routes. |
| Dashboard | As of 0.33.6.13a, `views/protected/dashboard.html` is a minimal `data-dashboard-host` shell and `public/js/dashboard.js` builds the Dashboard through `LongtailForge.view` primitives while loading the framework `/api/dashboard` read model. That read model is owned by `src/services/dashboard.service.js` and returns workspace summary, region metadata, Workspace Pulse, safe setup warnings, and permission-filtered contribution metadata. Panel placement is driven by the validated contribution `placement` field (`pulse`, `attention`, `today`, `main`, `activity`, `secondary`, `reporting`) and omitted placement defaults to `main`; the browser host must not special-case first-party panel IDs for placement. Project Summary, Tasks Needs Attention, Tasks Today / Upcoming, compact Tasks, and compact Time Tracking effort panels hydrate module data through their declared module-owned `dataRoute` values. As of 0.33.6.13b, the Tasks dashboard route owns attention dedupe/order, upcoming horizon selection, pressure metrics, and task-linked running/paused timer signals; the Dashboard browser renders the returned rows/actions without task due/status/timer recomputation or Task edit-modal row links. As of 0.33.6.13c, Time Tracking contributes only `Active Timers` and `Recent Time` Dashboard cards through `/api/time-tracking/dashboard/effort-summary`; default Dashboard placement must not render Current Month Billables, Hours & Billables by Month, billing tables, billing charts, invoice-ready copy, or Client billing language. As of 0.33.6.13d, `main` is the Module Overview grid for compact enabled-module cards and `activity` is Recent Activity with a quiet deferred/empty state until safe rows exist; Tasks pressure shows compact metrics plus at most one suggested row in overview. As of 0.33.6.14.1, the host guardrail is strict for this branch and the visual hierarchy keeps Pulse first, Needs Attention before calmer regions, Module Overview compact, Recent Activity secondary, and Reporting secondary. | Page header, status state, Workspace Pulse, setup warning region, dashboard regions, Module Overview grid, contribution-backed overview panels, compact metrics, safe task attention rows, compact time activity rows, quiet activity empty state, loading/empty/error states. | Keep Dashboard overview-focused and framework-owned. The browser host owns overview anatomy, renderer dispatch, region placement, pulse/warnings, sparse/deferred empty states, responsive wrapping/truncation, and status/empty states; modules own contribution declarations, module renderers, backing data routes, permissions, record meaning, row ordering, and drilldown targets. Reporting owns detailed time and billing analysis; QAC owns capture. |
| Reporting | As of 0.33.12.2, Reporting selected a narrow framework host adapter built on `LongtailForge.view` primitives rather than forcing report execution into the record-oriented `viewSurfaces` descriptor. As of 0.33.12.3, the permission-filtered `/api/reporting/catalog` and namespaced `/api/reporting/reports/:reportKey/run` contracts exist, with allowed renderer assets and registered-by-ID server dispatch kept separate from the manifest. As of 0.33.12.4, Time Tracking registers the production Project Time & Billing runner and owns its canonical calculation/read path; the retained bootstrap/summary endpoints are module routes, while the framework Reporting service is first-party-module-neutral. As of 0.33.12.5, recursive project/client totals calculate direct time with each owner’s effective billing settings before adding child branches, and nested child rows stay display-only rather than feeding footers twice. As of 0.33.12.6, `views/protected/reporting.html` is a minimal `data-reporting-host` shell and `public/js/reporting.js` creates the header, report selector, metadata-driven filters, conditional dates, shared status/empty/error states, results host, allowed-asset loader, query deep links, and generic execution/renderer dispatch through `LongtailForge.view`. The selected catalog entry supplies every filter definition, runner route key, renderer ID, and allowed asset; the first eligible report is the fallback. Time Tracking's dynamically loaded asset hydrates its readable scope/project/tag choices and registers the expandable Project Time & Billing table, while footer values come directly from runner totals. | Framework page shell/header, report selector, shared filter host, status/loading/error/empty states, results host with overflow, renderer dispatch, future report-action placement. | The framework owns catalog filtering, basic filter validation, safe execution envelopes, host anatomy, selection/dispatch placement, query-state synchronization, and allowed-asset loading. Modules own report metadata, runner/renderer IDs and registration, option hydration, dependent-filter behavior, data queries, domain calculations, result shapes, readable labels, record-level permission checks, compatibility reads during migration, and result rendering. Cross-module data must use owner service/provider contracts plus declared dependencies. Result renderers may display nested branch rows but must trust runner-provided root/footer totals instead of summing expanded rows. Do not pre-implement the 0.33.14 editable-field factory or keep Time Tracking-specific IDs/assets in the converted framework host. |
| Admin and Settings | User Settings, Workspace Settings, Tasks Settings, Time Tracking Settings, Files Settings, and Developer Example Settings are minimal `data-settings-host` pages built by `LongtailForge.settingsHost`; API Keys, Audit Log, Notifications, and User Admin retain their existing surface-specific hosts. | Settings page headers, field grids, individual or grouped contribution sections, attachment regions, action/status rows, operational readouts, modal shells; retained admin data tables and ordered Admin navigation. | The six Settings hosts use the permission-filtered catalog and shared renderer. Workspace may group optional module sections visually while preserving each namespaced payload. Ordinary module settings come from manifest contributions plus optional owner-registered handlers/effects; framework Settings code must not import feature modules, name first-party setting IDs, or invent module-specific branches. Developer Example's descriptor remains a reported fixture, while its actual settings page now uses the shared Settings host. Do not claim the broader Admin family is converted. |

As of 0.33.21.3.3, Workbench still treats the Tasks checklist mutation response as authoritative for the selected Task Focus status. Tasks may return Open after the last checklist item is unchecked only when its centralized evidence check finds no remaining checked item, running or paused task timer, or persisted task-linked time entry; the Workbench browser must not recompute those facts.

As of 0.33.12.7, the Reporting row is a strict framework-host contract even though it intentionally is not a `viewSurfaces` descriptor. Its HTML is one shared-width mount; page header, selector, filter/status/empty/results anatomy comes from `LongtailForge.view`; and the only permitted direct `document.createElement` calls load catalog-approved renderer `link`/`script` assets. Reporting-specific layout/footer classes and first-party report/module IDs, asset paths, service imports, or result fields are rejected in framework Reporting code. Modules still own their registered renderer bodies and report-specific behavior.

As of 0.33.6.14.2, the Notes list query path and Files browse plus File Context target reads consume the shared descendant-aware hierarchy scope resolver. Browser code continues to submit only the selected direct client/project values; the Notes service/repository and Files service expand readable descendants server-side so Notes/Files keep their existing permission and workflow ownership boundaries.
...
As of 0.33.21.17.1, `LongtailForge.capturePrompt.open()` is the shared one-value capture-modal adapter over those view primitives. Callers supply prompt, label, initial value, action labels, parent modal, and trigger; the helper owns the accessible modal/form/input shell, required-value check, stacked display, focus, and `{ confirmed, value }` result. Callers continue to own whether a prompt is needed, permission and workflow gates, cancellation rollback, persistence, and domain-specific validation. Tasks is the first consumer for Blocked-reason capture; the helper contains no Tasks status or route knowledge.
...
The protected Files page is a minimal host that loads the shared view builder, descriptor renderer, and Files adapter. The first Files slice did not move query construction, attachment reads, delete/restore actions, download URLs, upload helpers, storage accounting, scan/quarantine behavior, or record attachment semantics out of the existing Files routes and services. It initially mounted the browse UI through a transitional descriptor region; later 0.33.5.18.11 and 0.33.5.18.12 slices replaced that transitional region with explicit `files.browse.filters` and `files.browse.results` behavior mounts and strict guardrails.
...
Preview content keeps the same attachment-scoped target read and context-scoped `files.download` checks as the descriptor route. Unsupported, oversized, unavailable, failed-scan, deleted, pending, and unauthorized attachments do not return preview content. In-review attachments are previewable only when the scan state is already passed/no-review and the user can manage file review; previewing them does not make download available. This backend-only slice does not add browse-row Preview controls, a preview modal, Inspector behavior, or preview audit/lifecycle events; user-facing preview tracking can be added with the modal slice if it becomes useful.

## Implementation Notes For 0.33.5.18.11.12

Files browse rows now include an explicit icon-only Preview action for rows that are locally likely to be previewable: available image/TXT/MD attachments with passed/no-required scan status inside the text/Markdown preview size cap, plus in-review passed/no-required attachments when the user can manage file review. Available downloadable rows that are not previewable show a quiet download-only marker instead of a noisy detail panel. The marker and the Preview button both participate in row action isolation, so clicking row Preview does not open File Context and row click/Enter still opens only the File Context modal.
...
The upload action itself uses `LongtailForge.view.createActionButton()` when available, but the helper still owns submit/drop handling, accepted extension/category mapping, streamed multipart batch payload construction, target/module/client/project/visibility values, `/api/files/upload/batch` calls, partial-failure result rows, upload events, attachment refresh, and host callbacks. This slice does not add upload UI to the Files browse page, File Context modal, or Files Preview modal.
...
Files visual states now use shared chip and dense-action anatomy while preserving the compact browse boundary. Browse rows render availability and distinct review-state chips in the Status column, and the shared attachment helper renders size/status/review/visibility metadata as wrapping chips without duplicating contradictory status and review text. Deleted files appear as unavailable, in-review files appear as in review, pending/error scan states appear as review pending or review needed, and normal UI avoids quarantine/scan wording except where route/action IDs remain internal.

The Files page keeps icon-only controls for dense repeated Preview, Download, Report, Review, Delete, and Restore actions with accessible labels and titles. The Review control still calls the existing quarantine route and preserves Files service permission, audit, lifecycle, and restore/download behavior. File Context and Preview controls are included in visual parity checks but remain the route-backed modal workflows from 0.33.5.18.11.
...
## Implementation Notes For 0.33.5.18.12.6

Files strict declarative guardrail enforcement is active for the framework-owned `files.browse` surface. `views/protected/files.html` remains a minimal host, `files.browse` joins the strict declarative surface set, and `scripts/view-descriptor-declarative-guardrails.mjs` now fails if Files reintroduces hand-built protected-page anatomy, the old transitional browse fallback, direct DOM construction in `public/js/files.js`, inline Browse Summary/Preview/Metadata/detail/selected-row/Inspector behavior, exposed storage internals, or route bypasses.

`public/js/files.js` now builds browse filters, results mounts, table cells, status chips, dense row actions, Preview body content, and File Context controls through descriptor behaviors and shared view helpers. Files-owned behavior remains explicit: `/api/files/attachments` reads, `/api/files/:fileId/download`, Report, Review, Delete, Restore, `/api/files/attachable-targets`, File Context context saves, Preview descriptor/content reads, confirmations, availability decisions, workspace permission-shaped visibility, focus return, readable target labels, and the route-sanitized Markdown preview body.

`public/js/shared/file-attachments.js` now centralizes old/no-helper element fallback in `createAttachmentElement()` while using shared list shell, empty-state, action strip, and action button helpers when available. The strict guardrail allows that single centralized fallback plus Files-owned streamed upload payload construction, accepted categories, attachment refresh, host callbacks, remove/report/review/delete/restore routes, download links, and recovery-state wording. It still forbids turning the shared attachment panel into inline File Context, Preview, Metadata, or Inspector UI.

## Implementation Notes For 0.33.5.18.12.7

The Files conversion branch is closed at the compact listing-first boundary established by 0.33.5.18.11.13 and protected by the 0.33.5.18.12 upload/action/strict-guardrail work. The main Files page remains a workspace file recovery/audit listing with filters in the slide-out sidebar and listing-only content in the main panel.

File Context and Preview remain route-backed, attachment-scoped modal workflows. The completed upload shell, shared attachment panel shell, row/attachment actions, visual states, and strict `files.browse` guardrails standardize framework-owned anatomy around those workflows without creating inline browse detail, Preview, Metadata, selected-row state, or Inspector behavior.

Future Files slices should start from this closeout contract. Rename, binary replacement, storage moves, hard purge, permanent delete, raw storage controls, scanner/quarantine metadata editors, and direct static downloads remain out of scope until a future roadmap item adds explicit Files service routes, permissions, audit behavior, and regressions.

## Implementation Notes For 0.33.5.18.15

The 0.33.5.18 view-conversion branch is closed at a strict/deferred boundary. Strict fail-on-violation guardrails cover `lists.workspace`, `notes.workspace`, `tasks.workspace`, `files.browse`, `client-projects.clients`, and `client-projects.projects`. Those surfaces must keep protected HTML minimal and must render framework-owned page/filter/table/list/modal/action anatomy through descriptors or shared helpers.

Tags management and Developer Example descriptors remain reported descriptor proofs, not strict-converted surfaces. Admin/Settings, Reporting, Dashboard, Workbench, pagination/server-side paging, Inspector behavior, and any non-view workflow changes stay deferred to later roadmap lines. This closeout records the converted-surface inventory, docs, Decisions, changelog, version metadata, archive handoff, and verification stack; it does not add schema, write-payload, permission, public API, route, or workflow changes.

Future converted-surface work should start from this boundary and update the relevant strict guardrail inventory before changing ownership. Framework code owns shared anatomy and accessibility defaults; modules keep records, routes, payloads, permissions, readable labels, canonical sorting/filtering semantics, refresh behavior, and workflow meaning.

## Implementation Notes For 0.33.5.18.14.5

The Clients/Projects strict cleanup promotes `client-projects.clients` and `client-projects.projects` from reporting-only descriptors to strict declarative surfaces. `views/protected/clients.html` and `views/protected/projects.html` stay minimal hosts, and the adapter mounts those pages through `LongtailForge.view.renderSurface()` without the old page table, inline top filter, or standalone bulk-dialog fallback path.

The framework renderer owns the table-page slide-out filter surface, secondary table rows for tag chips, descriptor row-selection checkbox shell, repeated edit action icon-button anatomy, table wrappers, related read shells, and bulk-toolbar shell placement. Descriptor row actions may carry `icon`, `iconOnly`, and `title`; table descriptors may carry `secondaryRows`; and table-page descriptors may use `filterPlacement: "slide-out-sidebar"`.

Clients/Projects still owns filter option hydration, tag values and assignment, selected IDs, allowed bulk controls, route calls, payloads, confirmations, partial-failure messages, dialog bodies, billing/default editors, parent selectors, hierarchy validation, readable labels, query-param openers, refresh hooks, and workspace gating. This slice does not change schema, route payloads, permissions, or workflow behavior.

## Implementation Notes For 0.33.5.18.14.4

The Clients/Projects hierarchy/reparent pass keeps the converted read pages descriptor-mounted while moving canonical Projects ordering into `clients.service.js`. The Projects page consumes `/api/projects?include_depth=true`; the service returns workspace-level Projects first, then Client-backed Projects in readable Client hierarchy groups, and then parent-before-child Project hierarchy inside each group with alphabetical ordering only as a secondary sort.

The framework renderer owns display anatomy for the table and hierarchy metadata, but it does not own move rules. Descriptor row actions and query openers use registered behaviors to open the existing editors. Those editors keep readable parent selectors, move confirmation, payload construction, and refresh callbacks in `public/js/clients-projects.js`, while `project-update-planner.js` and the Clients/Projects service keep cycle prevention, same-scope checks, archived parent checks, Business-only Client rules, workspace Project behavior, and Project Client derivation.

This slice does not add drag/drop hierarchy editing, strict Clients/Projects guardrails, slide-out filters, secondary-row tag display, icon-only row-action cleanup, route changes, schema changes, or new write payloads.

## Implementation Notes For 0.33.5.18.14.3

The Clients/Projects bulk-control cleanup adds descriptor table selection metadata and before-table bulk regions to the reported Clients and Projects surfaces. The renderer owns the selectable-row checkbox anatomy, stable row-selection hooks, and explicit before-table region placement. The shared bulk toolbar helper owns the collapsed toolbar shell, selected-count chip placement, and generic toolbar classes.

Clients/Projects still owns selected Client and Project IDs, allowed bulk controls, Project Client reassignment options, billing/status payloads, confirmations, granular `/api/clients/:id` and `/api/projects/:id` route calls, partial-failure messages, refresh behavior, and permission/audit/search/event implications. Business workspaces may expose Project Client reassignment; Personal and Family workspaces must not expose Client reassignment controls or submit Client IDs from bulk Project updates.

This slice does not add service-owned bulk endpoints, hierarchy move/reparent behavior, strict fail-on-violation guardrails, route changes, schema changes, or write-payload changes.

## Implementation Notes For 0.33.6.10b

Quick Action Capture and future framework surfaces dispatch first-party create/edit/read modals through `LongtailForge.moduleActions`. `public/js/shared/module-actions.js` owns the action metadata and host-context envelope; `public/js/notes.js`, `public/js/lists.js`, and `public/js/files.js` publish module-owned opener APIs and register the same canonical action IDs when their adapters load. Notes and Lists wrappers call the existing `openEditor()` and `openListDialog()` paths, create only their existing dialog shells in dialog-only lazy-load mode, and signal host completion or cancel after the owning save/cancel path runs. Files registers only the existing attachment-scoped File Context and File Preview openers; QAC does not create a new targetless upload form.

## Implementation Notes For 0.33.5.18.14.2

The Clients/Projects related-region cleanup keeps the descriptor-rendered Clients and Projects pages list-first and does not add selected-row detail state. Related read content stays closest to the existing record dialogs/editor bodies.

Client detail reads mount related Projects through `LongtailForge.view.createCollapsibleIndexPanel()`, `createListShell()`, `createDataTable()`, and `createDetailActionStrip()`. Project editor reads mount Client and Parent Project context rows through the same shared list/table/action anatomy. The framework owns the related-region shell, table wrapper/header/cell anatomy, empty state, and dense row action placement.

Clients/Projects still owns related-row shaping, readable Client/Project labels, display-only hierarchy metadata, billing and task-default summaries, tag chip content, allowed row actions, route calls, save/archive/reparent behavior, permission implications, audit/search/event side effects, and refresh callbacks. This slice does not convert bulk controls, hierarchy move/reparent workflows, strict fail-on-violation guardrails, routes, schema, or write payloads.

## Implementation Notes For 0.33.5.18.14.1

The Clients/Projects action registration cleanup keeps the 0.33.5.18.13.3 descriptor-rendered read pages intact while normalizing Add/Edit dispatch. Descriptor behavior handlers for Add Client, Edit Client, Add Project, and Edit Project now call `LongtailForge.moduleActions.open(...)` with the canonical `clients.add`, `clients.edit`, `projects.add`, and `projects.edit` action IDs. The same bridge handles the `?client=`, `?project=`, `?addClient=true`, and `?addProject=true` query openers.

`public/js/shared/module-actions.js` owns the first-party action metadata and host-context envelope. `public/js/clients-projects.js` publishes `LongtailForge.clientProjectDialog` and keeps the module-owned dialog openers, field bodies, tag pickers, billing/task-default editors, parent selectors, validation, payloads, route calls, refresh callbacks, and save completion signaling. The adapter keeps a direct fallback from the canonical action IDs to those dialog openers for early-load/standalone use.

The duplicate page-level Add Client compatibility shell, duplicate Add Client submit path, and adapter-level first-party action registration blocks are removed. This slice does not convert related tables/detail regions, bulk controls, reparent interactions, or strict fail-on-violation guardrails.

## Implementation Notes For 0.33.5.18.13.3

The Clients/Projects framework-rendered read anatomy pass mounts `client-projects.clients` and `client-projects.projects` through `LongtailForge.view.renderSurface()` inside the minimal protected hosts. The framework owns the page header, loading/error/empty/status placement, select filter shells, table wrapper/header/cell anatomy, hierarchy-label indentation, chip-list display, page Add action placement, and row Edit action placement.

The Clients/Projects browser adapter registers behavior handlers for Add Client, Edit Client, Add Project, and Edit Project that call the existing dialog API. It also hydrates tag filter options and Business-only Client filter options through descriptor option-source behavior IDs, hides/disables Client filters outside Business workspaces, preserves `?client=`, `?project=`, `?addClient=true`, and `?addProject=true` entry behavior after descriptor render, and refreshes the descriptor surface after existing dialog saves.

Clients/Projects still owns safe row labels, readable tag values, billing display values, Project Client labels, dialog bodies, save payloads, route calls, permissions, query-param opener decisions, hierarchy mutation rules, parent/reparent validation, billing/task-default editors, tag assignment, audit/search/event side effects, and refresh/focus callbacks. This slice does not convert related tables/detail regions, bulk controls, reparent interactions, or strict fail-on-violation guardrails.

## Implementation Notes For 0.33.5.18.13.2

The Clients/Projects read descriptors and minimal hosts pass adds separate reported descriptors, `client-projects.clients` and `client-projects.projects`, in `src/modules/client-projects/module.js`. The Clients descriptor binds to `/api/clients?include_depth=true`; the Projects descriptor binds to `/api/projects?include_depth=true`. `/api/client-projects` remains the shared option/dialog route and is not the new page read source of truth.

`views/protected/clients.html` and `views/protected/projects.html` are minimal hosts that load `view-builder.js`, `view-renderer.js`, and `clients-projects.js` in that order. `public/js/clients-projects.js` creates a small transitional compatibility scaffold inside those hosts so the existing Add/Edit openers, filters, status text, and list/table rendering keep working until the 0.33.5.18.13.3 framework-rendered read anatomy slice.

The 0.33.5.18.13.1 readiness support remains the descriptor foundation for these reads: display-only hierarchy metadata for flattened tree rows, option-source hydration for filter controls, and table display hooks for hierarchy labels and chip lists. These hooks are display and shell contracts only. Clients/Projects keeps Client and Project option data, tag search suggestion values, typed tag text resolution to canonical tag IDs, hierarchy validation, parent/reparent rules, Business-only Client scope, Personal/Family project-only scope, billing and task-default editors, tag assignment, route calls, query-param openers, save payloads, permission implications, audit/search/event side effects, refresh behavior, and the already-converted Add/Edit dialog bodies.
