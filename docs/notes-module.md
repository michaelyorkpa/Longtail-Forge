...
Note and List picker labels are also provider-owned. Note targets display the plain note title and may show safe Library/collection secondary context such as `Reference Library / Project Notes`; inaccessible private or secure notes are filtered before labels are returned. List targets display the plain list title and may show the list type as secondary context. Note/List options and existing linked rows must not expose UUIDs, raw target ids, or synthetic `Note:` / `List:` prefixes. Note targets sort by Library bucket, collection path, note title, and stable id; List targets sort by list type label, list title, and stable id.
