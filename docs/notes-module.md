...
As of 0.33.27.4, the Notes repository is the authoritative generator for new note, revision, link, and library-collection IDs through the framework `createRecordId()` UUIDv7 operation. The service forwards accepted caller-supplied UUIDv4/UUIDv7 IDs without generating duplicates, so legacy parents can safely own new UUIDv7 children and existing link targets remain unchanged. Notes keeps its explicit updated-time/title, revision-number, collection path/sort-order, and link creation-time ordering; UUID lexical order is not a chronology or paging contract.

As of 0.33.6.12e-1, Workbench Task Focus related context consumes Notes through Notes-owned linked-target and list service paths. Linked notes outrank project and shared-tag matches, shared-tag matches use direct tags only, and the related-context read model emits safe labels/action descriptors without note body text, secure bodies, encrypted payload metadata, or storage-sensitive fields. As of 0.33.6.12m, those linked-note action descriptors use the Notes-owned `notes.view` read modal first; the modal reads the canonical note detail route, renders existing server-generated Markdown HTML, and hands off to the existing editor only through an explicit `Edit` action. As of 0.33.6.12n, recurring Task `All Future` propagation consumes Notes-owned link reads/writes for task-linked notes: Notes filters readable notes, writes future occurrence `note_links` rows with recurrence metadata, removes only propagated future links for that recurrence template when a source link is removed, and never copies note body Markdown or rendered HTML into Tasks recurrence storage. As of 0.33.6.14.1, the Dashboard/Workbench closeout keeps that read-first boundary intact: Workbench Task Focus linked notes open the Notes-owned read modal before editing, and Dashboard does not add a Notes overview card until Notes exposes a safe body-free summary route. Notes editor/preview behavior remains owned by the Notes module.
...
The framework owns the slide-out shell, footer-aware screen-left funnel trigger, backdrop, Escape/backdrop/trigger close behavior, focus return, panel shell, disclosure behavior, scroll-safe panel bodies, filter search suggestion popover placement, footer slot placement, and shared bulk-toolbar/modal anatomy. Notes owns the panel content and behavior: filter query state, tag suggestion option data, Library bucket/collection controls, Notes List data rendering, selected-note IDs, bulk field meaning, sorting/pagination controls, archive handling, selected-note detail rendering, Linked Context display, and Primary Context display. Filters start collapsed inside the drawer, Library starts open, and selecting a note closes the drawer so the central detail remains primary. Library and Collection controls stack on separate lines, Collection actions open from a modal beside the Collection dropdown, collection create/edit handoffs wait for that actions modal to close before opening the editor as a top-level modal, and compact Notes List rows show one visible tag chip plus overflow to avoid metadata overlap. Each editable row also has a checkbox separate from the note-opening button; selecting rows opens the shared Bulk Edit toolbar and its Notes-owned modal.
...
Create and per-catalog edit continue through the canonical collection routes and validation. `POST /api/notes/settings/catalogs/bulk` accepts a unique list of at most 100 catalog IDs and only `archive` or `restore`. Archive removes redundant selected descendants when an ancestor is selected because the canonical archive operation already handles that subtree; restore processes selected parents before children. Every operation retains canonical permission checks, Notes-disabled rejection, audit recording, descendant behavior, and search synchronization. The response reports safe per-catalog failures so the browser can retain failed selections for recovery.

As of 0.33.29.3, users with `notes.secure.manage` see explicit versus inherited protection and safe `stable`, `securing`, or recovery-needed state in this manager. Enable and Retry use the dedicated transition preflight and show bounded catalog/note/revision counts without content. Removing security is a separate destructive dialog requiring the exact catalog ID, preflight count, and current password; ordinary edit/move controls never perform downgrade. A secure ancestor cannot be overridden. Active work refreshes automatically, while failed work remains effective-secure until the operator corrects the reported safe blocker and retries.

`src/modules/notes/consumer-policy.js` is the downstream non-exposure authority. Protected Notes workspace, revision, and relationship reads still require canonical secure-note authorization. Normal Files attachments, activity/notifications, Search, resume/Workbench, public API, export, generic provider catalogs, and Support View exclude effectively secure notes entirely. In Support View, list, target, Library, revision, relationship, and direct-detail reads use the declared `notes.support-view` exclusion; direct reads return only generic not-found, and secure catalog names/tree counts are omitted. When an existing note becomes effectively secure, Notes also removes its normal notification/subscription and resume-state artifacts while Search cleanup removes indexed documents; read-time checks remain authoritative. The Notes manifest declares each consumer and its `notes.effective-security` assertion; an undeclared consumer fails closed. This prevents titles, counts, existence, excerpts, relationship labels, bodies, and encryption metadata from escaping through a secondary surface.
...
As of 0.33.5.27.15, the Notes repository is fully converted to named params and the database dialect seams. Notes writes, revisions, links, collections, and count helpers use `db.query(...)`, `db.get(...)`, `db.run(...)`, and `db.transaction(callback)` instead of SQL literal helpers. The converted paths preserve note create/update behavior, create-with-staged-links transaction behavior, revision numbering, link lifecycle, target reads through allowlisted direct-context columns, collection nesting/counts, nullable text trimming, integer fallback coercion, and plaintext secure-placeholder checks.
...
As of 0.33.6.14.2, direct Notes list client/project filters now use the shared hierarchy scope resolver. On Business workspaces, selecting a parent client includes readable descendant sub-clients and descendant projects, selecting a parent project includes readable descendant sub-projects, leaf selection still drills down to the selected client/project only, unreadable descendants remain excluded, and secure/private shaping plus current collection/tag behavior stay Notes-owned.
...
As of 0.33.6.15.1, the Add/Edit Linked Context picker shows a Business-only client context selector supplied by Notes to the shared picker shell. The default `All Clients` option keeps existing breadth and keeps Business Project labels as `Project Name - Client Name` or `Project Name - Workspace Name`; selecting the workspace-name option scopes to client-less workspace projects and drops the suffix; selecting a client scopes records through the shared descendant-aware hierarchy resolver and also drops the project suffix. Personal and Family workspaces do not render the selector, do not include a workspace-as-client entry, and keep project labels plain.
...
As of 0.33.11.2, Save on a newly created note persists the record and converts the still-open dialog into the canonical Edit Note state. Copy Link, notification follow state, saved-note Tags, and Files are remounted against the new note ID without discarding the captured context. The status directs the user to continue editing or choose Save & Close. Save on an existing note retains the normal save-and-close behavior, while the explicit Save & Close action always commits and dismisses the dialog.

As of 0.33.5.21.9.3, Notes file panels use the shared Files Preview modal for previewable attachments in both the note detail Files panel and the Add/Edit Note Files child dialog. The shared attachment helper renders Preview, Download, Remove, Report, Review, Delete, and Restore as icon-only action buttons with accessible labels and titles. Preview calls `LongtailForge.filePreview.openFilePreview()` and stays attachment-scoped and route-backed; Notes does not add inline preview, metadata, File Context, storage, scanner, or Inspector UI.

As of 0.33.5.18.10.8.1, the converted modal action ownership contract applies to Notes and Tasks: Tags, Files, and Copy Link belong in the footer utility group, commit/dismiss actions belong in the footer commit group using the compact Tasks-style treatment with accessible labels and titles, and Follow Notifications belongs in the heading action slot only for saved records that can emit meaningful notifications. Notes owns note-specific labels/icons, save payloads, validation, permissions, note URL construction, staged tag changes, file attachment availability, secure-note restrictions, Library/Linked Context behavior, and note notification event production. As of 0.33.5.18.10.8.4, Notes produces those notification events and the modal heading uses the saved-note follow bell. As of 0.33.11.2, the commit group contains Cancel, Save & Close, and Save.

As of 0.33.5.18.10.8.3, Notes uses the converted modal footer visual standard: Tags, Files, and Copy Link footer utilities use icon plus text, while commit/dismiss controls use the shared footer placement with accessible labels and titles. As of 0.33.5.18.10.8.4, the heading action slot uses a saved-note Follow Notifications bell for normal saved notes, backed by Notes-produced notification events and the framework subscription helper. The duplicate top Close button is removed; footer Cancel remains the dismissal control, and 0.33.11.2 adds the visible Save & Close commit action beside compact Save.

As of 0.33.5.18.10.8.5, the Notes editor is the Notes reference implementation for the finalized converted-modal action standard. Future Notes modal work should keep Tags, Files, and Copy Link in the shared utility footer group, keep Cancel, Save & Close, and Save in the shared commit footer group, keep the saved-note follow bell in the heading action slot only when note notifications are meaningful and allowed, and use stacked child dialogs for substantial utility bodies. Notes remains responsible for note URL construction, staged tags, file availability, secure-note restrictions, notification event meaning, save payloads, validation, permissions, Library behavior, and Linked Context behavior.
...
Saved note reads include `body_html` rendered through the Notes adapter. Draft preview uses the protected `POST /api/notes/preview` route, which validates the draft body and returns safe rendered HTML without persisting the note. Unsafe Markdown validation returns a fixed safe 400 error and never reflects the rejected body; public-demo admission accepts both `body_markdown` and `bodyMarkdown` under the shared 32 KiB rich-content ceiling before the Notes validator runs. As of 0.33.5.21.9.1, the per-user "Open external links in a new tab" preference is applied only in browser code after saved or preview Markdown HTML is injected: absolute `http`/`https` anchors may receive `target="_blank"` plus `rel="noopener noreferrer"`, while the shared server-rendered `body_html` cache stays byte-identical across users and wiki spans, relative app links, and `mailto:` links are not changed. The browser editor remains a Markdown textarea with preview; `public/js/shared/notes-editor.js` owns scoped authoring helpers for Tab indentation, Shift+Tab outdent, predictable Enter list continuation, empty-marker cleanup, and toolbar insertions for unordered and ordered list markers.
...
Creating a note inside an effectively secure catalog encrypts its body and initial revision atomically. Moving an ordinary note into one encrypts the note, every existing revision, and the before-move snapshot in one repository transaction before the new membership can be observed. Moving a note out preserves protection by setting explicit note security in the same database update; moving an inherited-secure catalog subtree out sets secure policy at the moved root. Catalog membership changes are never implicit decryption actions.

Catalog policy changes use the dedicated preflight, enable, retry, and remove-security routes under `/api/notes/collections/:collectionId/security`. They require `notes.manage_library` and `notes.secure.manage`; ordinary collection create/edit/move routes cannot change policy. Preflight reports safe catalog/note/revision/search counts, blockers, and whether the measured work will run synchronously or through the framework job queue. At most 100 note/revision storage transformations run on the request path; larger work uses the `notes.catalog-security` job in batches of 50.

Every transition has a monotonically increasing claim version. Enabling security claims `securing` while policy is still `normal`, which makes the whole subtree authorization-secure immediately. The transition encrypts missing note/revision payloads idempotently, removes canonical and FTS search documents, verifies no protected plaintext or stale search record remains, and only then publishes stable `secure` policy. A failure records only a safe error code and leaves the catalog in effective-secure `failed` state. Retry creates a newer claim; an older request or worker becomes a no-op. Ordinary edits or moves involving the transitioning subtree or its relevant ancestry are rejected until completion or retry.

Removing security is a distinct deliberate downgrade. The caller must provide the current password and confirm the exact catalog ID, `remove_security` action, and current affected-note count. Only inherited notes that will become effectively normal are decrypted; explicitly secure notes and notes under another secure ancestor or descendant policy remain encrypted. Partial downgrade work retains secure policy plus a fail-closed transition state, so plaintext batches cannot be read as ordinary content before the final policy update. Search reindexing begins only after the stable normal policy is published. Passwords, bodies, keys, payloads, and raw crypto failures never enter job or audit metadata.
...
The Notes editor heading action slot contains a saved-note Follow Notifications bell for normal saved notes. The previous duplicate top Close button is removed; footer Cancel remains the normal dismissal control. Unsaved notes hide or disable the follow affordance because there is no stable note id to subscribe to. Tags, Files, and Copy Link remain footer utilities, while Cancel, Save & Close, and Save remain footer commit actions.
