...
This guide describes the current `viewSurfaces` authoring contract as of 0.33.5.18.12.4.
...
The inventory below is current for 0.33.5.18.12.4. `strict` means the static guardrail fails on declarative-surface violations. `reported` means the view is known to the inventory but is not strict-converted in this slice.
