...
This guide describes the current `viewSurfaces` authoring contract as of 0.33.5.18.11.13.
...
The inventory below is current for 0.33.5.18.11.13. `strict` means the static guardrail fails on declarative-surface violations. `reported` means the view is known to the inventory but is not strict-converted in this slice.
