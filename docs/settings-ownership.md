...
As of 0.33.22.9, the backend lifecycle also exposes an administrator-only named collection through `/api/private-feeds/calendar-subscriptions`. It is workspace-rooted, uses `workspace_settings.manage`, binds creation to the authenticated actor, permits safe metadata listing and administrator revocation across the workspace, and reserves rotation for the owner. This slice does not add the final Settings destination: the existing User Settings singular adapter remains temporary compatibility until `0.33.22.9.2` moves management to Settings -> Admin -> Modules -> Calendar and removes the old surface.
