...
| Settings catalog and attachment points | Workspace Settings, User Settings, Tasks Settings, Time Tracking Settings, Files Settings, Create Workspace, and module-specific settings navigation. |
| Shared Settings host and renderer | The five protected Settings pages plus Create Workspace's contributed module settings section. |
...
| --- | --- | --- | --- | --- | --- |
| Workspace name | Framework-owned: workspace identity | `workspaces.name`; trimmed by `normalizeSettings`, with the configured initial workspace name as repository fallback. | Settings UI, app-shell/bootstrap workspace context, Users workspace lists, audit labels, and services that need a readable workspace label. | `PUT /api/settings` -> `settingsService.save` -> `settingsRepository.saveWorkspaceSettings`; workspace creation writes through `workspacesRepository`. | Retain `workspaces.name`; protected framework handler. It is canonical entity identity, not a generic setting value. |
| Workspace type | Framework-owned: workspace identity/capability policy | `workspaces.workspace_type`; `normalizeWorkspaceType` restricts to `business`, `personal`, or `family`. Non-Business normalization hides Business-only fiscal/rate values. | Capability filtering, module contribution eligibility, Client visibility, Tasks reminder inheritance, Lists terminology, Files/permissions, app shell, and workspace creation. | Same workspace settings transaction; workspace creation also sets it after install/user policy checks. | Retain `workspaces.workspace_type`; protected framework/owner-only handler because changing it affects capabilities and visibility. |
...
Other `users` columns are explicitly not settings: `user_id`, `home_workspace_id`, and `active_workspace_id` are identity/context; `password` is a credential changed through `/api/user/password`; `user_status` and `protected_user` are admin/security lifecycle controls. They remain in `users` and must not become ordinary settings contributions.
