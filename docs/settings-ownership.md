...
Calendar Settings is the sole subscription-management surface at Settings -> Admin -> Modules -> Calendar. The framework-owned protected view requires `workspace_settings.manage`, uses shared Settings anatomy without universal Save/Revert controls, and operates only through the workspace-rooted `/api/private-feeds/calendar-subscriptions` collection/item routes. Creation always binds to the authenticated administrator and one Workspace, readable Client, or readable Project scope; safe metadata is visible across the workspace, owners alone may rotate their active rows, and administrators may revoke any active row. One-time URLs live only in page memory. Tasks owns permission-shaped iCalendar content, while the framework owns secret authentication and lifecycle. The Calendar destination and revocation remain available when Tasks is disabled, but creation and rotation fail closed.
