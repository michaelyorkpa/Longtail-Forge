...
| Settings catalog and attachment points | Workspace Settings, User Settings, Calendar Settings, Tasks Settings, Time Tracking Settings, Files Settings, Workbench Settings, Notes Settings, Developer Example Settings, Create Workspace, and module-specific settings navigation. |
| Shared Settings host and renderer | The nine protected Settings pages plus Create Workspace's contributed module settings section. Workspace groups optional lifecycle sections in one Modules box while retaining each module's namespaced payload. |
...
User Settings layout remains framework anatomy. Appearance and Profile share the first visual column; User App Preferences occupies the second and saves through the same user-owned route; Notification Preferences, the isolated Leave Workspace and Delete Account actions, and the initially collapsed Workspace Creation disclosure span the full grid. Calendar subscriptions do not appear in User Settings. The timezone value remains the validated `users.timezone` setting, while the browser builds its complete supported IANA choice list and current offset labels at render time. Leave Workspace continues through the existing membership-removal route and repeats the same administrator-restoration warning in its section and confirmation; it does not become workspace deletion. Delete Account is an immediate lifecycle action through `DELETE /api/user/account`, not a saved setting: it retires credentials, sessions, API keys, roles, creation grants, and all workspace memberships while retaining the identity row and readable authored-record attribution.

Calendar Settings is the sole subscription-management surface at Settings -> Admin -> Modules -> Calendar. The framework-owned protected view requires `workspace_settings.manage`, uses shared Settings anatomy without universal Save/Revert controls, and operates only through the workspace-rooted `/api/private-feeds/calendar-subscriptions` collection/item routes. Creation always binds to the authenticated administrator. Business workspaces may use Workspace, readable Client, or readable Project scope; Personal and Family workspaces render and accept Workspace or readable Project scope without Client. Safe metadata, including the owner's current profile timezone, is visible across the workspace, owners alone may rotate their active rows, and administrators may revoke any active row. One-time URLs live only in page memory and the standalone danger-colored warning uses user-facing copy without storage terminology. Manual Revoke records the security lifecycle and removes the credential row; already-revoked rows expose Delete for audited metadata cleanup. Tasks owns permission-shaped iCalendar content and publishes name/timezone metadata, while the framework owns secret authentication, friendly-filename URL construction, lifecycle, workspace-type enforcement, and backward-compatible token-only routing. The profile timezone remains a per-user setting rather than a Calendar credential setting. The Calendar destination, revocation, and deletion remain available when Tasks is disabled, but creation and rotation fail closed.
