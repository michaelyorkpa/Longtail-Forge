...
As of 0.33.27.5, `framework.identifier-authority` makes `src/core/identifiers.js` the only allowed `uuid` package entry point, proves that the authority exposes only record UUIDv7 and opaque UUIDv4 operations, pins every audited framework persistent-record generator to `createRecordId()`, every audited operational generator to `createOpaqueId()`, and the exact 28 server-side module record calls across 11 authoritative owners. The production direct-generator baseline is empty; the guard rejects unauthorized Node `randomUUID`, UUID-package `v4`/`v7`, and browser `crypto.randomUUID` use while freezing the existing session, CSRF, API-key secret, private-feed credential, password, Secure Notes, and related dedicated crypto-helper patterns. Clients/Projects browser-action and API regressions prove server-authoritative top-level and nested creation, canonical optimistic/action identity handoff, audit metadata, UUIDv7 generation, and caller-supplied UUIDv4 preservation. Clients/Projects, Tasks/checklist, Notes/revision/link/collection, and Lists/item integration owners retain mixed-version relationship and module-owned timestamp, due-date, sequence, title, revision, timer-slot, and paging-order proof. Documented unit, regression, seed, fixture, and recovery-drill generators remain outside the production baseline.
