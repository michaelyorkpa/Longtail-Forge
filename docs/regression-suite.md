...
As of 0.33.27.2, `framework.identifier-authority` makes `src/core/identifiers.js` the only allowed `uuid` package entry point, proves that the authority exposes only record UUIDv7 and opaque UUIDv4 operations, pins every audited framework persistent-record generator to `createRecordId()`, and compares every remaining production runtime/browser/operator `randomUUID` call against the classified temporary migration baseline. The framework record entries are gone; the baseline must still match exactly and may only shrink as 0.33.27.3-0.33.27.5 convert operational UUIDs, module records, and browser creation in order. Existing integration owners additionally prove UUIDv7 audit, API-key row, Files row/attachment, job, and recovery-audit identity; independent API-key secrets and Files storage keys; mixed UUIDv4/UUIDv7 relationships; and byte-for-byte UUIDv4 preservation through restore. Dedicated token/crypto helpers and documented unit, regression, seed, fixture, and recovery-drill generators remain outside the production baseline.
