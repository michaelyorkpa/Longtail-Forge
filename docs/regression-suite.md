...
As of 0.33.27.4, `framework.identifier-authority` makes `src/core/identifiers.js` the only allowed `uuid` package entry point, proves that the authority exposes only record UUIDv7 and opaque UUIDv4 operations, pins every audited framework persistent-record generator to `createRecordId()`, every audited operational generator to `createOpaqueId()`, and the exact 28 server-side module record calls across 11 authoritative owners. The guard also freezes the existing session, CSRF, API-key secret, private-feed credential, password, Secure Notes, and related dedicated crypto-helper patterns. The direct-generator baseline now contains only the exact Clients/Projects browser generator assigned to 0.33.27.5. Clients/Projects, Tasks/checklist, Notes/revision/link/collection, and Lists/item integration owners prove UUIDv7 generation, caller-supplied UUIDv4 preservation, mixed-version parent/child relationships, and unchanged module-owned timestamp, due-date, sequence, title, revision, timer-slot, and paging order. Documented unit, regression, seed, fixture, and recovery-drill generators remain outside the production baseline.
