...
As of 0.33.27.1, `framework.identifier-authority` makes `src/core/identifiers.js` the only allowed `uuid` package entry point, proves that the authority exposes only record UUIDv7 and opaque UUIDv4 operations, and compares every production runtime/browser/operator `randomUUID` call against the classified temporary migration baseline. The baseline must match exactly and may only shrink as 0.33.27.2-0.33.27.5 convert framework records, operational UUIDs, module records, and browser creation in order. Dedicated token/crypto helpers and documented unit, regression, seed, fixture, and recovery-drill generators remain outside the production baseline.
