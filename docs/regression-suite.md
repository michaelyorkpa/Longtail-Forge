...
As of 0.33.27.3, `framework.identifier-authority` makes `src/core/identifiers.js` the only allowed `uuid` package entry point, proves that the authority exposes only record UUIDv7 and opaque UUIDv4 operations, pins every audited framework persistent-record generator to `createRecordId()` and every audited operational generator to `createOpaqueId()`, and freezes the existing session, CSRF, API-key secret, private-feed credential, password, Secure Notes, and related dedicated crypto-helper patterns. The framework record and operational entries are gone; the baseline contains only the exact first-party module and Clients/Projects browser generators assigned to 0.33.27.4-0.33.27.5. Existing integration owners prove UUIDv7 persistent identities; UUIDv4 request, lock, local/S3 storage, whole-instance/workspace backup, purge-fence, demo-data, deploy-operation, and temporary-path identities; separation from accompanying durable rows; mixed UUIDv4/UUIDv7 relationships; and byte-for-byte restore preservation. Documented unit, regression, seed, fixture, and recovery-drill generators remain outside the production baseline.
