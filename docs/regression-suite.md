...
The current suite contains 406 discovered scripts: 311 active paths in `scripts/regression-legacy-snapshot.json` plus 95 convention-path metadata guardrails. The only post-snapshot retirement is the credited `check-js.mjs` assertion movement to the cached ESLint stage; every stateful script and all high-risk contract coverage remain registered. The 46 required release-gate entries include the rename-aware GitHub-only documentation classifier and its protected Development/nightly routing contract, including the rule that runtime Help retains artifact and deployment handling. The isolated private-calendar owners split cleanly: framework coverage proves authentication, lifecycle revocation, trusted-IP throttling, provider dispatch, and secret-free persistence/logging, while Tasks coverage proves permission-shaped bounded RFC 5545 content. The native SQLite data-compatibility owner proves the pinned driver, fresh migration identity, transaction and deferred-foreign-key behavior, WAL/concurrency and busy-timeout semantics, bindings/results/BLOBs, FTS5 ranking, checkpoint/reopen persistence, and clean integrity/foreign-key results.
