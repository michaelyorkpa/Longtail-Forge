...
As of 0.33.22.8, `framework.calendar-subscription-settings` pins the User Settings self-service seam: shared Settings anatomy, masked read-only URL control, generate/reveal/copy/rotate/disable actions, official client guidance, periodic read-only framing, page-memory-only secret handling, explicit destructive confirmations, lifecycle route ownership, and current Help wording. `framework.private-calendar-feed-authentication` additionally proves that authenticated status reads expose lifecycle metadata but never `feedUrl`; `user-settings-appearance.spec.mjs` renders the complete one-time URL lifecycle without mutating a real token.

As of 0.33.22.9, `framework.private-calendar-feed-authentication` also owns the named collection contract: multiple same-owner scopes, one-time URLs, safe list metadata, owner-only rotation, administrator cross-owner revocation, role-loss and Tasks-disable invalidation, generic fail-closed public reads, and zero active orphans after lifecycle reconciliation. `database.private-calendar-subscriptions-migration` independently proves eligible legacy selector/digest continuity, invalid and disabled legacy revocation, workspace-scoped target foreign keys, and SQLite integrity.
...
The current suite contains 407 discovered scripts: 311 active paths in `scripts/regression-legacy-snapshot.json` plus 96 convention-path metadata guardrails. The only post-snapshot retirement is the credited `check-js.mjs` assertion movement to the cached ESLint stage; every stateful script and all high-risk contract coverage remain registered. The 47 required release-gate entries include the Calendar Subscription Settings closeout owner and the rename-aware GitHub-only documentation classifier with its protected Development/nightly routing contract, including the rule that runtime Help retains artifact and deployment handling. The isolated private-calendar owners split cleanly: framework coverage proves authentication, lifecycle revocation, trusted-IP throttling, provider dispatch, secret-free persistence/logging, and one-time self-service URL handling, while Tasks coverage proves permission-shaped bounded RFC 5545 content. The native SQLite data-compatibility owner proves the pinned driver, fresh migration identity, transaction and deferred-foreign-key behavior, WAL/concurrency and busy-timeout semantics, bindings/results/BLOBs, FTS5 ranking, checkpoint/reopen persistence, and clean integrity/foreign-key results.

| Bucket | Registered scripts | Declared mode | Declared concurrency | Current safety boundary |
| --- | ---: | --- | ---: | --- |
| `static/source regressions` | 197 | parallel | 6 | Read-only/parallel-safe checks only; these do not receive a runner database fixture, and database access from a regression entry point is refused unless the script selected an OS-temp database before importing runtime/database modules. |
...
| `isolated database regressions` | 174 | parallel | 4 fallback | Database-backed checks receive per-script fixture environments. The runner auto-tunes isolated parallelism with a conservative cap while preserving explicit environment overrides. |
...
The default full run uses the table order above: 197 cheap static/source checks run first, followed by 6 serial default-database checks, 20 retained serial Files checks, 9 isolated Files checks with adaptive safe parallelism, and 174 isolated-database checks with adaptive safe parallelism. Each bucket prints actual wall time as well as summed script time and its longest script. The runner executes buckets sequentially and stops after the first failing bucket.

This is an explicit ordering guarantee, not a coverage reduction. The flattened bucket paths must remain exactly equal to the 406 discovered registry entries, each bucket retains its declared concurrency and fixture boundary, and narrow area/tag/tier filters preserve the relative order of whichever buckets they select. `LTF_REGRESSION_BUCKET=file-storage` selects both Files buckets; `isolated-files` selects only the audited parallel subset. A focused runner regression seeds a static failure and proves that no stateful bucket is scheduled. Typecheck, Vitest, and cached ESLint run before this sequence without replacing it.
