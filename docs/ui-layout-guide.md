...
Dashboard keeps framework page/region anatomy in `public/css/dashboard.css`. Module panel layout belongs beside the module renderer (`tasks-dashboard.css`, `time-tracking-dashboard.css`) and is loaded from the permission-filtered Dashboard browser-asset catalog. Preserve the month/week/day segmented controls as real `type="button"` controls with an updated `aria-pressed` state, keep task-editor focus return, and retain the 720px framework and 640px Tasks stacking rules when changing the module entry or asset loader.

The shared read-only calendar renderer uses the canonical 700px breakpoint for its Automatic initial view: Day on mobile and Month above mobile. Dashboard and Actions Calendar both honor the per-user Day/Week/Month override, while an explicit Actions Calendar `?view=` wins. Keep this as client-side presentation over the same server-shaped calendar window; do not create a mobile-only data path or duplicate the Day renderer.

The Dashboard native entry and `LongtailForge.esModuleBridge` are page-local transition machinery, not a framework-wide frontend loader. Tasks and Time Tracking are the two real consumers of the generalized Dashboard `browserAssets` contribution path; the generic host loads both modules' scripts/styles without naming their renderer IDs or paths. As of 0.33.21.19.4, the entry starts the workspace- and release-keyed stale-while-revalidate Dashboard manifest read after only the navigation/API foundation is ready, warms each contribution's live `dataRoute` in one route-keyed promise map, and overlaps those `no-store` reads with parallel host and contributed-asset loading. Core Dashboard anatomy may render before contribution scripts finish. The bounded calendar warm route remains anchored to `new Date()` (Today), and `task-dialog.js` loads only when a Dashboard calendar task is opened. Another page must measure and qualify its own conversion, preserve its current accessibility and responsive behavior, and satisfy the Two-Module Rule before any shared loader is extracted.
- Below the mobile breakpoint the framework shell hides the `span.site-brand-name` wordmark while keeping global Search and Notifications in the main header immediately before the hamburger toggle; `#primary-menu` contains navigation items only. The framework renders that primary navigation as a fixed left-side drawer owned by `public/js/navigation.js`: the `.nav-toggle` control opens `#primary-menu` (`.nav-links.is-open`) above a `.nav-drawer-overlay` click-away backdrop, body scroll locks while open (`body.nav-drawer-open`), focus moves into the drawer on open, Escape/overlay/toggle all close it, and closing returns focus to the toggle, which stays visible above the overlay. Desktop navigation above the breakpoint is unchanged, and modules must not restyle or repurpose the drawer anatomy.
- At the mobile breakpoint Workbench places its icon-only `Open Inspector` action immediately before `Change Focus` and presents the existing Inspector as a shared full-screen slide-out with contained focus, Escape/close dismissal, focus return, and scroll lock. Above the phone breakpoint that header action is hidden; the existing Inspector remains absent at intermediate narrow widths and resumes its unchanged right-side grid placement on wide Workbench layouts.
- At the same mobile breakpoint, Workbench Task Focus uses a one-column summary header so status, priority, due-date, and direct-tag chips wrap below the task title and Client/Project context. Each `Other Active Timers` summary uses a caret/title row followed by that card's own wrapping source/status chip row; the timer body and actions remain grouped inside the same card. Desktop keeps the compact side-by-side summary placement.
...
`public/js/shared/settings-host.js` owns the framework page/anatomy layer. Workspace, User, Tasks, Time Tracking, Files, Workbench, and Developer Example Settings HTML each contain one `data-settings-host` mount; the host builds their framework fields, individual or grouped sections, action rows, statuses, operational readouts, and dialogs through view primitives, then exposes `data-settings-attachment="workspace|user|module|new-workspace"` targets. Page adapters read eligible contributed sections from `/api/settings/catalog` and never use the retired implicit module-settings anchors. A protected framework contribution may use the module attachment for information architecture without becoming a feature-module manifest; the generic module adapter routes its submitted fields through `frameworkSettings` based on catalog target metadata.
