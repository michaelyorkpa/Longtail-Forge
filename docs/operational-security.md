...
The `/api/private-feeds/calendar-subscriptions` collection/item routes are browser-session protected and require `workspace_settings.manage`. Settings -> Admin -> Modules -> Calendar is their sole management surface; User Settings and the retired singular lifecycle routes cannot manage subscriptions. Create always binds the new row to the authenticated actor, owner-only rotation returns a replacement URL once, and safe list reads expose only lifecycle, owner, scope, and effective owner-timezone metadata—never URLs, selectors, digests, or token prefixes. Workspace administrators may revoke another owner's active row but cannot rotate it or recover its secret. The Calendar page remains available for listing and revocation while Tasks is disabled; create and rotate fail closed until Tasks and the owner's required live scope are active.
