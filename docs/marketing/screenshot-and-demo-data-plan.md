...
The shipped fat `development-seed` and separate `sanitized-demo` profiles seed Business/Personal/Family workspaces with seven private role fixtures, clients/projects, a full spread of task states (due, overdue, upcoming, blocked, recurring, completed, undated), checklists, next actions, resume context, work-resume state, active/paused/completed timers, manual time, Notes (collections/links/tags/revisions/safe Markdown), reusable/active/finalized/partial Lists, tiny harmless Files fixtures, notifications/reminders, Search, Dashboard, and Workbench Focus Selection / Task Focus inputs. These polished worlds are distinct from the oversized scale/stress dataset.

Set a unique local `SUPER_ADMIN_PASSWORD`, configure the ignored seven-role credential document, and run `npm run dev:data:seed` for the canonical local world as documented in [Development and Demo Data](../development-and-demo-data.md). Use the matching development reset before rebuilding it. `npm run demo:data:seed` remains available when a separate capture-only `data/sanitized-demo` directory is useful. The named public demo derives six public visitor accounts from the same fictional roles/scopes but retains a separate private operator; never show or reuse that operator in captures. Generated databases, Files objects, private credentials, and capture-time sessions remain runtime material and must not be committed or reused for invited users.
...
```
Examples:
- `ltf__workbench-task-focus__northwind__desktop__v0.33.25.6__2026-07-30.png`
- `ltf__mobile-dashboard__northwind__mobile__v0.33.25.6__2026-07-30.png`
