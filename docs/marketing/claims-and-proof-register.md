...
- **Classification:** `current` (shipped in the 0.33.25.6 repository baseline) · `preview` (bounded private-preview posture with invitation decision still gated) · `planned` (committed roadmap, not shipped) · `commercial-hypothesis` (later commercial/hosted, no commitment).
- **Verified at:** the app version / date the claim was last checked. Current baseline: **v0.33.25.6 · 2026-07-30**.
