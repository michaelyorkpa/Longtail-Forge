...
- Lists items table (display) refinement — the final Lists sub-slice of the .18.5 line. Tuned the read-only items table that lists existing items on the Lists detail page (`listsItemRowsDescriptor` columns + `createItemRow` + `.lists-items-table`); this is the table display, distinct from .18.5.9's item *entry* form.
- New column set/order: Done · **Item** · **Qty** · **Cost** · **Needed By** · **Status** · Actions. Dropped the **Assigned** column (assignment stays editable in the item entry form, it just isn't a table column) and added a dedicated **Cost** column (`applyItemCostCell` shows actual-or-estimated formatted, with an `Estimated … · Actual …` tooltip) instead of burying cost in the old row-meta line.
...
- Item names are capped at ~20 characters with an ellipsis (`truncateItemName`), with the full name kept in the cell `title`; the Item column absorbs the freed width. Renamed the table heading "Needed" → "Needed By".
...
- Follow-up 2026-06-17 23:30 -04:00: the item-form "Details" disclosure had no caret — `.lists-item-advanced summary` set `display: grid`, which replaces the summary's default `display: list-item` and removes the native disclosure marker. Dropped the display override (kept `cursor`/`font-weight`) so the caret returns, matching the framework collapsible panels. Added a durable rule to `docs/ui-layout-guide.md` (disclosure summaries must keep their native marker; do not flex/grid the caret away — add an explicit `::after` marker if a flex/grid summary is truly needed) enforced by `view-builder-closeout`, plus a `lists-ui-workflow` guard that `.lists-item-advanced summary` carries no `display: grid/flex`. Bumped `longtail-forge.css?v=28` on `lists.html`.

## Version 0.33.5.18.5.8 - 2026-06-17 21:30 -04:00

- Lists main page refinement (first Lists sub-slice of the .18.5 line; reuses the Notes detail patterns). Reorganized the detail into a clear top-to-bottom flow: header (identity) → description → Next (what to do) → Source (only when meaningful) → Linked Records → add-item form → the items table → the Costs rollup. The Costs panel now sits **below** the items table it totals (was above).
- Rebuilt the detail header to mirror Notes: a title row (title + Reusable/BOM/Working-Copy badges on the left, a 3-dot overflow action menu on the right via `view.renderDescriptorActionMenu`), a rule, then a compact labeled meta line (`detailMetaItems` — Status/Type/Context/Created/Updated/Finalized as `.lists-detail-meta` spans with `Label: value` tooltips). Moving the workflow actions into the "..." menu (replacing the wide inline `renderDescriptorActionStrip` row) also fixes the items list overlapping the detail action buttons.
...
- Notes detail metadata + panels refinement. The detail metadata row now carries all note-level metadata: `detailMetaItems` adds Ticket, Created, Updated, and Owner, where Owner renders the display name (resolved server-side — `attachNoteIntegrations` now adds `owner_display_name` via `resolveNoteOwnerLabel`, falling back to the id). The duplicated read-only linked-context `<dl>` (Client/Project/Task/User — already shown in the Linked Records panel) was removed, along with the now-dead `addContext`/`addLinkedContext`/`sectionHeading` helpers and `.notes-context-list` CSS.
...
- The Files panel is now a collapsible `<details class="notes-files-panel">` (collapsed by default) with no redundant outer box — the file-attachments component already provides its own bordered surface.
...
- Follow-up 2026-06-17 19:30 -04:00: gave the detail Files panel the same outer box as Linked Records and Revisions. The Files `<details>` now carries `notes-detail-section` (the shared boxed-section treatment), and the embedded `file-attachments` component drops its own surface chrome (border/padding/background) and its redundant "Files" heading inside `.notes-files-panel` — leaving a single outer box (with the attachment count kept, right-aligned) instead of the previous unboxed summary + nested widget box. The editor-modal Files panel is unaffected (it mounts the component standalone, outside `.notes-files-panel`). Bumped `notes.js?v=30` and `longtail-forge.css?v=35`.

## Version 0.33.5.18.5.6 - 2026-06-17 16:40 -04:00

- Notes navigation standardization. Standardized the action-page disclosure headings: the framework `.view-filter-panel-title` (the "Filters" heading) was plain weight while the collapsible-index summaries (Library, Notes List) were bold — made the Filters heading bold/clickable too so Filters, Library, and Notes List read consistently across action pages (also visible on Lists).
- Simplified the Notes Library panel: retired the All/Active/Ongoing Areas/Reference Library/Archive bucket-tab buttons (and `updateBucketTabs`) in favor of a single tight row — Library dropdown · Collection dropdown · selected-collection actions · New Collection icon. The Library dropdown gained an "Archive" option and is now the sole bucket selector; `selectBucket`/`renderCollections` handle the archive bucket through it.
...
- `notes.js` now renders the Notes detail workflow actions through a new framework overflow-menu primitive — `view.createDetailActionMenu` / `view.renderDescriptorActionMenu` — which preserves the compact three-dot ("...") disclosure while owning the dropdown anatomy in the framework. The menu is driven by the descriptor `detail.actionStrip` actions and dispatches through a registered behavior map (`NOTE_WORKFLOW_HANDLERS` + `view.registerBehavior`), showing Edit + Archive/Restore by note status while keeping archive/restore service logic (`/api/notes/:id/archive`, `/restore`) in the module.
...
- Bugfix 2026-06-16 17:35 -04:00: fixed a Notes page load failure (`ReferenceError: can't access lexical declaration 'NOTE_WORKFLOW_HANDLERS' before initialization`). `buildNotesViewShell()` runs at module top, calling `registerNotesViewBehaviors()` before the `const NOTE_WORKFLOW_HANDLERS` (declared later) had initialized — a temporal-dead-zone error. Moved the declaration above the `buildNotesViewShell()` call.
...
- Bugfix 2026-06-16 15:45 -04:00 (framework renderer): fixed the surface being wiped to an empty shell after opening the Create modal on a module-owned surface. The framework no longer re-renders a surface whose data it does not own — `runBehaviorAction`/the action error path only call `rerenderState` when `descriptor.dataSource.route` is set. Previously, clicking Create (a registered behavior) ran `rerenderState`, which re-rendered the descriptor layout and discarded the module-mounted chrome (Library panel, populated filters) and loaded data on Notes and Lists, so cancelling left a blank page until a manual refresh.
- Bugfix 2026-06-16 16:20 -04:00 (framework modals, follow-up): fixed every framework modal rendering visible/stacked on page load. The previous modal-sizing change set `display: flex` directly on `.view-modal`, which overrode the browser's `dialog:not([open]) { display: none }` and forced all closed dialogs (the module-built editors and the renderer's auto-rendered `descriptor.modals` duplicates) to show — so opening Lists/Notes popped open empty editors whose Close button did nothing (the dialogs were never actually `showModal()`-opened). The flex layout is now scoped to `.view-modal[open]`, so closed dialogs keep the user-agent `display: none`. Also stopped the renderer from rendering duplicate modal shells on Notes and Lists (both pass `modals: []` to `renderSurface` since they build and own their own dialogs), removing the stray second modal that appeared in the page body.
- Bugfix 2026-06-16 15:45 -04:00: bumped cache-busts for the changed shared assets on the affected pages — `notes.html` (`css?v=27`, `view-builder.js?v=5`, `view-renderer.js?v=3`, `notes.js?v=19`) and `lists.html` (`css?v=23`, `view-renderer.js?v=3`, `lists.js?v=7`) — and updated the regressions asserting those values in lockstep. (Tasks modals use the legacy `.form-actions` footer, which overrides the shared rules, so they are unaffected.)
